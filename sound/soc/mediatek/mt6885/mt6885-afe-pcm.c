...
static int mt6885_afe_runtime_resume(struct device *dev)
{
	...
	if (afe_priv->extern_adc_support)
		regmap_update_bits(afe->regmap, AFE_MEMIF_CON0,
			CPU_HD_ALIGN_MASK_SFT, 1 << CPU_HD_ALIGN_SFT);
	else
...
static void get_board_extern_adc_supprot(struct device_node *node,
	struct mt6885_afe_private *afe_priv)
{
	unsigned int val;

	if (!of_property_read_u32(node, "extern_adc_support", &val)) {
		if (val != 0)
			afe_priv->extern_adc_support = true;
		else
			afe_priv->extern_adc_support = false;
	} else {
		afe_priv->extern_adc_support = false;
	}
	pr_info("%s, extern_adc_support is %s\n", __func__,
		afe_priv->extern_adc_support ? "true" : "false");
}
...
static int mt6885_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	get_board_extern_adc_supprot(pdev->dev.of_node, afe_priv);
