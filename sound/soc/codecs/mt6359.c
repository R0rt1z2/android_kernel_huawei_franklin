...
#include "mt6359.h"
#include "extern_adc/extern_adc.h"
...
enum {
	EXT_ADC_MUX_CLOSE = 0,
	EXT_ADC_MUX_MUTE_OPEN,
	EXT_ADC_MUX_MASK = 0x2,
};

static const char * const ext_adc_mux_map[] = {
	"Close", "Open"
};

static int ext_adc_mux_map_value[] = {
	EXT_ADC_MUX_CLOSE,
	EXT_ADC_MUX_MUTE_OPEN,
};

static SOC_VALUE_ENUM_SINGLE_DECL(ext_adc_mux_map_enum,
				  SND_SOC_NOPM,
				  0,
				  EXT_ADC_MUX_MASK,
				  ext_adc_mux_map,
				  ext_adc_mux_map_value);

static const struct snd_kcontrol_new ext_adc_mux_control =
	SOC_DAPM_ENUM("EXT_ADC Select", ext_adc_mux_map_enum);
...
static const struct snd_soc_dapm_widget mt6359_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("EXT_ADC Select", SND_SOC_NOPM, 0, 0, &ext_adc_mux_control),
	...
	SND_SOC_DAPM_OUTPUT("EXT_ADC_OUT"),
...
static const struct snd_soc_dapm_route mt6359_dapm_routes[] = {
	...
	{"EXT_ADC_IN", NULL, "MIC_BIAS_0"},
	{"EXT_ADC_IN", NULL, "MIC_BIAS_2"},
	{"EXT_ADC Select", "Open", "EXT_ADC_IN"},
	{"EXT_ADC_OUT", NULL, "EXT_ADC Select"},
...
static int mt6359_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (priv->extern_adc_enable) {
		pr_info("%s() add extern adc kcontrol\n", __func__);
		extern_adc_add_kcontrol(codec);
	}
	return 0;
}
...
static void get_extern_adc_cfg_enable(struct device_node *node,
	struct mt6359_priv *priv)
{
	unsigned int val;

	if (!of_property_read_u32(node, "extern_adc_enable", &val)) {
		if (val != 0)
			priv->extern_adc_enable = true;
		else
			priv->extern_adc_enable = false;
	} else {
		priv->extern_adc_enable = false;
	}
}
...
static int mt6359_platform_driver_probe(struct platform_device *pdev)
{
	...
	get_extern_adc_cfg_enable(pdev->dev.of_node, priv);
