...
out_max>=encoded_bytes_max(in_max), =true otherwise */
{
	...
	const uint_fast32_t bytes_left = r_bytes_max - REPEAT_MIN - r_mask;
	if (likely(r_bytes_max - REPEAT_MIN < r_mask))
		return out_at;
	if (unlikely(check_out &&
	   (!out_at || size_bytes_count(bytes_left) > u_32(out_end - out_at))))
		return NULL;
	return out_size_bytes(out_at, bytes_left);
} /* out_r_bytes_left */
...
inline static uint_fast32_t hash_v(const uint64_t r, uint32_t shift)
{
	return hash64v_7b(r, shift);
}

inline static uint_fast32_t hash(const uint8_t *r, uint32_t shift)
{
	return hash64_7b(r, shift);
}
...
{ /* caller guarantees off_log2 <=16 */
	...
	const uint8_t *r = in0;
	const uint8_t *nr0 = r++;
	const uint8_t *q = hashed(in0, ht, hash(r, HT_LOG2), r);
	for (;; q = hashed(in0, ht, hash(r, HT_LOG2), r), nr0 = r) {
		uint_fast32_t step = 1 << STEP_LOG2;
		uint_fast32_t utag = 0;
		const uint8_t *r_end = 0;
		uint_fast32_t r_bytes_max = 0;
		for (; !equal4(q, r); q = hashed(in0, ht, hash(r, HT_LOG2), r))
		...
		if (equal8(q = r - 1, r)) {
			r_end = repeat_end(q, r, in_end_safe, in_end);
			r_bytes_max = u_32(r_end - r);
			out_at = out_repeat(out_at, out_end, 1, r_bytes_max,
					    nr_log2, off_log2, check_out);
			if (unlikely(check_out && out_at == NULL))
				return LZ4K_STATUS_WRITE_ERROR;
			if (unlikely((r += r_bytes_max) > in_end_safe))
		...
		ht[hash(r - 1 - 1, HT_LOG2)] = (uint16_t)(r - 1 - 1 - in0);
	} /* for */
} /* encode_any */
...
{
	static const char *version = "2020.07.22";
	return version;
}
/*lint -e580*/
