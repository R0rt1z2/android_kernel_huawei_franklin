...
static struct queue_sysfs_entry queue_mas_direct_flush_entry = {
	.attr = {.name = "direct_flush", .mode =  S_IWUSR | S_IRUSR },
	.store = mas_queue_direct_flush_store,
};

extern ssize_t mas_queue_panic_dump_store(
	struct request_queue *q, const char *page, size_t count);
static struct queue_sysfs_entry queue_mas_panic_dump_entry = {
	.attr = {.name = "panic_dump", .mode =  S_IWUSR | S_IRUSR },
	.store = mas_queue_panic_dump_store,
};

ssize_t mas_queue_panic_store(
	struct request_queue *q, const char *page, size_t count)
{
	ssize_t ret;
	unsigned long val;
...
static struct queue_sysfs_entry queue_mas_panic_test_entry = {
	.attr = {.name = "panic", .mode =  S_IWUSR | S_IRUSR },
	.store = mas_queue_panic_store,
};

static struct queue_sysfs_entry queue_mas_flush_reduce_entry = {
	.attr = {.name = "flush_reduce", .mode =  S_IWUSR | S_IRUSR },
	.show = mas_queue_flush_reduce_show,
	.store = mas_queue_flush_reduce_store,
};

extern ssize_t mas_queue_sr_store(
	struct request_queue *q, const char *page, size_t count);
static struct queue_sysfs_entry queue_mas_sre_entry = {
	.attr = {.name = "suspend_resume", .mode =  S_IWUSR | S_IRUSR },
	.store = mas_queue_sr_store,
};
...
#endif
#if 1
	&queue_mas_flush_reduce_entry.attr,
	&queue_mas_direct_flush_entry.attr,
	&queue_mas_panic_dump_entry.attr,
	&queue_mas_panic_test_entry.attr,
	&queue_mas_sre_entry.attr,
#endif
#endif /* CONFIG_MAS_DEBUG_FS */
