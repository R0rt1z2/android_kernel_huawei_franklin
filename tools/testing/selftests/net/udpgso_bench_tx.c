...
static char data_buf[NUM_PKT][ETH_MAX_MTU];
...
static int send_udp_sendmmsg(int fd, char *data)
{
	...
	memset_s(mmsgs, sizeof(mmsgs), 0, sizeof(mmsgs));
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < sizeof(data_buf[0]); i++)
		data_buf[0][i] = 'a' + (i % 26);
	for (i = 1; i < NUM_PKT; i++) {
		int err;
		err = memcpy_s(data_buf[i], ETH_MAX_MTU, data_buf[0], sizeof(data_buf[0]));
		if (err != EOK)
			error(1, err, "memcpy_s");
	}
	...
	do {
		if (cfg_tcp)
			num_sends += send_tcp(fd, data_buf[i]);
		else if (cfg_segment)
			num_sends += send_udp_segment(fd, data_buf[i]);
		else if (cfg_sendmmsg)
			num_sends += send_udp_sendmmsg(fd, data_buf[i]);
		else
			num_sends += send_udp(fd, data_buf[i]);
		num_msgs++;
