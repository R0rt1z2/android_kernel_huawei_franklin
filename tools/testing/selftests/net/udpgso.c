...
static char data_buf[ETH_MAX_MTU];
...
static unsigned int get_device_mtu(int fd, const char *ifname)
{
	...
	err = memset_s(&ifr, sizeof(ifr), 0, sizeof(ifr));
	if (err != EOK)
		error(1, err, "memset_s");

	err = strcpy_s(ifr.ifr_name, IFNAMSIZ, ifname);
	if (err != EOK)
		error(1, err, "strcpy_s");
...
static void set_route_mtu(int mtu, bool is_ipv4)
{
	...
	if (is_ipv4)
		memcpy_s(RTA_DATA(rta), alen, &addr4, alen);
	else
		memcpy_s(RTA_DATA(rta), alen, &addr6, alen);
...
static int recv_one(int fd, int flags)
{
	...
	ret = recv(fd, data_buf, sizeof(data_buf), flags);
