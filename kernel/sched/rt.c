...
#ifdef CONFIG_HUAWEI_RT_BLOCKED_REASON
	if (flags & ENQUEUE_WAKEUP) {
		if (schedstat_enabled()) {
			u64 block_start = schedstat_val(p->se.statistics.block_start);
...
out:
#if defined(CONFIG_HW_RT_CAS) && defined(CONFIG_HW_RTG_NORMALIZED_UTIL)
