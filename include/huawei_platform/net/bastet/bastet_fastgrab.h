...
#define bst_fg_set_acc_state(sk, value) do { \
		(sk)->acc_state = (value); } while (0)
...
#define bst_fg_set_discard_timer(sk, value) do { \
		(sk)->discard_duration = (value); } while (0)

#ifdef CONFIG_HW_DPIMARK_MODULE
#define bst_fg_set_app_type(sk, value) do { \
		(sk)->__sk_common.skc_hwdpi_mark |= (value << 8); } while (0)

#define bst_fg_init_app_type(sk) do { \
		(sk)->__sk_common.skc_hwdpi_mark &= (0xFFFF00FF); } while (0)
#endif
