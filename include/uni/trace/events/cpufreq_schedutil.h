...
TRACE_EVENT(cpufreq_schedutil_get_util,
	    TP_PROTO(unsigned int cpu,
		     ...
		     unsigned int ed,
		     unsigned int od),
	    TP_ARGS(cpu, util, max, top, pred, max_pred, cpu_pred, iowait,
		    flag, ed, od),
	    TP_STRUCT__entry(
		    __field(unsigned int,	cpu)
		    __field(unsigned long,	util)
	    ...
	    TP_printk("cpu=%u util=%lu max=%lu top=%lu pred=%lu,%lu,%lu "
		      "iowait=%u flag=%u ed=%u od=%u", __entry->cpu,
		      __entry->util, __entry->max, __entry->top, __entry->pred,
		      __entry->max_pred, __entry->cpu_pred, __entry->iowait,
		      __entry->flag, __entry->ed, __entry->od)
);
