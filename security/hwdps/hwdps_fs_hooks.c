...
uint8_t *hwdps_do_get_attr(struct inode *inode, size_t size, u32 flags)
{
	...
	if ((flags == HWDPS_XATTR_ENABLE_FLAG &&
		size != HWDPS_ENCODED_WFEK_SIZE_V2) ||
		(flags == HWDPS_XATTR_ENABLE_FLAG_NEW &&
	...
	err = inode->i_sb->s_cop->get_hwdps_attr(inode, wfek, size, NULL);
	if (err == -ENODATA) {
	...
	void *fs_data, u32 parent_flags)
{
	...
	if ((parent_flags & HWDPS_ENABLE_FLAG) != 0) {
	...
	const struct dentry *dentry, void *fs_data, struct page *page)
{
	...
	err = hwdps_create_fek(parent->i_crypt_info->ci_master_key_descriptor,
		inode, dentry, &buffer_fek_efek, parent_flags);
