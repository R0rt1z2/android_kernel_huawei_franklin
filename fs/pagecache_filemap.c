...
static void file_map_hitrate_set_ino(unsigned long idx,
					    unsigned long ino)
{
...
{
	struct file_map_uid_hitrate_s hitrate_tmp;
	struct file_map_uid_hitrate_s *ph = &hitrate_tmp
	unsigned long i = 0;
	long h, m, r;
	...
	list_for_each_entry(pos, &file_map_history_list.head, list) {
		if (uid == -1 || pos->uid == uid)
			fm_print(log_level_info,
				 ...
				 i++, pos->uid, pos->state, pos->res,
				 pos->hitrate);
	}
	spin_unlock(&file_map_history_list.lock);
}

static void file_map_data_show_head(int id,
					   struct file_map_entry *pentry)
{
	struct rtc_time tm;
	unsigned long i;
...
static void file_map_data_show(void)
{
	...
	list_for_each_entry(pentry, &file_map.used, list) {
		file_map_data_show_head(id, pentry);
	}
...
#ifdef FILE_MAP_DEBUG
#define MAX_INPUT_PARAM_LENGTH	(128)
#define MAX_SINGAL_PARAM_LENGTH	(8)
static ssize_t file_map_proc_stats_write(struct file *filp,
			const char __user *buf, size_t count, loff_t *off)
{
	char param0[MAX_SINGAL_PARAM_LENGTH];
	char param1[MAX_SINGAL_PARAM_LENGTH];
	char kbuf[MAX_INPUT_PARAM_LENGTH];
	long val;

	if (copy_from_user(kbuf, buf, MAX_INPUT_PARAM_LENGTH - 1))
		return -EINVAL;
	kbuf[127] = '\0';

	if (sscanf(kbuf, "%7s %7s", param0, param1) != 2)
		return -EINVAL;

	if (!strncmp(param0, "-h", 2)) {
		if (!strncmp(param1, "0", 1)) {
			file_map_history_show(-1);
		} else {
			if (kstrtol(param1, 0, &val))
				return -EINVAL;
			file_map_history_show((uid_t)val);
		}
	} else if (!strncmp(param0, "-r", 2)) {
...
static int file_map_proc_stats_open(struct inode *inode,
					   struct file *filp)
{
...
{
	struct file_map_entry *entry = NULL, *tmp = NULL;
