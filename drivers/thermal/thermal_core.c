...
#ifdef CONFIG_HW_IPA_THERMAL
	bool is_ipa_cdev = !!(nametoactor(cdev->type) != -1);
#endif
...
#ifdef CONFIG_HW_IPA_THERMAL
		/* for IPA cdev, we only bind them to IPA tz */
		if (is_ipa_cdev && !is_ipa_thermal(pos))
			continue;
#endif
...
#ifdef CONFIG_HW_IPA_THERMAL
			/* for IPA cdev, we only bind them to IPA tz */
			if (nametoactor(pos->type) != -1 && !is_ipa_thermal(tz))
				continue;
#endif
...
#ifdef CONFIG_HW_IPA_THERMAL
		/* for IPA cdev, we only bind them to IPA tz */
		if (nametoactor(cdev->type) != -1 && !is_ipa_thermal(tz))
			continue;
#endif
		if (tz->ops->unbind) {
			tz->ops->unbind(tz, cdev);
			     ...
			     unsigned long mode, void *_unused)
{
	...
	case PM_POST_SUSPEND:
		...
		list_for_each_entry(tz, &thermal_tz_list, node) {
#ifdef CONFIG_HW_IPA_THERMAL
			if (strncmp(tz->governor->name, USER_SPACE_GOVERNOR, strlen(USER_SPACE_GOVERNOR))) {
				thermal_zone_device_reset(tz);
				thermal_zone_device_update(tz, THERMAL_EVENT_UNSPECIFIED);
			}
#else
