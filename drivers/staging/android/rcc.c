...
/*
 * Record the reason of rcc sleep and stop.
 * Sleep means that rcc was trying to run but failed.
 * Stop means that rcc was running but breaked out.
 */
static void count_rcc_s_reason(unsigned int stat, unsigned long long *s_reason,
	int num)
{
	int i;
...
static unsigned int get_system_stat(struct rcc_module *rcc, bool end)
{
	...
	if (end)
		count_rcc_s_reason(ret, rcc->stop_reason, RCC_S_REASON_NUM);
	else
		count_rcc_s_reason(ret, rcc->sleep_reason, RCC_S_REASON_NUM);
...
static ssize_t s_reason_show(char *buf, const unsigned long long *s_reason,
	int num)
{
	return scnprintf(buf, PAGE_SIZE,
		"cpu busy     : %llu\n"
		...
		"no anon      : %llu\n"
		"avail enough : %llu\n",
		s_reason[0],           // 0 : cpu is busy
		s_reason[1],           // 1 : free memory is enough
		s_reason[2],           // 2 : swap space is not enough
		s_reason[3],           // 3 : anon memory is not enough
		s_reason[4]);          // 4 : available memory is enough
}

static ssize_t sleep_reason_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	return s_reason_show(buf, rcc_module.sleep_reason, RCC_S_REASON_NUM);
}

static ssize_t stop_reason_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	return s_reason_show(buf, rcc_module.stop_reason, RCC_S_REASON_NUM);
}

static ssize_t s_reason_clear(unsigned long long *s_reason, int num)
{
	int i;
...
static ssize_t sleep_reason_clear(struct kobject *kobj,
			struct kobj_attribute *attr,
			const char *buf, size_t len)
{
	return s_reason_clear(rcc_module.sleep_reason, RCC_S_REASON_NUM);
}

static ssize_t stop_reason_clear(struct kobject *kobj,
			struct kobj_attribute *attr,
			const char *buf, size_t len)
{
	return s_reason_clear(rcc_module.stop_reason, RCC_S_REASON_NUM);
}
...
static RCC_ATTR(force_once, RCC_MODE_RW, force_once_show,
		force_once_store);
static RCC_ATTR(sleep_reason, RCC_MODE_RW, sleep_reason_show,
		sleep_reason_clear);
static RCC_ATTR(stop_reason, RCC_MODE_RW, stop_reason_show,
		stop_reason_clear);
...
static struct attribute *rcc_attrs[] = {
	...
	&kobj_attr_sleep_reason.attr,
	&kobj_attr_stop_reason.attr,
	NULL,
};
