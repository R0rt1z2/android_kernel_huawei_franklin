...
#define DEFAULT_TARGET_RATIO 200
...
int register_memlat(struct device *dev, struct memlat_hwmon *hw)
{
	...
	hw->freq_map = init_core_dev_map(dev, "core-dev-table");
	if (hw->freq_map == NULL) {
	...
	ret = of_property_read_u32(dev->of_node,
				   "switch-on-cpufreq", &temp);

	if (ret != 0) {
		dev_err(dev, "[%s] node %s doesn't have switch-on-cpufreq property!\n",
