...
#if defined(CONFIG_MEMORY_STATIC_FOOTPRINT)
...
#if defined(CONFIG_MEMORY_STATIC_FOOTPRINT)
static int d_reserved_cnt;
static const char *d_reserved_array[MAX_RESERVED_REGIONS];
static int cma_mem_reserved_count;
static const char *cma_mem_reserved_array[MAX_RESERVED_REGIONS];

struct reserved_mem *get_reserved_memory(int *array_len)
{
	*array_len = reserved_mem_count;
	return &reserved_mem[0];
}

const char **get_dynamic_reserved_memory(int *array_len)
{
	*array_len = d_reserved_cnt;
	return &d_reserved_array[0];
}

const char **get_cma_reserved_memory(int *array_len)
{
	*array_len = cma_mem_reserved_count;
	return &cma_mem_reserved_array[0];
}
#endif
...
#if defined(CONFIG_MEMORY_STATIC_FOOTPRINT)
	if ((d_reserved_cnt < MAX_RESERVED_REGIONS) && (d_reserved_cnt >= 0)) {
		d_reserved_array[d_reserved_cnt] = uname;
		d_reserved_cnt++;
	}
#endif
