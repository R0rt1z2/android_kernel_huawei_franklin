...
static inline struct mmc_card *get_card_from_mtk_msdc_host()
{
    bool card_check_invalid = (mtk_msdc_host[0] == NULL) ||
        (mtk_msdc_host[0]->mmc == NULL);
    if (card_check_invalid)
        return NULL;

    return mtk_msdc_host[0]->mmc->card;
}
