...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	if (host->mux_sdsim == 1) {
		if (unlikely(cmd->error &&
		    cmd->opcode == MMC_SEND_OP_COND &&
		    host->id == 1)) {
			dump_mmc_gpio_info();
			print_voltage_state_info(host);
		}
	}
#endif
...
int get_cd_state(struct msdc_host *host)
{
	if (host == NULL || host->hw == NULL) {
		pr_err("%s %s parameter is invalid\n",
	...
	pr_info("[msdc%d] card_insert state=%d\n",
		host->id, host->card_inserted);
	if (host->hw->cd_level == level)
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	if (mmc->caps & MMC_CAP_NONREMOVABLE) {
		...
		pr_info("[msdc%d] gpio_get_value level=%d, cd_gpio=%d\n",
			host->id, level, cd_gpio);
#endif
		host->card_inserted = (host->hw->cd_level == level) ? 1 : 0;
		pr_info("[msdc%d] card_insert state=%d\n",
			host->id, host->card_inserted);
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
		if (host->mux_sdsim == 1) {
			level = sd_sim_detect_run(host, !(level), MODULE_SD,
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	if (host->mux_sdsim == 1) {
		pr_info("%s %s volt switch not support for nano card,"
			"just return\n", MUX_SDSIM_LOG_TAG, __func__);
		return 0;
	}
#endif
...
static bool msdc_dev_rename(struct platform_device *pdev)
{
	int ret;
	int pri_id;
	struct device *dev = &pdev->dev;
	static const char *const mtkmsdc0 = "mtk_msdc0";
	static const char *const mtkmsdc1 = "mtk_msdc1";

	pri_id = of_alias_get_id(dev->of_node, "mtk-msdc");
	switch (pri_id) {
	case MMC_EMMC:
		pdev->name = mtkmsdc0;
		ret = device_rename(dev, mtkmsdc0);
		if (ret < 0) {
			pr_err("[msdc] %s dev set name %s fail\n",
			       __func__, mtkmsdc0);
			goto error;
		}
		break;
	case MMC_SD:
		pdev->name = mtkmsdc1;
		ret = device_rename(dev, mtkmsdc1);
		if (ret < 0) {
			pr_err("[msdc] %s dev set name %s fail\n",
			       __func__, mtkmsdc1);
			goto error;
		...
		break;
	default:
		pr_err("[msdc] %s pri_id is out of range\n", __func__);
		goto error;
	}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	ret = msdc_dev_rename(pdev);
	if (ret == false) {
		pr_err("[msdc] %s rename err, use default name\n", __func__);
	}
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	ret = msdc_sdsim_gpio_probe(&pdev->dev, host);
	if (ret != SDSIM_OK) {
		pr_err("%s %s gpios parse err:%d\n",
			MUX_SDSIM_LOG_TAG, __func__, ret);
		return -ENOMEM;
	}

	ret = msdc_sem_init(host);
	if (ret != SDSIM_OK) {
		pr_err("%s %s sem init err:%d\n",
			MUX_SDSIM_LOG_TAG, __func__, ret);
		return -ENOMEM;
	}

	ret = msdc_sdsim_set_sd_host(host);
	if (ret != SDSIM_OK) {
		pr_err("%s %s set sd host err:%d\n",
			MUX_SDSIM_LOG_TAG, __func__, ret);
		return -ENOMEM;
	}
#endif
	...
	if (host->mux_sdsim == 1) {
		mmc->caps2 |= MMC_CAP2_NMCARD;
		pr_info("%s %s NMCARD flag set\n",
			MUX_SDSIM_LOG_TAG, __func__);
	}
#endif
	if ((host->hw->host_function == MSDC_SD) && !(mmc->caps2 & MMC_CAP2_NMCARD)) {
		mmc->caps |= MMC_CAP_AGGRESSIVE_PM;
#ifdef NMCARD_SUPPORT
		mmc->caps2 |= MMC_CAP2_NMCARD;
#endif
	}
	if (!(mmc->caps2 & MMC_CAP2_NMCARD))
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	host->suspend_flag = MSDC_NMC_NORMAL;
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	if (host->suspend_flag != MSDC_NMC_SUSPEND)
		host->suspend_flag = MSDC_NMC_RUNTIME_SUSPEND;
	pr_info("%s:%d msdc runtime suspend.suspend_flag=%d.\n",
			__func__, __LINE__, host->suspend_flag);
	up_read(&host->suspend_rwsem);
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	host->suspend_flag = MSDC_NMC_NORMAL;
	pr_info("%s:%d msdc runtime resume.suspend_flag=%d.\n",
		__func__, __LINE__, host->suspend_flag);
#endif
	return 0;
}
...
static int msdc_suspend(struct device *dev)
{
	...
	might_sleep();
	down_read(&host->suspend_rwsem);
	host->suspend_flag = MSDC_NMC_SUSPEND;
	up_read(&host->suspend_rwsem);
#endif
