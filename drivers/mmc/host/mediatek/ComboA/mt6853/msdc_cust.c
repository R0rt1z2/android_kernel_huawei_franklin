...
int msdc_regulator_set_and_enable(struct regulator *reg, int powerVolt)
{
	...
	ret = regulator_set_voltage(reg, powerVolt, powerVolt);
	if (ret) {
		pr_err("%s %s regulator set vlotage fail, ret=%d\n",
		       MUX_SDSIM_LOG_TAG, __func__, ret);
		return ret;
	}
	ret = regulator_enable(reg);
	if (ret) {
		pr_err("%s %s regulator enable fail, ret=%d\n",
		       MUX_SDSIM_LOG_TAG, __func__, ret);
		return ret;
	}
#else
...
void dump_mmc_gpio_info(void)
{
#ifndef SD_GPIO_PAD_A_EN
	pr_info("MSDC1 MODE14: 0x%08x should: 0x0*******\n",
		MSDC_READ32(MSDC1_GPIO_MODE14));
	pr_info("MSDC1 MODE15: 0x%08x should: 0x111***00\n",
		MSDC_READ32(MSDC1_GPIO_MODE15));
	pr_info("MSDC1 MODE16: 0x%08x should: 0x*****111\n",
		MSDC_READ32(MSDC1_GPIO_MODE16));
	pr_info("MSDC1 IES: 0x%08x 5-0bits should: 2b111111\n",
		MSDC_READ32(MSDC1_GPIO_IES));
	pr_info("MSDC1 SMT: 0x%08x 5-0bits should: 2b111111\n",
		MSDC_READ32(MSDC1_GPIO_SMT));
	pr_info("MSDC1 DRV0: 0x%08x\n", MSDC_READ32(MSDC1_GPIO_DRV0));
	pr_info("MSDC1 PUPD0: 0x%08x 11-0bits should: 2b111***000001\n",
		MSDC_READ32(MSDC1_GPIO_PUPD0));
	pr_info("MSDC1 R0: 0x%08x 11-0bits should: 0x2b000***111110\n",
		MSDC_READ32(MSDC1_GPIO_R0));
	pr_info("MSDC1 R1: 0x%08x 11-0bits should: 0x2b000***111110\n",
		MSDC_READ32(MSDC1_GPIO_R1));
#endif
}

void print_voltage_state_info(struct msdc_host *host)
{
	if (!(host && host->mmc))
		return;
	int vmch_voltage = regulator_get_voltage(host->mmc->supply.vmmc);
	int vmch_isenable = regulator_is_enabled(host->mmc->supply.vmmc);
	int vmc_voltage = regulator_get_voltage(host->mmc->supply.vqmmc);
	int vmc_isenable = regulator_is_enabled(host->mmc->supply.vqmmc);

	pr_info("%s [msdc%d] vmch:vol=%d, en=0x%02x, vmc:vol=%d, en=0x%02x\n",
		__func__, host->id, vmch_voltage, vmch_isenable,
		vmc_voltage, vmc_isenable);

	if (host->mux_sdsim == 1) {
		int vsim2_isenable;
		int vsim2_vol_1_1 = 0;
		int vsim2_vol_1_2 = 0;
		int vsim2_release_state = 0;

		vsim2_isenable = regulator_is_enabled(host->mmc->supply.vsim2);
		pmic_read_interface(PMIC_RG_VSIM2_VOSEL_ADDR,
				    &vsim2_vol_1_1,
				    PMIC_RG_VSIM2_VOSEL_MASK,
				    PMIC_RG_VSIM2_VOSEL_SHIFT);
		pmic_read_interface(PMIC_RG_VSIM2_VOCAL_ADDR,
				    &vsim2_vol_1_2,
				    PMIC_RG_VSIM2_VOCAL_MASK,
				    PMIC_RG_VSIM2_VOCAL_SHIFT);
		pmic_read_interface(PMIC_RG_VSIM2_NDIS_EN_ADDR,
				    &vsim2_release_state,
				    PMIC_RG_VSIM2_NDIS_EN_MASK,
				    PMIC_RG_VSIM2_NDIS_EN_SHIFT);

		pr_info("%s [msdc%d] vsim2:vol_1_1=0x%02x, vol_1_2=0x%02x, "
			"en=0x%02x, release_state=%d\n",
			__func__, host->id, vsim2_vol_1_1, vsim2_vol_1_2,
			vsim2_isenable, vsim2_release_state);
	}
}

static enum sdsim_result mmc_card_gpioctl_with_power(struct msdc_host *host,
						     unsigned int on)
{
	int ret = SDSIM_OK;
	...
	if (sdsim_get_current_detect_status() != SD_SIM_DETECT_STATUS_SIM) {
		if (on) {
...
enum sdsim_result mmc_card_ldo_power(struct msdc_host *host, bool on)
{
	int ret = SDSIM_OK;
	...
	if (sdsim_get_current_detect_status() == SD_SIM_DETECT_STATUS_SD) {
		if (regulator_is_enabled(host->mmc->supply.vsim2)) {
			msdc_ldo_power(LDO_POWER_OFF, host->mmc->supply.vsim2,
				       VOL_1800, &host->power_flash_vsim2);
			mdelay(POWER_OFF_DELAY_MS_TIME);
		}

		if (on) {
			/* disable vsim2 release power function */
			pmic_config_interface(PMIC_RG_VSIM2_NDIS_EN_ADDR,
					      LDO_NDIS_DISABLE,
					      PMIC_RG_VSIM2_NDIS_EN_MASK,
					      PMIC_RG_VSIM2_NDIS_EN_SHIFT);

			//VMCH Power up with 3V
			msdc_ldo_power(LDO_POWER_ON, host->mmc->supply.vmmc,
				       VOL_3000, &host->power_flash);

			/* enable VMCH release power function */
			mt6360_ldo5_ndis_en(LDO_NDIS_ENABLE);

		} else {
			msdc_ldo_power(LDO_POWER_OFF, host->mmc->supply.vmmc,
				       VOL_3000, &host->power_flash);
			mdelay(POWER_OFF_DELAY_MS_TIME);

			/* disable vmch release power function */
			mt6360_ldo5_ndis_en(LDO_NDIS_DISABLE);

		}
		pr_info("%s %s VMCH 3V power state:%d\n",
			  MUX_SDSIM_LOG_TAG, __func__, on);
	} else {
		if (regulator_is_enabled(host->mmc->supply.vmmc)) {
			msdc_ldo_power(LDO_POWER_OFF, host->mmc->supply.vmmc,
				       VOL_3000, &host->power_flash);
			mdelay(POWER_OFF_DELAY_MS_TIME);
		}
		...
		if (on) {
			...
			/* VSIM2 Power up with 1.7V */
			msdc_ldo_power(LDO_POWER_ON, host->mmc->supply.vsim2,
				       VOL_1700, &host->power_flash_vsim2);

			/* VSIM2 Power up with 1.8V */
			msdc_ldo_power(LDO_POWER_ON, host->mmc->supply.vsim2,
				       VOL_1800, &host->power_flash_vsim2);

			/* enable vsim2 release power function */
			pmic_config_interface(PMIC_RG_VSIM2_NDIS_EN_ADDR,
					      LDO_NDIS_ENABLE,
					      PMIC_RG_VSIM2_NDIS_EN_MASK,
					      PMIC_RG_VSIM2_NDIS_EN_SHIFT);
		...
		pr_info("%s %s VSIM2 1.8V power state:%d\n", MUX_SDSIM_LOG_TAG,
			__func__, on);
	}
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
static void mmc_card_status_detect(bool on)
{
	if (sdsim_get_current_detect_status() == SD_SIM_DETECT_STATUS_SD) {
		if (on) {
			mdelay(POWER_ON_SD_DELAY_MS_TIME);
			pr_info("%s nmc delay 400ms before power on vmch",
				__func__);
		}
	}
}
...
void msdc_sd_power(struct msdc_host *host, u32 on)
{
	...
	case 1:
		...
		if (host->mux_sdsim != 1) {
			/* VMCH VOLSEL */
			msdc_ldo_power(card_on, host->mmc->supply.vmmc,
				       VOL_3000, &host->power_flash);
			/* VMC VOLSEL */
			...
			/* VMC VOLSEL */
#ifdef CONFIG_MMC_DW_MUX_SDSIM
			mmc_card_status_detect(on);
#endif
			msdc_ldo_power(on, host->mmc->supply.vqmmc,
				       VOL_1800, &host->power_io);
			pr_info("%s %s VMC_vqmmc 1.8V state:%d\n",
				MUX_SDSIM_LOG_TAG, __func__, on);
			/* VMCH VOLSEL */
			mmc_card_ldo_power(host, card_on);

			mmc_card_gpioctl_with_power(host, card_on);
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
static void msdc_sdsim_get_switch_gpio_nums(
	struct msdc_host *host, struct device_node *np)
{
	int value = SWITCH_GPIO_DEFAULT_NUMBER;

	value = of_get_named_gpio(np, "switch_gpio_nums", 0);
	if (!gpio_is_valid(value)) {
		value = SWITCH_GPIO_DEFAULT_NUMBER;
		pr_err("%s %s [msdc%d] switch_gpio_nums not found,"
		       "using value of 0xFFFF as default\n",
		       MUX_SDSIM_LOG_TAG, __func__, host->id);
	}
	pr_info("%s %s [msdc%d] switch_gpio_number = %x\n",
		  MUX_SDSIM_LOG_TAG, __func__, host->id, value);
	sdsim_set_switch_default_gpio(value);
}

static void msdc_sdsim_get_switch_sd_value(struct msdc_host *host,
					   struct device_node *np)
{
	int value;

	if (of_property_read_u32(np, "switch-sd-side", &value)) {
		pr_err("%s %s [msdc%d] switch-sd-side property not found,"
		       "using value of 0 as default\n", MUX_SDSIM_LOG_TAG,
		       __func__, host->id);
		value = 0;
	}

	sdsim_set_switch_sd_side_value(value);
	pr_info("%s %s [msdc%d] dts switch-sd-side = %d\n", MUX_SDSIM_LOG_TAG,
		__func__, host->id, value);
}

static void msdc_sdsim_get_gpio_nums(struct msdc_host *host,
	struct device_node *np)
{
	u32 value    = SDSIM_GPIO_DEFAULT_NUMBER;
	u32 sim_clk  = SDSIM_GPIO_DEFAULT_NUMBER;
	u32 sim_rst  = SDSIM_GPIO_DEFAULT_NUMBER;
	u32 sim_data = SDSIM_GPIO_DEFAULT_NUMBER;

	value = of_get_named_gpio(np, "sd_clk_gpio_nums", 0);
	sdsim_set_current_sd_clk_gpios(value);
	value = of_get_named_gpio(np, "sd_cmd_gpio_nums", 0);
	sdsim_set_current_sd_cmd_gpios(value);
	value = of_get_named_gpio(np, "sd_data0_gpio_nums", 0);
	sdsim_set_current_sd_data0_gpios(value);
	value = of_get_named_gpio(np, "sd_data1_gpio_nums", 0);
	sdsim_set_current_sd_data1_gpios(value);
	value = of_get_named_gpio(np, "sd_data2_gpio_nums", 0);
	sdsim_set_current_sd_data2_gpios(value);
	value = of_get_named_gpio(np, "sd_data3_gpio_nums", 0);
	sdsim_set_current_sd_data3_gpios(value);
	sim_clk = of_get_named_gpio(np, "sim_clk_gpio_nums", 0);
	sim_rst = of_get_named_gpio(np, "sim_rst_gpio_nums", 0);
	sim_data = of_get_named_gpio(np, "sim_data_gpio_nums", 0);
	sdsim_set_current_sim_gpios(sim_clk, sim_rst, sim_data);
}

static enum sdsim_result msdc_sdsim_get_gpio_dt(struct platform_device *pdev,
	struct mmc_host *mmc)
{
	int ret = SDSIM_OK;

	if (pdev == NULL) {
		pr_err("%s %s [msdc%d] argument:pdev is invalid\n",
			MUX_SDSIM_LOG_TAG, __func__);
		ret = SDSIM_PARAM_INVALID;
		return ret;
	}
	if (mmc == NULL) {
		pr_err("%s %s argument:pdev is invalid\n",
			MUX_SDSIM_LOG_TAG, __func__);
		ret = SDSIM_PARAM_INVALID;
		return ret;
	}
	...
	if (of_property_read_u32(np, "mux-sdsim", &value)) {
		pr_err("%s %s [msdc%d] mux-sdsim property not found,"
		       "using value of 0 as default\n", MUX_SDSIM_LOG_TAG,
		       __func__, host->id);
		value = 0;
	}
	host->mux_sdsim = value;
	pr_info("%s %s [msdc%d] dts mux-sdsim = %d\n", MUX_SDSIM_LOG_TAG,
		__func__, host->id, host->mux_sdsim);

	if (host->mux_sdsim == 1) {
		msdc_sdsim_get_switch_gpio_nums(host, np);
		msdc_sdsim_get_switch_sd_value(host, np);
		msdc_sdsim_get_gpio_nums(host, np);
	}
	return ret;
}
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	msdc_sdsim_get_gpio_dt(pdev, mmc);
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	if (host->mux_sdsim == 1) {
		mmc->supply.vsim2 = regulator_get(mmc_dev(mmc), "vsim2");
		if (IS_ERR(mmc->supply.vsim2)) {
			pr_info("err=%ld ,failed to get vsim2\n",
				PTR_ERR(mmc->supply.vsim2));
			goto vsim2_fail;
		}
	}
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
vsim2_fail:
	regulator_put(mmc->supply.vsim2);
#endif
vqmmc_fail:
