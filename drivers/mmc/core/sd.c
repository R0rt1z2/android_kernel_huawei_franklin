	...
	bool reinit)
{
	...
	if (!reinit) {
		...
		if (err) {
			pr_err("%s: send acmd51 to get scr fail, err=%d, "
			       "please check data0\n", mmc_hostname(host), err);
			return err;
		}
	...
	struct mmc_card *oldcard)
{
	...
	if (rocr & SD_ROCR_S18A) {
		...
		if (err > 0)
			...
			pr_err("%s: send cmd6 into high-speed mode fail(not uhs-1),"
			       " err=%d\n", mmc_hostname(host), err);
			goto free_card;
		}
		...
		if ((host->caps & MMC_CAP_4_BIT_DATA) &&
			...
			err = mmc_app_set_bus_width(card, MMC_BUS_WIDTH_4);
			if (err) {
				pr_err("%s: send acmd6 into 4bit mode fail(not uhs-1),"
				       " err=%d\n", mmc_hostname(host), err);
				goto free_card;
			}
...
unsigned int mmc_get_sd_speed(void)
{
	...
	/* Less than 2MB/s or nonsupport*/
	case 0x00:
		speed = 0;
		break;
	/* speed class 2:2MB/s at least */
	case 0x01:
		speed = 2;
		break;
	/* speed class 4:4MB/s at least */
	case 0x02:
		speed = 4;
		break;
	/* speed class 6:6MB/s at least */
	case 0x03:
		speed = 6;
		break;
	/* speed class 10:10MB/s at least */
	case 0x04:
		speed = 10;
		break;
	/* default speed class 2 */
	default:
		speed = 2;
	}
	return speed;
