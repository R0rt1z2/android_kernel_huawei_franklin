			 ...
			 int type)
{
	...
	if (mmc_card_mmc(host->card))
		DSM_EMMC_LOG(host->card, DSM_EMMC_RW_TIMEOUT_ERR,
			"%s:eMMC enter reset, type=%u, partition=%d\n",
			__func__, type, md->part_type);
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
			if (mmc_card_sd(card))
				dsm_sdcard_cmd_logs[DSM_SDCARD_REPORT_UEVENT].value =
					DSM_REPORT_UEVENT_FALSE;
#endif
		}
		mmc_blk_put(md);
	}
