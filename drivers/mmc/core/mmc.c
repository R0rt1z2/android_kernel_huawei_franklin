...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 3) {
		...
		/* for sdcard ,no need bootpartition */
		pr_err("close boot_partition_access for SD card\n");
#endif
		/*
...
static int _mmc_suspend(struct mmc_host *host, bool is_suspend)
{
	...
	/* or if it's nano sd, turn off cache */
	if ((host->caps2 & MMC_CAP2_NMCARD) || (host->card->ext_csd.rev < 7))
