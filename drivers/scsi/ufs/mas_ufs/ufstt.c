...
static int ufstt_load_bitmap(struct ufs_hba *hba)
{
	...
	return ufshcd_query_vcmd_retry(hba, UPIU_QUERY_OPCODE_VENDOR_READ,
				       (int)TURBO_TABLE_READ_BITMAP, 0, 0,
				       (u8 *)bitmap, &size);
}
...
static int __ufstt_ppn_load(uint32_t lba, void *buf, uint32_t len)
{
	...
	return scsi_execute_req(ufstt_sdev, cmd, DMA_FROM_DEVICE, buf, len,
				&sshdr, READ_BUFFER_TIMEOUT * HZ,
...
static bool ufstt_allowed(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (!rq || req_tz(rq) || blk_rq_sectors(rq) != SECTORS_PER_BLK)
		return false;
...
static void ufstt_node_update_fn(struct work_struct *work)
{
	...
	while (!ufshcd_in_shutdown_or_suspend() &&
		...
		if (!node->ppn) {
			...
			if (ret) {
				printk_ratelimited(KERN_ERR "alloc ppn fail\n");
				spin_unlock_irqrestore(&node_lock, flags);
...
static int ufstt_enable(struct ufs_hba *hba)
{
	...
	int buf_len = QUERY_DESC_TTUNIT_MAX_SIZE;
	uint8_t desc_buf[QUERY_DESC_TTUNIT_MAX_SIZE] = { 0 };

	if (ufshcd_query_vcmd_retry(hba, UPIU_QUERY_OPCODE_VENDOR_READ,
				    VENDOR_IDN_TT_UNIT_RD, 0, 0, desc_buf,
				    &buf_len)) {
		return -EIO;
	}
	...
	if (!enabled) {
		...
		desc_buf[TTUNIT_DESC_PARAM_TURBO_TABLE_EN] |=
			BIT(UFSTT_LUN_NUM);
		buf_len = QUERY_DESC_TTUNIT_MAX_SIZE;
		if (ufshcd_query_vcmd_retry(hba, UPIU_QUERY_OPCODE_VENDOR_WRITE,
					    VENDOR_IDN_TT_UNIT_WT, 0, 0,
					    desc_buf, &buf_len)) {
			return -EIO;
		}
		buf_len = QUERY_DESC_TTUNIT_MAX_SIZE;
		if (ufshcd_query_vcmd_retry(hba, UPIU_QUERY_OPCODE_VENDOR_READ,
					    VENDOR_IDN_TT_UNIT_RD, 0, 0,
					    desc_buf, &buf_len)) {
			return -EIO;
		}
		enabled = !!desc_buf[TTUNIT_DESC_PARAM_TURBO_TABLE_EN];
...
{
	if (!ufstt_sdev) {
		pr_err("ufstt sdev is NULL\n");
		goto fail;
	}
	/* can not get correct scsi device for UFSTT_LUN_NUM */
	if (ufstt_sdev->lun != UFSTT_LUN_NUM) {
		pr_err("ufstt sdev lun = %d\n", ufstt_sdev->lun);
		goto fail;
	}
	/* device capability and host capability */
	if (!hba->ufstt_support) {
		pr_err("ufstt not support\n");
		goto fail;
	}

	if (!(hba->caps & UFSHCD_CAP_HPB_TURBO_TABLE))
		pr_err("ufstt no caps skip\n");

	if (!ufstt_dev_rev_validate(hba))
		pr_err("ufstt dev rev not meet skip\n");
