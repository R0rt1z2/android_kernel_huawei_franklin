...
#include "mas_ufs/ufshcd_mas_interface.h"
#ifdef CONFIG_MAS_UFSTT
#include "mas_ufs/ufstt.h"
#endif
...
#include "mas_ufs/mas_ufs.h"
#ifdef CONFIG_MAS_UFS_MANUAL_BKOPS
#include "mas_ufs/mas-ufs-bkops.h"
#endif
#endif
...
static void ufshcd_update_dev_cmd_lun(struct ufs_hba *hba,
				      struct ufshcd_lrb *lrbp);
#endif
static void mas_ufs_parse_dev_desc(struct ufs_hba *hba,
	...
	u32 *upiu_flags, enum dma_data_direction cmd_dir)
{
	...
	mas_ufshcd_prepare_req_desc_hdr(lrbp, upiu_flags);
#endif
			       ...
			       struct ufs_dev_desc *dev_desc)
{
	...
	mas_ufs_parse_dev_desc(hba, dev_desc, desc_buf);
	...
	/* Get serial number */
	err = ufshcd_vops_get_device_info(hba, dev_desc, str_desc_buf);
	if (err) {
				     ...
				     enum ufs_dev_pwr_mode pwr_mode)
{
	 ...
	 * another sync cache
	 */
	if (pwr_mode == UFS_SLEEP_PWR_MODE || pwr_mode == UFS_POWERDOWN_PWR_MODE) {
	 ...
	 */
#ifdef CONFIG_MAS_BLK
	ret = ufshcd_send_scsi_ssu(
		hba, sdp, cmd, (unsigned int)QUUEUE_CMD_TIMEOUT, &sshdr);
#else
...
static void __maybe_unused ufs_set_sec_unique_number(struct ufs_hba *hba,
					uint8_t *str_desc_buf,
					uint8_t *desc_buf,
					char *product_name)
{
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	ufs_inline_crypto_attr(hba);
#ifdef CONFIG_HUAWEI_KERNEL_DEBUG
	ufs_inline_crypto_debug_init(hba);
#endif
	/*
	 * MTK PATCH:
...
void ufs_get_device_health_info(struct ufs_hba *hba)
{
	...
	err = ufshcd_read_device_health_desc(hba, desc_buf,
						QUERY_DESC_HEALTH_MAX_SIZE);
	if (err) {
	...
	/*
	 * 1 Indicates that UFS has used more than 10% of its life.
	 * Report a DMD when life_time_est_typ_a/b more than 10%.
	 */
...
#if defined(CONFIG_MAS_BOOTDEVICE) && defined(CONFIG_HUAWEI_STORAGE_ROFA)
	if (get_bootdevice_type() == BOOT_DEVICE_UFS)
				    ...
				    const u8 *desc_buf)
{
	...
	if (hba->ufs_device_spec_version >= UFS_DEVICE_SPEC_3_1) {
		hba->wb_type = desc_buf[DEVICE_DESC_PARAM_TW_BUF_TYPE];
		hba->wb_shared_alloc_units = get_unaligned_be32(
			desc_buf + DEVICE_DESC_PARAM_NUM_SHARED_WB_BUF_AU);
