...
#include "mas_ufs/ufs_vendor_cmd.h"
#include "mas_ufs/ufshcd_mas_extend.h"
...
static bool ufs_mtk_bypass_clk_op_in_rpm(struct ufs_mtk_host *host)
{
	return (host->caps & UFS_MTK_CAP_BYPASS_CLK_OP_IN_RPM);
}
...
void ufs_mtk_hwfde_cfg_cmd(struct ufs_hba *hba, struct scsi_cmnd *cmd)
{
	...
	if (!(hba->host->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_DRIVER_TAG_ALLOC)))
		lrbp = &hba->lrb[cmd->request->tag];
	else
		lrbp = &hba->lrb[cmd->tag];
	...
	enum ufs_notify_change_status stage)
{
	...
	if (host && ufs_mtk_bypass_clk_op_in_rpm(host) &&
		host->clk_op_bypassed)
		return -EAGAIN;
...
static int ufs_mtk_init(struct ufs_hba *hba)
{
	 ...
	 *
	 * De
	 ce rename shall be prior to any pinctrl operation to avoid
...
out:
	...
	if (!ret && ufs_mtk_bypass_clk_op_in_rpm(host) &&
		ufshcd_is_runtime_pm(pm_op))
		host->clk_op_bypassed = true;
...
void ufs_mtk_parse_dt(struct ufs_hba *hba)
{
	...
	if (np) {
		...
		found = of_property_read_bool(np,
				"mediatek,bypass-clk-op-in-rpm");
		if (found)
			host->caps |= UFS_MTK_CAP_BYPASS_CLK_OP_IN_RPM;
...
int ufs_mtk_ioctl_query(struct ufs_hba *hba, u8 lun, void __user *buf_user)
{
	...
	case UPIU_QUERY_OPCODE_WRITE_ATTR:
		...
		length = min_t(int, sizeof(int), idata->buf_size);
	...
	case UPIU_QUERY_OPCODE_READ_DESC:
	case UPIU_QUERY_OPCODE_READ_HI1861_FSR:
		idata->buf_size = min_t(int, idata->buf_size, length);
		data_ptr = desc;
		break;
	case UPIU_QUERY_OPCODE_READ_ATTR:
		idata->buf_size = sizeof(att);
		data_ptr = &att;
		break;
	case UPIU_QUERY_OPCODE_READ_FLAG:
		idata->buf_size = 1;
	...
	err = copy_to_user(user_buf_ptr, data_ptr, idata->buf_size);
	if (err)
		dev_err(hba->dev, "%s: err %d copying back to user.\n",
				__func__, err);
	goto out_release_mem;
