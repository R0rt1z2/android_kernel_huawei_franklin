...
#define OCP_DAEMON_RESTART_MAX_NUM 100 /* from hardware design */
#define CAM_MODULE_STR "camera"
...
#ifdef IMGSENSOR_OC_ENABLE
static int g_vcamio_oc;
static unsigned int g_camio_ocp_occur_count;
	...
	enum IMGSENSOR_SENSOR_IDX sensor_idx, bool enable)
{
	...
	if (enable && !g_vcamio_oc) {
		mdelay(5); /* wait steady */

		preg = regulator_get(pdevice, "vcamio");
		if (preg) {
			if (regulator_is_enabled(preg)) {
				pmic_enable_interrupt(INT_VCAMIO_OC, 1, CAM_MODULE_STR);
				g_vcamio_oc = 1; /* ocp occur */
				pr_info(
		...
		rcu_read_unlock();
	} else if (!enable && g_vcamio_oc){
		reg_instance.pid = -1; /* init pid */
		/* Disable interrupt before power off */
		pmic_enable_interrupt(INT_VCAMIO_OC, 0, CAM_MODULE_STR);
		g_vcamio_oc = 0;
		pr_info(
			"[regulator] %s INT_VCAMIO_OC %d, idx %d\n",
			__func__, enable, sensor_idx);
	...
	unsigned int regulator_index)
{
	...
	if (regulator_status[sensor_index][regulator_index] == false) {
		int ret = memset_s(str_regulator_name, sizeof(str_regulator_name),
			0, sizeof(str_regulator_name));
		if (ret != EOK)
			pr_err("memset str_regulator_name err\n");
