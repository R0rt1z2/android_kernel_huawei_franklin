/*
 * s5k3p9_sunny_sensor.c
 *
 * Copyright (c) 2018-2019 Huawei Technologies Co., Ltd.
 *
 * s5k3p9_sunny image sensor driver
 *
...
#define S5K3P9_REG_GAIN_1X 32
#define S5K3P9_MAXGAIN 16 // 16X
#define RETRY_TIMES 2
#define VENDORID_OFFSET 4 // vendorID offset in eeprom

/* Modify Following Strings for Debug */
#define PFX "[s5k3p9_sunny]"
#define DEBUG_S5K3P9_SUNNY 0
#define LOG_DBG(fmt, args...) \
	do { \
		if (DEBUG_S5K3P9_SUNNY) \
...
static kal_uint32 return_sensor_id(void)
{
	...
	(void)imgsensor_sensor_i2c_write(&imgsensor,
		0x602C,0X4000,IMGSENSOR_I2C_WORD_DATA);
	(void)imgsensor_sensor_i2c_write(&imgsensor,
		0x602E,0X0000,IMGSENSOR_I2C_WORD_DATA);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	reg_gain = (gain * S5K3P9_REG_GAIN_1X) / BASEGAIN;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN) {
		...
		gain = BASEGAIN;
	} else if (gain > S5K3P9_MAXGAIN * BASEGAIN) {
		LOG_ERR("Invaild gain: %d\n", gain);
		gain = S5K3P9_MAXGAIN * BASEGAIN;
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*/
			if (imgsensor_info.sensor_id == reg_sensor_id) {
				*sensor_id = reg_sensor_id;
				rc = imgsensor_sensor_i2c_read(&imgsensor_otp, VENDORID_OFFSET,
					&reg_vendor_id, IMGSENSOR_I2C_BYTE_DATA);
				if (rc < 0) {
					LOG_ERR("Read id failed.id reg: 0x%x\n", VENDORID_OFFSET);
					reg_vendor_id = 0xFFFF;
				}
				LOG_INF("OTP_ADDR: 0x%x, reg_vendor_id: 0x%x",
					VENDORID_OFFSET, reg_vendor_id);
				LOG_INF("Check sensor id Success");
				LOG_INF("sensor_id reg: 0x%x, otp_id reg: 0x%x, read id == expect id:0x%x\n",
					imgsensor.i2c_write_id,
					imgsensor_otp.i2c_write_id,
...
static kal_uint32 feature_control_s5k3p9_sunny(MSDK_SENSOR_FEATURE_ENUM feature_id,
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_s5k3p9_sunny,
	control,
	close
};

UINT32 S5K3P9_SUNNY_SensorInit(struct SENSOR_FUNCTION_STRUCT **pf_func)
{
	if (pf_func != NULL)
		*pf_func = &sensor_func;
	return ERROR_NONE;
