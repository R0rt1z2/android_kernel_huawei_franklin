...
struct IMGSENSOR_HW_CFG imgsensor_dm_config[] = {
	{
...
struct IMGSENSOR_HW_CFG imgsensor_ok_config[] = {
	{
		IMGSENSOR_SENSOR_IDX_MAIN,
		IMGSENSOR_I2C_DEV_0,
		{
...
struct IMGSENSOR_HW_CFG imgsensor_tianshan_config[] = {
	{
		IMGSENSOR_SENSOR_IDX_MAIN,
		IMGSENSOR_I2C_DEV_0,
		{
...
struct IMGSENSOR_HW_POWER_SEQ sensor_power_sequence[] = {
	...
	{
		SENSOR_DRVNAME_OV48B2Q_LUXVISIONS,
		{
			{ AVDD, Vol_2800, 0 },
			{ AFVDD, Vol_2800, 0 },
			{ DVDD, Vol_1100, 0 },
...
void get_imgsensor_custom_conf(const char *product_name)
{
	if (product_name == NULL) {
		imgsensor_custom_config = &imgsensor_dm_config[0];
		PK_PR_ERR("get product name failed so use default config\n");
		return;
	}
	if (!strcmp(product_name, "Maxw3Cam")) {
		imgsensor_custom_config = &imgsensor_dm_config[0];
		PK_PR_ERR("get_imgsensor_custom_conf get Maxw3Cam\n");
	} else if (!strcmp(product_name, "Kunming")) {
		imgsensor_custom_config = &imgsensor_ok_config[0];
		PK_PR_ERR("get_imgsensor_custom_conf get Kunming\n");
	} else if (!strcmp(product_name, "Tianshan") || !strcmp(product_name, "TianshanSub")) {
		imgsensor_custom_config = &imgsensor_tianshan_config[0];
		PK_PR_ERR("get_imgsensor_custom_conf get Tianshan\n");
	} else {
		imgsensor_custom_config = &imgsensor_dm_config[0];
		PK_PR_ERR("get_imgsensor_custom_conf get default dm\n");
	}
}
