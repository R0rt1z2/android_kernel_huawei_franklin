...
static int rfled_set_regulator_driver(struct flashlight_device *flashlight_dev,
	struct notifier_block *np)
{
	struct rt_fled_dev *fled_dev = flashlight_get_data(flashlight_dev);
	if (fled_dev->hal->rt_hal_fled_reg_set_driver)
		return fled_dev->hal->rt_hal_fled_reg_set_driver(fled_dev, np);
	return 0;
}

static int rfled_set_torch_current(struct flashlight_device *flashlight_dev,
		int min_uA, int max_uA)
{
	int sel = 0;
	struct rt_fled_dev *fled_dev = flashlight_get_data(flashlight_dev);
	pr_info("%s, Enter min_uA=%d, max_uA=%d\n", __func__, min_uA, max_uA);
	if (!fled_dev->hal->rt_hal_fled_set_torch_current)
		return -EINVAL;

	return fled_dev->hal->rt_hal_fled_set_torch_current(fled_dev,
			min_uA, max_uA, &sel);
}

static int rfled_set_strobe_current(struct flashlight_device *flashlight_dev,
		int min_uA, int max_uA)
{
	int sel = 0;
	...
	struct rt_fled_dev *fled_dev = flashlight_get_data(flashlight_dev);
	pr_info("%s, Enter min_uA=%d, max_uA=%d\n", __func__, min_uA, max_uA);
	if (!fled_dev->hal->rt_hal_fled_set_strobe_current)
		return -EINVAL;

	return fled_dev->hal->rt_hal_fled_set_strobe_current(fled_dev,
			min_uA, max_uA, &sel);
}


static int rfled_set_reg_current(struct flashlight_device *flashlight_dev,
		int min_uA, int max_uA)
{
	int sel = 0;
	...
	struct rt_fled_dev *fled_dev = flashlight_get_data(flashlight_dev);
	pr_info("%s, Enter min_uA=%d, max_uA=%d\n", __func__, min_uA, max_uA);
	if (!fled_dev->hal->rt_hal_fled_set_reg_current)
		return -EINVAL;

	return fled_dev->hal->rt_hal_fled_set_reg_current(fled_dev,
			min_uA, max_uA, &sel);
}
...
static struct flashlight_ops rtfled_impl_ops = {
	...
	.set_regulator_driver = rfled_set_regulator_driver,
	.set_torch_current = rfled_set_torch_current,
	.set_strobe_current = rfled_set_strobe_current,
	.set_reg_current = rfled_set_reg_current,
};
...
static int rtled_impl_set_reg_current(struct rt_fled_dev *fled_dev,
					int min_uA, int max_uA, int *selector)
{
	int sel = 0;
	int rc;
	...
	for (sel = 0;; sel++) {
		rc = fled_dev->hal->rt_hal_fled_reg_current_list(fled_dev,
					sel);
		if (rc < 0)
			return rc;
		if (rc >= min_uA && rc <= max_uA) {
			*selector = sel;
			return fled_dev->hal->rt_hal_fled_set_reg_current_sel
								(fled_dev, sel);
		}
	}
	return -EINVAL;
...
static int rtfled_check_hal_implement(struct rt_fled_hal *hal)
{
	...
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_strobe_current))
		hal->rt_hal_fled_set_strobe_current =
						...
						rtled_impl_set_torch_current;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_reg_current))
		hal->rt_hal_fled_set_reg_current =
						rtled_impl_set_reg_current;
	...
	rc |= check_hal_implemented(hal->rt_hal_fled_reg_current_list);
