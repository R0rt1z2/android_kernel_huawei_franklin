...
int flashlight_set_regulator_driver(struct flashlight_device *flashlight_dev,
	struct notifier_block *np)
{
	int rc;

	if ((flashlight_dev->ops ==  NULL) ||
		(flashlight_dev->ops->set_regulator_driver == NULL))
		return -EINVAL;

	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_regulator_driver(flashlight_dev, np);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		pr_err("%s set failed\n", __func__);
...
EXPORT_SYMBOL(flashlight_set_regulator_driver);

int flashlight_set_torch_current(struct flashlight_device *flashlight_dev,
	int min_uA, int max_uA)
{
	int rc;

	if ((flashlight_dev->ops ==  NULL) ||
		(flashlight_dev->ops->set_torch_current == NULL))
		return -EINVAL;

	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_torch_current(flashlight_dev,
		min_uA, max_uA);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		pr_err("%s set failed\n", __func__);
...
EXPORT_SYMBOL(flashlight_set_torch_current);

int flashlight_set_strobe_current(struct flashlight_device *flashlight_dev,
	int min_uA, int max_uA)
{
	int rc;

	if ((flashlight_dev->ops ==  NULL) ||
		(flashlight_dev->ops->set_strobe_current == NULL))
		return -EINVAL;

	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_strobe_current(flashlight_dev,
		min_uA, max_uA);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		pr_err("%s set failed\n", __func__);
...
EXPORT_SYMBOL(flashlight_set_strobe_current);

int flashlight_set_reg_current(struct flashlight_device *flashlight_dev,
	int min_uA, int max_uA)
{
	int rc;
	...
	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_reg_current(flashlight_dev,
		min_uA, max_uA);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		pr_err("%s set failed\n", __func__);
...
EXPORT_SYMBOL(flashlight_set_reg_current);
