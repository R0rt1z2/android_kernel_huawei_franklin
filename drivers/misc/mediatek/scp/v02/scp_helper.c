...
static int params_to_scp(void)
{
	...
	mt_reg_sync_writel(scp_get_reserve_mem_phys(SCP_LOGGER_MEM_ID),
			&(scp_region_info->scp_logbuff_start));
	mt_reg_sync_writel(scp_get_reserve_mem_size(SCP_LOGGER_MEM_ID),
			&(scp_region_info->scp_logbuff_size));
	mt_reg_sync_writel(scp_get_reserve_mem_phys(SENS_MEM_ID),
			&(scp_region_info->shmem_start));
	mt_reg_sync_writel(scp_get_reserve_mem_phys(XHUB_CFG_MEM_ID),
			&(scp_region_info->cfg_reserved_mem_start));
	mt_reg_sync_writel(scp_get_reserve_mem_phys(FLP_MEM_ID),
			&(scp_region_info->flp_reserved_mem_start));
	mt_reg_sync_writel(scp_get_reserve_mem_phys(ALS_MEM_ID),
			&(scp_region_info->als_reserved_mem_start));
#endif
