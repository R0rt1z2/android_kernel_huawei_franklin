...
static size_t scp_A_get_last_log(size_t b_len)
{
	...
	if (last_log_info.scp_log_end_addr > scpreg.total_tcmsize) {
		pr_err("[SCP] %s: scp_log_end_addr %x is over tcm_size %x\n",
		__func__, last_log_info.scp_log_end_addr,
		scpreg.total_tcmsize);
		goto exit;
	}
	if (last_log_info.scp_log_buf_addr + last_log_info.scp_log_buf_maxlen >
		scpreg.total_tcmsize) {
		pr_debug("[SCP] %s: scp_last_log_buf end %x is over tcm_size %x\n",
		__func__, last_log_info.scp_log_buf_addr +
		last_log_info.scp_log_buf_maxlen, scpreg.total_tcmsize);
		goto exit;
	}
...
void scp_crash_log_move_to_buf(enum scp_core_id scp_id)
{
	...
	/*cofirm last log information is less than tcm size*/
	if (last_log_info.scp_log_buf_addr > scpreg.total_tcmsize) {
		pr_err("[SCP] %s: scp_log_buf_addr %x is over tcm_size %x\n",
		__func__, last_log_info.scp_log_buf_addr,
		scpreg.total_tcmsize);
		goto exit;
	}
	if (last_log_info.scp_log_start_addr > scpreg.total_tcmsize) {
		pr_err("[SCP] %s: scp_log_start_addr %x is over tcm_size %x\n",
		__func__, last_log_info.scp_log_start_addr,
		scpreg.total_tcmsize);
		goto exit;
	}
	if (last_log_info.scp_log_end_addr > scpreg.total_tcmsize) {
	...
	if (last_log_info.scp_log_buf_addr + last_log_info.scp_log_buf_maxlen >
		scpreg.total_tcmsize) {
		pr_debug("[SCP] %s: scp_log_buf end %x is over tcm_size %x\n",
		__func__, last_log_info.scp_log_buf_addr +
		last_log_info.scp_log_buf_maxlen, scpreg.total_tcmsize);
		goto exit;
	}
	...
	if (log_start_idx > last_log_info.scp_log_buf_maxlen) {
		pr_debug("[SCP] %s: scp_logger_buf %x is over tcm_size %x\n",
