...
{
	struct cmdq_sec_task *task = NULL, *temp = NULL, *cur_task = NULL;
	struct cmdq_sec *cmdq =
		container_of(thread->chan->mbox, struct cmdq_sec, mbox);
	unsigned long flags;
	s32 done;
...
static s32 cmdq_sec_session_init(struct cmdq_sec_context *context)
{
	...
	case IWC_CONTEXT_INITED:
		...
		if (context->mtee_iwc_msg) {
			cmdq_err("mtee_iwcMessage not NULL:%p",
	...
	struct cmdq_sec_task *task, u32 thrd_idx)
{
	...
	struct cmdq_pkt_buffer *buf = NULL, *last = NULL;
	u32 size = CMDQ_CMD_BUFFER_SIZE, offset = 0, *instr = NULL;
...
{
	s32 err = 0;
	bool mem_ex1 = false, mem_ex2 = false;
	u64 cost;
	struct iwcCmdqMessage_t *iwc_msg = NULL;
...
{
	struct cmdq_sec_thread *thread = NULL;
	s32 idx = sp->args[0];
