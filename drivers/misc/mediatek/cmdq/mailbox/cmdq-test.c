...
static void cmdq_test_mbox_cpr(struct cmdq_test *test)
{
	...
	unsigned long pa = CMDQ_GPR_R32(test->gce.pa,
		CMDQ_GPR_DEBUG_DUMMY);
	u32 pttn = 0xdeaddead, *buf_va = NULL, mark_assign, mark_write;
	s32 i, ret;
	struct cmdq_pkt *pkt = NULL;
	struct cmdq_pkt_buffer *buf = NULL;
	...
	for (i = 0; i < 1312; i++) { // CPR_CNT
		...
		buf_va[mark_assign + 1] =
			(buf_va[1] & 0xffff0000) | (u32)(CMDQ_CPR_STRAT_ID + i);
		buf_va[mark_write] =
	...
	const bool aee)
{
	...
	u64				cpu_time;
	u32				i, val = 0, *out_va = NULL, gce_time;
...
static void cmdq_test_mbox_handshake_event(struct cmdq_test *test)
{
	...
	struct cmdq_pkt *pkt_wait = NULL, *pkt_shake = NULL;
	int ret;
