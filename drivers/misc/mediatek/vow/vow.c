...
int vow_model_table[] = {0, (210 * 1024), 0, (420 * 1024), 0};
			 ...
			 void *msg_data)
{
	...
	case IPIMSG_VOW_COMBINED_INFO: {
		...
		VOWDRV_DEBUG("[vow] IPIMSG_VOW_COMBINED_INFO: ipi_type:%x\n", ipi_ptr->ipi_type_flag);
		bypass_flag = false;
		if (ipi_ptr->ipi_type_flag & RECOG_OK_IDX_MASK) {
			if ((vowserv.recording_flag == true) && (vowserv.tx_keyword_start == true)) {
				VOWDRV_DEBUG("%s(), bypass this recog ok\n", __func__);
				bypass_flag = true;
			}
			if (bypass_flag == false) {
				vowserv.ap_received_ipi_cycle = get_cycles();
				vowserv.scp_recognize_ok_cycle = ipi_ptr->recog_ok_os_timer;
				vowserv.enter_phase3_cnt++;
				if (vowserv.bypass_enter_phase3 == false) {
					vow_ipi_reg_ok((short)ipi_ptr->recog_ok_uuid, ipi_ptr->confidence_lv,
...
static void vow_ipi_reg_ok(short uuid, int confidence_lv, unsigned int extradata_len)
{
	...
	if (extradata_len <= VOW_EXTRA_DATA_SIZE)
		vowserv.extradata_bytelen = extradata_len;    //alg_type
	else
...
static void vow_service_Init(void)
{
	...
	if (init_flag != 1) {
		...
		for (I = 0; I < MAX_VOW_SPEAKER_MODEL; I++) {
			...
			vowserv.vow_speaker_model[I].model_size = 0;
			vowserv.vow_speaker_model[I].alg_type = 0;
			vowserv.vow_speaker_model[I].scene = 0;
			vowserv.vow_speaker_model[I].send_model_size = 0;
		}
...
int vow_service_GetParameter(unsigned long arg)
{
	...
	VOWDRV_DEBUG("vow get parameter: %lu %lu %lu %lu %lu %lu %lu %lu %lu\n",
		     ...
		     vowserv.vow_info_apuser[7],
		     vowserv.vow_info_apuser[8]);

	return 0;
}

/*
vowserv.vow_info_apuser[0]   id
vowserv.vow_info_apuser[1]   addr
vowserv.vow_info_apuser[2]   size
vowserv.vow_info_apuser[3]   return_size_addr
vowserv.vow_info_apuser[4]   uuid
vowserv.vow_info_apuser[5]   alg_type
vowserv.vow_info_apuser[6]   scene
vowserv.vow_info_apuser[7]   send_size
vowserv.vow_info_apuser[8]   data
*/
#ifdef CONFIG_MTK_TINYSYS_SCP_SUPPORT
static int vow_service_CopyModel(int slot)
{
	if (vowserv.vow_info_apuser[7] > VOW_TRANS_MODEL_SIZE) {
	...
	vowserv.vow_speaker_model[slot].id = vowserv.vow_info_apuser[0];    //handle
	vowserv.vow_speaker_model[slot].uuid = vowserv.vow_info_apuser[4];  //uuid - "HUAWEI"
	vowserv.vow_speaker_model[slot].model_size = vowserv.vow_info_apuser[2];
	vowserv.vow_speaker_model[slot].alg_type = vowserv.vow_info_apuser[5];
	vowserv.vow_speaker_model[slot].scene = vowserv.vow_info_apuser[6];
	vowserv.vow_speaker_model[slot].send_model_size = vowserv.vow_info_apuser[7];
...
static bool vow_service_SendSpeakerModel(int slot, bool release_flag)
{
	...
	if (release_flag == VOW_CLEAN_MODEL) {
		...
		if ((vowserv.vow_speaker_model[slot].flag == 0) ||
		    ...
		    (vowserv.vow_speaker_model[slot].send_model_size == 0)) {
			VOWDRV_DEBUG("%s(), slot:%d, no model need to set\n", __func__, slot);
			return ret;
		}
		vow_ipi_buf[0] = VOW_MODEL_SPEAKER;
	}
	scene = (int)vowserv.vow_speaker_model[slot].scene;
	vow_ipi_buf[1] = vowserv.vow_speaker_model[slot].uuid;
	vow_ipi_buf[2] = scp_get_reserve_mem_phys(VOW_MEM_ID) + vow_model_table[slot];
	vow_ipi_buf[3] = vowserv.vow_speaker_model[slot].model_size;
	vow_ipi_buf[4] = (unsigned int)vowserv.vow_speaker_model[slot].id;
	vow_ipi_buf[5] = (unsigned int)vowserv.vow_speaker_model[slot].alg_type;
	vow_ipi_buf[6] = (unsigned int)vowserv.vow_speaker_model[slot].scene;
	vow_ipi_buf[7] = (unsigned int)vowserv.vow_speaker_model[slot].send_model_size;

	VOWDRV_DEBUG("vow Model:slot_%d, model_%x, addr_%x, uuid_%x, size_%x, id_%x algo_type_%d scene:_%u send_model_size_%d\n",
		      slot,
		      vow_ipi_buf[0],
		      vow_ipi_buf[2],
		      vow_ipi_buf[1],
...
static bool vow_service_ReleaseSpeakerModel(int id)
{
	...
	vowserv.vow_speaker_model[I].flag = 0;
	vowserv.vow_speaker_model[I].enabled = 0;
	vowserv.vow_speaker_model[I].alg_type = -1;
...
#ifdef CONFIG_MTK_TINYSYS_SCP_SUPPORT
	scene = (int)vowserv.vow_info_apuser[6];
	VOWDRV_DEBUG("SetSPKModel: current scene: %d\n", scene);
	vowserv.vow_speaker_model[scene].model_ptr = (void *)(scp_get_reserve_mem_virt(VOW_MEM_ID)) + vow_model_table[scene];

	if (vow_service_CopyModel(scene) != 0)
		return false;

	ptr8 = (int *)vowserv.vow_speaker_model[scene].model_ptr;
	VOWDRV_DEBUG("SetSPKModel:slot: %d, ID: %x, UUID: %x, flag: %x, algo_type: %d\n",
		      scene,
		      vowserv.vow_speaker_model[scene].id,
		      vowserv.vow_speaker_model[scene].uuid,
		      vowserv.vow_speaker_model[scene].flag,
		      vowserv.vow_speaker_model[scene].alg_type);
	VOWDRV_DEBUG("SetSPKModel: model[0]: %d, model[1]\n", ptr8[0], ptr8[1]);

	ret = vow_service_SendSpeakerModel(scene, VOW_SET_MODEL);
#else
...
static bool vow_service_SendModelStatus(int slot, bool enable)
{
	...
	vow_ipi_buf[2] = (unsigned int)vowserv.vow_speaker_model[slot].alg_type;
	vow_ipi_buf[3] = vowserv.vow_speaker_model[slot].id;
...
static bool vow_service_SetModelStatus(bool enable, unsigned long arg)
{
	...
	if (enable == VOW_MODEL_STATUS_START) {
		...
		VOWDRV_DEBUG("VOW_MODEL_START, id:%d, enabled:%d, conf_lv:%d, module alg_type:%d, start algo_type:%d\n",
			     (int)model_start.handle,
			     vowserv.vow_speaker_model[slot].enabled,
			     vowserv.vow_speaker_model[slot].confidence_lv,
			     vowserv.vow_speaker_model[slot].alg_type,
			     model_start.alg_type);
...
static bool vow_setparam_to_scp(unsigned long arg)
{
	bool ret = false;
	vow_setparam_t vow_setparam;
	unsigned int vow_ipi_buf[4];

	VOWDRV_DEBUG("vow_setparam_to_scp ++ : %lu\n", arg);

	if (copy_from_user((void *)(&vow_setparam),
			(const void __user *)(arg),
			sizeof(vow_setparam_t))) {
		VOWDRV_DEBUG("vow get parma_value from user fail\n");
		return false;
	}

	vow_ipi_buf[0] = vow_setparam.alg_type;
	vow_ipi_buf[1] = vow_setparam.handle;
	vow_ipi_buf[2] = vow_setparam.param_key;
	vow_ipi_buf[3] = vow_setparam.param_value;

	ret = vow_ipi_send(IPIMSG_VOW_SET_PARAM, 4, &vow_ipi_buf[0], VOW_IPI_BYPASS_ACK);
	if (ret == false){
		VOWDRV_DEBUG("vow_ipi_send param fail\n");
	}

	VOWDRV_DEBUG("vow_setparam_to_scp --\n");
	return ret;
}
...
static long VowDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case VOW_SET_CONTROL:
		...
		case VOWControlCmd_ReadVoiceData:
			VOWDRV_DEBUG("VOW_SET_CONTROL ReadVoiceData");
				...
				unsigned long arg)
{
	...
	case VOW_MODEL_STOP: {
		...
		err |= get_user(l, &data32->alg_type);
		err |= put_user(l, &data->alg_type);
	...
	case VOW_SET_APREG_INFO: {
		...
		err |= get_user(l, &data32->alg_type);
		err |= put_user(l, &data->alg_type);
		err |= get_user(l, &data32->scene);
		err |= put_user(l, &data->scene);
		err |= get_user(l, &data32->send_size);
		err |= put_user(l, &data->send_size);
		...
		break;
	case VOW_SET_PARAM: {
		vow_setparam_kernel_t __user *data32;
		vow_setparam_t __user *data;
		int err;
		compat_size_t l;
		...
		err |= get_user(l, &data32->handle);
		err |= put_user(l, &data->handle);
		err |= get_user(l, &data32->param_key);
		err |= put_user(l, &data->param_key);
		err |= get_user(l, &data32->param_value);
		err |= put_user(l, &data->param_value);
		ret = fp->f_op->unlocked_ioctl(fp, cmd, (unsigned long)data);
	}
		break;
	default:
			   ...
			   loff_t *offset)
{
	...
	vowserv.vow_eint_data_struct.id = vowserv.scp_command_id;    //handle
	...
	vowserv.vow_eint_data_struct.data[1] = (char)vowserv.extradata_bytelen;   //alg_type
