	...
	int brightness)
{
	...
	lcd_kit_bl_ic_set_backlight(brightness);
...
int mt_leds_brightness_set(char *name, int level)
{
	...
	if (mtk_leds_get_satus()) {
		if (disp_info->quickly_sleep_out.support && led_Level > 0 &&
			disp_info->quickly_sleep_out.panel_on_tag)
			lcd_kit_disp_on_check_delay();

		if (led_dat->conf.level == led_Level)
			return 0;
	...
	} else if (panel_info->bl_ic_ctrl_mode == MTK_MIPI_MODE) {
		mtkfb_set_backlight_level(led_Level);
	} else {
	...
	led_dat->last_level = led_Level;
#endif
	send_sensor_scp_brightness(led_Level);
	return 0;
}
		...
		struct mtk_leds_info *m_leds)
{
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		if (mtk_leds_get_satus() == 0) {
			pr_info("disp led is not support\n");
			ret = -EINVAL;
			goto out_led_dt;
		}
#endif
