...
struct command_entry {
	...
	uint64_t cmd_id;
	uint32_t multicore_total; // how many cores to exec this cmd

	__u64 deadline_t;

	u32 fin_cid;         /* record the last finished command id */
	u32 wish_fin_cid;
	u32 cmd_tmp_size;  /* command tmp size */
	u16 priority;       /* cmd priority */
	struct list_head *cmd_list_head;/* list of cmd tmp */
