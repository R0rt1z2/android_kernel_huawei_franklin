	...
	u16 priority)
{
	...
	if (ce->priority == MDLA_LOW_PRIORITY && ce->cmd_list_head != NULL)
		mdla_split_command(ce);
	...
	u16 priority)
{
	...
	if (likely(apusys_hd != NULL)) {
		...
		if (apusys_hd->multicore_total == 2) {
			...
			ce->cmd_tmp_size = apusys_hd->cluster_size;
			ce->deadline_t = deadline;
		}
	} else {
		ce->ctx_id = INT_MAX;
	...
	cb_size = ce->cmd_tmp_size;
	if (priority == MDLA_LOW_PRIORITY && cb_size < ce->count) {
		ce->cmd_list_head =
			kzalloc(sizeof(struct list_head), GFP_KERNEL);
		if (unlikely(!ce->cmd_list_head))
			return;
		INIT_LIST_HEAD(ce->cmd_list_head);
	} else
		ce->cmd_list_head = NULL;
	return;
