...
#if defined(USE_CPU_TO_DRAM_MAP) || defined(CONFIG_HUAWEI_FREQ_LINK)
...
static int cpu_ddr_freq_link_notify(struct notifier_block *nb,
				    unsigned long val, void *v)
{
	if (val == LINK_LEVEL_1)
		cm_mgr_cpu_map_dram_enable = 1;
	else
		cm_mgr_cpu_map_dram_enable = 0;

	return NOTIFY_OK;
}

static struct notifier_block cpu_ddr_freq_link_notifier = {
	.notifier_call = cpu_ddr_freq_link_notify,
};

static void cpu_ddr_freq_link_init(void)
{
	int ret;

	ret = pm_qos_add_notifier(PM_QOS_FREQ_LINK_LEVEL, &cpu_ddr_freq_link_notifier);
	if (ret != 0)
		pr_err("cpu_ddr_freq_link init fail%d\n", ret);
}
#endif
