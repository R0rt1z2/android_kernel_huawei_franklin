...
/* this function should be called with ppm_main_info.lock locked */
bool mt_ppm_dvfs_limit_callback_locked(void)
{
	bool dvfs_limit_enabled = false;

	/* only when ppm's dvfs limit has changed, callback will be called */
	ppm_main_info.is_dvfs_limit_changed = true;
	wake_up_poll(&ppm_main_info.dvfs_limit_wqh, POLLIN);

	if (ppm_main_info.is_dvfs_limit_enabled)
		dvfs_limit_enabled = true;

	return dvfs_limit_enabled;
}
#endif
