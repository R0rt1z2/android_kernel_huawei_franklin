...
int Ripi_cpu_dvfs_thread(void *data)
{
	...
	(int)(log_box_parsed[num_log - 1].cluster_opp_cfg[i].base_idx);
				...
				if (p->idx_opp_ppm_limit != previous_limit ||
					...
					trace_cpufreq_hw_min_max_ipi(p->mt_policy->cpu,
							p->mt_policy->cur,
							cpu_dvfs_get_freq_by_idx(p, p->idx_opp_tbl),
							cpu_dvfs_get_freq_by_idx(p, previous_base),
							cpu_dvfs_get_freq_by_idx(p, p->idx_opp_ppm_base),
							cpu_dvfs_get_freq_by_idx(p, previous_limit),
...
#endif
#ifdef CONFIG_HW_CPU_FREQ
	trace_cpufreq_hw_set_min_max(current, cluster_id,
			cpu_dvfs_get_freq_by_idx(id_to_cpu_dvfs(cluster_id), base),
			cpu_dvfs_get_freq_by_idx(id_to_cpu_dvfs(cluster_id), limit));
#endif
	return 0;
}
