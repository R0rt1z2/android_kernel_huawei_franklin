...
static void update_ddr_ip_bc(void)
{
	int i, j;
	struct mem_ip_bc *ddr_ip_bc_ptr = NULL;
					   ...
					   struct ddr_ip_bc_stats *stats)
{
	...
	if (stats && data_ip_num == NR_DDR_BC_IP &&
		...
		spin_lock_irqsave(&swpm_sp_spinlock, flags);
		for (i = 0; i < NR_DDR_BC_IP; i++) {
			strncpy(stats[i].ip_name,
				ddr_ip_stats[i].ip_name, MAX_IP_NAME_LENGTH - 1);
			if (!stats[i].bc_stats)
				continue;
			memcpy(stats[i].bc_stats, ddr_ip_stats[i].bc_stats,
				       ...
				       struct ip_stats *stats)
{
	...
	if (stats && ip_num == NR_CORE_IP &&
		...
		for (i = 0; i < NR_CORE_IP; i++) {
			strncpy(stats[i].ip_name,
				core_ip_stats[i].ip_name, MAX_IP_NAME_LENGTH - 1);
			if (!stats[i].vol_times)
				continue;
			memcpy(stats[i].vol_times, core_ip_stats[i].vol_times,
...
#ifdef CONFIG_HUAWEI_DDR_PERF_CTRL
/* for huawei ddr perf ctrl */
static struct ddr_flux g_last;

void plat_get_ddr_flux(struct ddr_flux *data)
{
	int j;
	struct ddr_flux cur = {0};
	int ddr_freq_num = swpm_plat_nums(DDR_FREQ);
	...
	for (j = 0; j < ddr_freq_num; j++) {
		cur.rd_flux += ddr_ip_stats[DDR_BC_TOTAL_R].bc_stats[j].value;
		cur.wr_flux += ddr_ip_stats[DDR_BC_TOTAL_W].bc_stats[j].value;
	}

	if (cur.rd_flux >= g_last.rd_flux)
		data->rd_flux = cur.rd_flux - g_last.rd_flux;
	else
		data->rd_flux = ULLONG_MAX - g_last.rd_flux + cur.rd_flux;
	data->rd_flux >>= 4;

	if (cur.wr_flux >= g_last.wr_flux)
		data->wr_flux = cur.wr_flux - g_last.wr_flux;
	else
		data->wr_flux = ULLONG_MAX - g_last.wr_flux + cur.wr_flux;
	data->wr_flux >>= 4;

	memcpy(&g_last, &cur, sizeof(struct ddr_flux));
}
#endif
...
static int swpm_sp_test_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < core_ip_num; i++)
		core_ip_stats_ptr[i].vol_times = kmalloc_array(core_vol_num,
