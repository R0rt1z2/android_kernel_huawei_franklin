...
void hw_gpufreq_thermal_protect(unsigned int limit_idx)
{
        mutex_lock(&mt_gpufreq_power_lock);

        mt_gpufreq_update_limit_idx(KIR_THERMAL, limit_idx, LIMIT_IDX_DEFAULT);

        if (g_cur_opp_freq > g_opp_table[limit_idx].gpufreq_khz)
                mt_gpufreq_target(limit_idx, KIR_THERMAL);
