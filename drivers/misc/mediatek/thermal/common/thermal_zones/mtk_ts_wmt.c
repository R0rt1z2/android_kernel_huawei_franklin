...
#ifdef CONFIG_HW_WIFI_TEMP
static const int DEVICE_SIZE = 5;
static const int MIN_THROTTLE = 1024;
struct net_device *net_dev = NULL;
extern void cfg80211_drv_temp_result(struct net_device *dev, gfp_t gfp, const u8 *buf, size_t len);
typedef struct {
    int limit_tput;
    int temp;
} tput_stru;
...
static int wmt_send_signal(int level)
{
	...
	// temp upload
	if (net_dev != NULL) {
		memset_s(&pst_tput_stru, sizeof(tput_stru), 0, sizeof(tput_stru));
		pst_tput_stru.limit_tput = level;
		pst_tput_stru.temp = g_curr_temp;
		cfg80211_drv_temp_result(net_dev, GFP_KERNEL, (char *)&pst_tput_stru, sizeof(pst_tput_stru));
	}
#endif
...
#ifdef CONFIG_HW_WIFI_TEMP
				if (!strncmp(dev->name, "wlan0", DEVICE_SIZE)) {
					net_dev = dev;
				}
#endif
			}
