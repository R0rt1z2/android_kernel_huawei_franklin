...
static ssize_t set_gpu_error_info(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct kbase_device *kbdev;
...
int kbase_wa_execute(struct kbase_device *kbdev, u64 cores)
{
	...
	if (failed) {
		...
		dev_err(kbdev->dev, "Dummy Clear Success\n");
		print_out++;
	}
...
#define CORE_MASK_LEVEL_2	9

#define CORE_MASK_LEVEL_1_VALUE	0x33
#define CORE_MASK_LEVEL_2_VALUE	0x1FF

void hw_gpu_thermal_cores_control(u64 cores)
{
	u64 updated_core_mask = CORE_MASK_LEVEL_2_VALUE;
	unsigned long flags;
...
EXPORT_SYMBOL(hw_gpu_thermal_cores_control);
#endif
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	kbdev->runmode_normal = 0; // default factory mode
#endif

#ifdef CONFIG_MTK_IOMMU_V2
	kbdev->init_client_thread =
			kthread_run(mtk_drm_ion_create_mali_client,
			kbdev, "init_mali_kbase_ion");
