...
void kbase_add_queue(struct kbase_jd_atom *katom)
{
	struct atom_info *atom_ptr = NULL;
	bool creat_new = true;

	if (katom == NULL || katom->frame_nr == 0 || katom->kctx->kbdev->dev_data.game_pid != katom->kctx->tgid) {
		return;
	}
	list_for_each_entry(atom_ptr, &katom->kctx->kbdev->dev_data.atom_queue_head, atom_queue) {
		if (atom_ptr->frame_nr == katom->frame_nr) {
			atom_ptr->num++;
			creat_new = false;
			break;
		}
	...
	if (creat_new) {
		atom_ptr = vzalloc(sizeof(struct atom_info));
		atom_ptr->frame_nr = katom->frame_nr;
		atom_ptr->num++;
		INIT_LIST_HEAD(&atom_ptr->atom_queue);
		list_add_tail(&atom_ptr->atom_queue, &katom->kctx->kbdev->dev_data.atom_queue_head);
	}
}

void kbase_delete_queue(struct kbase_jd_atom *katom)
{
	struct atom_info *atom_ptr = NULL;
	struct atom_info *n = NULL;

	if (katom == NULL || katom->frame_nr == 0 || katom->kctx->kbdev->dev_data.game_pid != katom->kctx->tgid) {
		return;
	}
	list_for_each_entry_safe(atom_ptr, n, &katom->kctx->kbdev->dev_data.atom_queue_head, atom_queue) {
		if (atom_ptr->frame_nr == katom->frame_nr) {
			atom_ptr->num--;
			if (atom_ptr->num == 0) {
				list_del(&atom_ptr->atom_queue);
				vfree(atom_ptr);
				katom->kctx->kbdev->dev_data.signaled_seqno++;
			}
			break;
		}
	}
	...
	case BASE_JD_REQ_SOFT_FENCE_TRIGGER:
		{
			...
			mutex_lock(&katom->kctx->kbdev->dev_data.atom_queue_mutex);
			kbase_add_queue(katom);
			mutex_unlock(&katom->kctx->kbdev->dev_data.atom_queue_mutex);
#endif
