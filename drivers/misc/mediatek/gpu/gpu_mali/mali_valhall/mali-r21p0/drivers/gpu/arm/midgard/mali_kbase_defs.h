...
#include <mali_kbase_config_mt6885.h>
#endif
...
#ifdef CONFIG_MALI_HUAWEI_CL_BOOST
#include "mali_kbase_huawei_callback.h"
#endif /* CONFIG_MALI_HUAWEI_CL_BOOST */
...
struct atom_info {
	u32 frame_nr;
	u64 num;
	struct list_head atom_queue;
};

struct kbase_device_data {
	/* GPU AI frequency schedule specific pid to collect fence info */
	pid_t game_pid;
	/* GPU AI frequency schedule fence trigger out counter */
	u64 signaled_seqno;
	struct list_head atom_queue_head;
	struct mutex atom_queue_mutex;
	struct kbase_huawei_callbacks *callbacks;
};
...
struct kbase_device {
...
#ifdef CONFIG_GPU_GMC_GENERIC
	struct kbase_mtk_device_data mtk_dev_data;
#endif
	struct {
	...
	struct kbase_device_data dev_data;
};
...
#ifdef CONFIG_GPU_GMC_GENERIC
	struct kbase_mtk_ctx_data mtk_ctx_data;
#endif
	struct kbase_mem_pool_group mem_pools;
