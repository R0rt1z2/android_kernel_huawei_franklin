 ...
 * Description: This file describe MALI GPU memory compress feature.
	...
	struct kbase_device *kbdev, int group_id)
{
	...
	err = gmc_storage_get_page(
		kbdev->mtk_dev_data.kbase_gmc_device.storage, page, handle);
	if (err) {
				...
				struct kbase_device *kbdev)
{
	...
	handle = gmc_storage_put_page(
		kbdev->mtk_dev_data.kbase_gmc_device.storage, page);
	if (IS_ERR(handle)) {
		dma_addr_t dma_addr;
 ...
 * Return: nothing
 */
extern void gmc_storage_invalidate_page(struct gmc_storage *storage, struct gmc_storage_handle *handle);
	...
	for (i = 0; i < pages_num; i++) {
		...
		if (as_phys_addr_t(*p) && kbase_is_entry_compressed(*p)) {
			...
			gmc_storage_invalidate_page(
				kctx->kbdev->mtk_dev_data.kbase_gmc_device.storage,
				kbase_get_gmc_handle(p));
			p->tagged_addr = 0;
			kbase_clear_entry_compressed(p);
					...
					size_t nr)
{
	...
	if (kctx->mtk_ctx_data.set_pt_flag) {
	...
	struct kbase_gmc_arg arg, struct rb_root *va_rb_root, int *nr_pages_to_compressed)
{
	...
	if (kbdev->mtk_dev_data.gmc_workqueue != NULL)
		local_workqueue = kbdev->mtk_dev_data.gmc_workqueue;
	else
	...
	struct kbase_gmc_arg arg)
{
	...
	if (arg.op == GMC_DECOMPRESS)
		kctx->mtk_ctx_data.set_pt_flag = true;
	...
	if ((arg.op == GMC_COMPRESS) && (kbdev->mtk_dev_data.gmc_cancel != 0))
		kbdev->mtk_dev_data.gmc_cancel = 0;
	...
	if (arg.op == GMC_DECOMPRESS)
		kctx->mtk_ctx_data.set_pt_flag = false;
...
int kbase_gmc_compress(pid_t pid, struct gmc_device *gmc_dev,
			long compress_size)
{
	struct kbase_mtk_device_data *mtk_dev = container_of(gmc_dev,
		struct kbase_mtk_device_data,kbase_gmc_device);
	struct kbase_device *kbdev = container_of(mtk_dev, struct kbase_device,
		mtk_dev_data);
	struct kbase_gmc_arg arg;
...
int kbase_gmc_cancel(long cancel, struct gmc_device *gmc_dev)
{
	...
	struct kbase_device *kbdev = container_of(mtk_dev, struct kbase_device,
		mtk_dev_data);
	if (cancel == 1)
		kbdev->mtk_dev_data.gmc_cancel = 1;
	else
		kbdev->mtk_dev_data.gmc_cancel = 0;
	return 0;
}
