...
#ifdef CONFIG_HUAWEI_DETECT_SR_DEBUG
static ssize_t pmic_smart_reset_write(struct file *file,
				    const char __user *buf,
				    size_t size,
				    loff_t *ppos)
{
	...
	if (val == '0') {
		pr_err("disable mrdump\n");
		pmic_enable_smart_reset(0, 0);
	} else {
		pr_err("invalid value, can't disable mrdump\n");
	}
	return size;
}

static int pmic_smart_reset_show(struct seq_file *s, void *unused)
{
	seq_puts(s, "set 0 to disable mrdump\n");
	return 0;
}

static int pmic_smart_reset_open(struct inode *inode, struct file *file)
{
	return single_open(file, pmic_smart_reset_show, NULL);
}

static const struct file_operations pmic_smart_reset_operations = {
	.open    = pmic_smart_reset_open,
	.read    = seq_read,
	.write   = pmic_smart_reset_write,
	.llseek  = seq_lseek,
	.release = single_release,
};
#endif
...
#ifdef CONFIG_HUAWEI_DETECT_SR_DEBUG
	debugfs_create_file("pmic_smart_reset", (S_IFREG | 0644),
				mtk_pmic_dir, NULL,
				&pmic_smart_reset_operations);
#endif
