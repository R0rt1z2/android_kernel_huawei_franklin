...
#ifdef CONFIG_HUAWEI_DETECT_SR
static int is_ap_press_timeout;

static void ap_press6s_callback(unsigned long data)
{
	pr_err("Press pwrkey 6s happened\n");
	pmic_set_register_value(PMIC_TOP_STATUS, data);
	is_ap_press_timeout = 1;
#ifdef CONFIG_BOOT_DETECTOR
...
void pmic_set_sr_status(unsigned long pressed)
{
	if (pressed == 1) {
		mod_timer(&pwrkey_timer, jiffies + AP_PRESS_TIMEOUT * HZ);
	} else if (pressed == 0) {
		del_timer_sync(&pwrkey_timer);
		if (is_ap_press_timeout) {
			pmic_set_register_value(PMIC_TOP_STATUS, 0);
			is_ap_press_timeout = 0;
		}
	}
}
#endif
