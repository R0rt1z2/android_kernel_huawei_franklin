...
static struct flash_error_info_t g_flash_error_info[] = {
	{
		.reg_addr = MT6360_PMU_FLED_IRQ1,
		.error_table = {
			{
				(FLED1_SHORT | FLED2_SHORT),
				REG_VAL_AND_MASK,
...
static irqreturn_t mt6360_pmu_fled1_strb_to_evt_handler(int irq, void *data)
{
	...
	dev_err(mpfi->dev, "%s fled2 strb timeout\n", __func__);
	return IRQ_HANDLED;
}
				     ...
				     struct mt6360_fled_platform_data *pdata)
{
	...
	if (of_property_read_u32(np, "fled-reg-channel", &pdata->fled_reg_channel)) {
		dev_info(dev, "Not support reg\n");
		pdata->fled_reg_channel = -1;
	}
	dev_info(dev, "fled_reg_channel:%d\n", pdata->fled_reg_channel);
	dev_info(dev, "%s --\n", __func__);
	return 0;
}
...
static enum flashlight_mode mt6360_fled_set_regulator_mode(
	struct mt6360_pmu_fled_info *fi,
	enum flashlight_mode mode)
{
	enum flashlight_mode temp_mode;

	if (fi->regulator_channel == -1)
		return mode;

	pr_info("%s, %d: mode = %d Enter\n", __func__, __LINE__, mode);

	switch (mode) {
	case FLASHLIGHT_MODE_DUAL_FLASH:
		temp_mode = FLASHLIGHT_MODE_FLASH;
		break;
	case FLASHLIGHT_MODE_DUAL_TORCH:
		temp_mode = FLASHLIGHT_MODE_TORCH;
		break;
	case FLASHLIGHT_MODE_DUAL_OFF:
		temp_mode = FLASHLIGHT_MODE_OFF;
		break;
	default: /* donot change the mode */
		temp_mode = mode;
		break;
	}

	if (fi->regulator_channel != fi->id)
		goto exit;
	/*
	 * if current channel is regulator,
	 * check if flash module has set TORCH mode
	 */
	if (temp_mode == FLASHLIGHT_MODE_MIXED &&
		mt6360_global_mode == FLASHLIGHT_MODE_TORCH) {
			pr_info("%s, work on torch\n", __func__);
			temp_mode = FLASHLIGHT_MODE_TORCH;
	}

exit:
	pr_info("%s, %d: temp_mode = %d Exit\n", __func__, __LINE__, temp_mode);
	return temp_mode;
}

static int mt6360_pmu_set_mixed_mode(struct mt6360_pmu_fled_info *fi, u8 en_bit)
{
	u8 val;
	int ret;

	val = en_bit | MT6360_FL_TORCH_MASK | MT6360_FL_STROBE_MASK;

	ret = mt6360_pmu_reg_set_bits(fi->mpi, MT6360_PMU_FLED_EN, val);
	mdelay(5);
	mt6360_global_mode = FLASHLIGHT_MODE_MIXED;
	return ret;
}

static void *mt6360_fled_reg_get_platdata(struct mt6360_pmu_fled_info *fi)
{
	if (!fi->dev) {
		pr_err("no dev\n");
		return NULL;
	...
	if (!mt6360_fled_inited) {
		dev_err(fi->dev, "%s not inited\n", __func__);
		return NULL;
	}

	if (!fi->dev->parent) {
		dev_err(fi->dev, "%s no parent\n", __func__);
		return NULL;
	}

	return dev_get_platdata(fi->dev->parent);;
}
static void mt6360_fled_reg_callback(struct mt6360_pmu_fled_info *fi,
	enum flashlight_mode mode)
{
	int ret;
	struct mt6360_fled_platform_data *pdata = NULL;
	struct notifier_block *nb = NULL;

	if (!fi) {
		pr_err("Get fi failed\n");
		return;
	}

	if (fi->id == fi->regulator_channel) {
		pr_err("no need update status\n");
		return;
	}

	pdata = mt6360_fled_reg_get_platdata(fi);
	if (!pdata) {
		pr_err("Get platdata failed\n");
		return;
	}
	...
	ret = nb->notifier_call(nb, mode, &fi->id);
	if (ret < 0)
				...
				enum flashlight_mode mode)
{
	...
	mode = mt6360_fled_set_regulator_mode(fi, mode);
	switch (mode) {
	...
	case FLASHLIGHT_MODE_DUAL_OFF:
		...
		break;
	case FLASHLIGHT_MODE_MIXED:
		ret = mt6360_pmu_set_mixed_mode(fi, en_bit);
		break;
	default:
...
out:
	...
	flash_clear_error_and_unlock(fi->mpi->i2c, g_flash_error_info,
		array_size(g_flash_error_info), &g_name_info);
	mt6360_fled_reg_callback(fi, mt6360_global_mode);
...
#define FLED_REG_MAX_VAL 12
#define FLED_REG_BASE_CURRENT 50000
#define FLED_REG_CURRENT_STEP 12500
static int mt6360_fled_reg_current_list(struct rt_fled_dev *fled,
					   int selector)
{
	if (selector < 0 || selector > FLED_REG_MAX_VAL)
		return -EINVAL;

	return FLED_REG_BASE_CURRENT + selector * FLED_REG_CURRENT_STEP;
}
...
#define MAX_TIMEOUT_LEVEL 0x7  /* 200mA if ISTRB is 0, or 100mA */
static int mt6360_fled_set_reg_current_sel(struct rt_fled_dev *fled,
					      int selector)
{
	struct mt6360_pmu_fled_info *fi = to_fled_info(fled);
	...
	pr_info("%s %d, sel:%d, fi->id:%d\n",
		__func__, __LINE__, selector, fi->id);

	if (selector > FLED_REG_MAX_VAL || selector < 0)
		return -EINVAL;

	(void)mt6360_fled_set_timeout_level_sel(fled, MAX_TIMEOUT_LEVEL);

	(void)mt6360_pmu_reg_clr_bits(fi->mpi, reg_strb_cur,
					MT6360_UTRAL_ISTRB_MASK);
...
static int mt6360_fled_reg_set_driver(struct rt_fled_dev *fled,
	struct notifier_block *nb)
{
	...
	pdata = mt6360_fled_reg_get_platdata(fi);
	if (!pdata) {
		pr_err("get pdata failed\n");
		return -1;
	}
	/* Register the notifier to fled platform dev */
	pdata->nb = nb;

	pr_info("%s, %d regulator_channel=%d\n", fi->regulator_channel);
	return 0;
}

static int mt6360_fled_get_reg_channel(struct platform_device *pdev)
{
	struct mt6360_fled_platform_data *pdata = NULL;
...
static struct rt_fled_hal mt6360_fled_hal = {
	...
	.rt_hal_fled_reg_set_driver = mt6360_fled_reg_set_driver,
	.rt_hal_fled_reg_current_list = mt6360_fled_reg_current_list,
	.rt_hal_fled_set_reg_current_sel = mt6360_fled_set_reg_current_sel,
};
...
static int mt6360_pmu_fled_probe(struct platform_device *pdev)
{
	...
	if (!mt6360_fled_inited) {
		...
		mpfi->id = pdev->id;
		mpfi->regulator_channel = mt6360_fled_get_reg_channel(pdev);
