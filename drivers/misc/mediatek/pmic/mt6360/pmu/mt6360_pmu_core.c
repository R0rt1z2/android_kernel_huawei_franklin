...
struct mt6360_pmu_core_info *g_core_mpci;
...
struct mt6360_pmu_core_info *get_core_mpci(void)
{
	return g_core_mpci;
}
...
static int mt6360_pmu_core_probe(struct platform_device *pdev)
{
	...
	ret = mt6360_pmu_reg_read(mpci->mpi, MT6360_PMU_CORE_CTRL1);
	dev_info(&pdev->dev, "%s: MT6360_PMU_CORE_CTRL1: 0x%02X\n", __func__, ret);
