...
unsigned long get_last_kmsg_pstore(char *dst, unsigned long length)
{
	...
	pr.psi = psinfo;
	mutex_lock(&(pr.psi)->read_mutex);
	if (pr.psi->open && pr.psi->open(pr.psi))
		goto out;
	size = pr.psi->read(&pr);
	while (size > 0) {
		if (pr.type == PSTORE_TYPE_CONSOLE) {
			if (size > length) {
				pr_notice("ram console buffer size %zx\n", size);
				memcpy(dst, pr.buf + size - length, length);
				log_length = length;
			} else {
				memcpy(dst, pr.buf, size);
				log_length = size;
			}
		}
		kfree(pr.buf);
	...
	if (pr.psi->close)
		pr.psi->close(pr.psi);
out:
	mutex_unlock(&pr.psi->read_mutex);
