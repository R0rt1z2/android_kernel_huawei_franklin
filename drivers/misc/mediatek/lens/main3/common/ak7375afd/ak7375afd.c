...
#define AF_DRVNAME "AK7375AFD_DRV"
#define AF_I2C_SLAVE_ADDR       (0x1E >> 1)
#define AF_IC2_I2C_SLAVE_ADDR   (0x5A >> 1)
...
static int af_read_ic2_reg(u8 addr, u8 *result)
{
	int ret = 0;
	char buff = 0;
	char send_data[READ_SIZE] = { addr };

	g_af_i2c_client->addr = AF_IC2_I2C_SLAVE_ADDR;
	ret = i2c_master_send(g_af_i2c_client, send_data, READ_SIZE);
	if (ret < 0) {
		log_err("I2C read - send failed\n");
		return -1;
...
static int af_write_ic2_reg(u8 addr, u8 data)
{
	int ret = 0;
	char send_data[2] = { addr, data };
...
static int init_af(void)
{
	...
	if (*g_af_opened == VCM_INIT_STATUS) {
		...
		/* Device IC2 check */
		log_info("ak7375 ic2 check+\n");
		loge_if_ret(af_read_ic2_reg(VCM_CHIPID_REG, &data));
		data = data & VCM_CHIPID_MASK;
		if (data != VCM_WAKE_UP) {
			log_info("ak7375 ic2 check ic error data = %u", data);
			return -1;
		}
		/* start ak7375 ic2 */
		g_af_i2c_client->addr = AF_IC2_I2C_SLAVE_ADDR;
...
static inline int set_vcm_pos(unsigned long position)
{
	...
	/* set ic2 vcm pos */
	loge_if_ret(af_write_ic2_reg(VCM_SET_POS_REG_H,
		(af_conv_code >> VCM_SET_POS_REG_OFFSET)));
	loge_if_ret(af_write_ic2_reg(VCM_SET_POS_REG_L,
		((af_conv_code & VCM_SET_POS_REG_L_SET_BIT) <<
			VCM_SET_POS_REG_OFFSET)));
...
long ak7375afd_ioctl(struct file *a_file, unsigned int a_command,
...
int ak7375afd_release(struct inode *a_inode, struct file *a_file)
{
	log_info("Start\n");
	...
	if (*g_af_opened == VCM_OPEN_STATUS) {
		...
		af_write_ic2_reg(VCM_MODE_SETTING, 0x01); /* 1:sleep mode */
	}
...
int ak7375afd_set_i2c_client(struct i2c_client *af_i2c_client,
...
int ak7375afd_get_file_name(unsigned char *file_name)
{
