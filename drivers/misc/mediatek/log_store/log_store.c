...
#endif
#ifdef CONFIG_BOOT_DETECTOR
	g_boot_phase = step & BOOT_PHASE_MASK;
#endif
...
#if defined(CONFIG_BOOT_DETECTOR) || defined(CONFIG_HUAWEI_STORAGE_ROFA_FOR_MTK)
	if (pbuff == NULL)
#endif
...
char *get_phys_mem_base(void)
{
	...
	return pbuff + LOG_STORE_SIZE - 1024;
}

char *get_rofa_mem_base(void)
{
	if (pbuff != NULL)
		return pbuff + LOG_STORE_SIZE - ROFA_MEM_SIZE;

	if (sram_dram_buff == NULL || sram_dram_buff->buf_addr == 0 ||
		sram_dram_buff->buf_size == 0) {
...
{
	return (char *)(uintptr_t)sram_dram_buff->klog_addr;
}

u32 get_klog_size(void)
{
	return sram_dram_buff->klog_size;
}

u32 get_total_klog_size(void)
{
	return sram_dram_buff->buf_size;
}

char *get_pl_lk_log_addr(void)
{
	return pbuff + dram_curlog_header->off_pl;
}

u32 get_pl_lk_log_size(void)
{
	return dram_curlog_header->sz_lk + dram_curlog_header->sz_pl;
}

u32 get_total_pl_lk_log_size(void)
{
	return dram_curlog_header->buff_size;
}
#endif
