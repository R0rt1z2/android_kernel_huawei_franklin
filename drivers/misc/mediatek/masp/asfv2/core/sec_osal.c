 ...
 * PORTING LAYER
 *****************************************************************************/
void osal_kfree(const void *buf)
{
	/* kfree(buf); */
	vfree(buf);
}
...
unsigned long osal_copy_from_user(void *to, const void *from,
				  unsigned long size)
{
...
unsigned long osal_copy_to_user(void *to, const void *from, unsigned long size)
{
...
void *osal_get_filp_struct(int fp_id)
{
	...
	if (fp_id >= 1 && fp_id < OSAL_MAX_FP_COUNT) {
		...
		if (!val) { // 0: Get Semaphone Successful!
			ret = g_osal_fp[fp_id];
...
loff_t osal_filp_seek_set(int fp_id, loff_t off)
{
	...
	if (fp_id >= 1 && fp_id < OSAL_MAX_FP_COUNT) {
		val = down_interruptible(&osal_fp_sem);
		if (val)   /* Failed to get semaphore */
...
long osal_filp_read(int fp_id, char *buf, unsigned long len)
{
	...
	if (fp_id >= 1 && fp_id < OSAL_MAX_FP_COUNT) {
		...
		read_len =
			g_osal_fp[fp_id]->f_op->read(g_osal_fp[fp_id],
						     buf,
						     len,
