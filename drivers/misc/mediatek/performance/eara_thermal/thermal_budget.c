...
#define EARA_THRM_LOGD(fmt, args...) pr_debug(EARA_THRM_TAG fmt, ##args)
...
#if defined(CONFIG_MACH_MT6893) && !defined(CONFIG_MTK_PLAT_MT6885_EMULATION)
	cobra_tbl = NULL;
#else
...
static int eara_thrm_table_show(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "max cluster: %d\n", max_cluster);
	seq_printf(m, "min cluster: %d\n", min_cluster);
	seq_printf(m, "#core: %d\n", g_nr_cpus);
