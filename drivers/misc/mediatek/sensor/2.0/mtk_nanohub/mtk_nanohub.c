...
static uint16_t data_action_cnt;
...
{
	if (data->sensor_type >= ID_SENSOR_MAX)
		return;
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		data_action_cnt++;
		if (data_action_cnt > DATA_ACTION_SIZE) {
			data_action_cnt = 0;
			pr_info("kernel get sensor(%u) bias = %d %d %d %d %d %d\n", event.sensor_type,
				event.word[0], event.word[1], event.word[2],
