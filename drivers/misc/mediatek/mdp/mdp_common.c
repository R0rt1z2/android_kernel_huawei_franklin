	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	CMDQ_LOG_PMQOS(
		"%s handle:%p engine:%#llx thread:%d list size:%u mdp:%u isp:%u\n",
		__func__, handle, handle->engineFlag, handle->thread, size,
		mdp_curr_pmqos->mdp_total_pixel,
		mdp_curr_pmqos->isp_total_pixel);

	CMDQ_LOG_PMQOS(
		"[MDP]mdp %u pixel %u byte, isp %u pixel %u byte, submit %06ld us, end %06ld us\n",
		mdp_curr_pmqos->mdp_total_pixel,
	...
	CMDQ_LOG_PMQOS(
		"[MDP][%d]mdp_curr_bandwidth %u isp_curr_bandwidth %u act_throughput %u\n",
	...
	if (mdp_is_isp_camin(handle)) {
		...
		pm_qos_update_request(&isp_clk_qos_request[thread_id],
			act_throughput);
		CMDQ_LOG_PMQOS(
			"[MDP][%d]begin task qos update throughput isp %u (isp cam)\n",
	...
	if (mdp_curr_pmqos->mdp_total_pixel) {
		...
		pm_qos_update_request(&mdp_clk_qos_request[thread_id],
			act_throughput);
		CMDQ_LOG_PMQOS(
			"[MDP][%d]begin task qos update throughput mdp %u (curr pixel)\n",
			thread_id, act_throughput);
		CMDQ_SYSTRACE_END();
	}
	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	CMDQ_LOG_PMQOS(
		"[MDP][%d]mdp_curr_bandwidth %d isp_curr_bandwidth %d act_throughput %d mdp data %u isp data %u\n",
		thread_id, mdp_curr_bandwidth, isp_curr_bandwidth,
		act_throughput, mdp_data_size, isp_data_size);
	...
	if (act_throughput == 0) {
		update_isp_throughput = true;
		CMDQ_LOG_PMQOS(
			"end_task thread_id: %d act_throughput 0 so update isp throughput!\n",
			thread_id);
	}
	...
	if (update_isp_throughput) {
		...
		pm_qos_update_request(&isp_clk_qos_request[thread_id],
			act_throughput);
		CMDQ_LOG_PMQOS(
			"[MDP][%d]end task qos update throughput isp %u (update isp throughput)\n",
			thread_id, act_throughput);
		CMDQ_SYSTRACE_END();
	} else {
		/*update clock*/
		if (mdp_curr_pmqos->isp_total_pixel) {
			...
			pm_qos_update_request(&isp_clk_qos_request[thread_id],
				0);
			CMDQ_LOG_PMQOS(
				"[MDP][%d]end task qos update throughput isp %u (curr pixel)\n",
				thread_id, act_throughput);
			CMDQ_SYSTRACE_END();
		}
	}
...
#else
	CMDQ_LOG_PMQOS(
		"act_tput %u curr->mdp_total_pixel %u curr->mdp_total_data %u\n",
	...
	if (target_pmqos && target_pmqos->mdp_total_datasize) {
		...
		CMDQ_SYSTRACE_BEGIN("%s mm mdp off\n", __func__);
		mm_qos_update_all_request_zero(
			&qos_mdp_module_request_list[thread_id]);
		CMDQ_SYSTRACE_END();
	}
	...
	/* update clock */
	if (target_pmqos && target_pmqos->mdp_total_pixel) {
		CMDQ_SYSTRACE_BEGIN("%s pm mdp\n", __func__);
		pm_qos_update_request(&mdp_clk_qos_request[thread_id],
			act_throughput);
		CMDQ_LOG_PMQOS(
			"[MDP][%d]end task qos update throughput mdp %u (curr pixel data)\n",
			thread_id, act_throughput);
		CMDQ_SYSTRACE_END();
	} else if (mdp_curr_pmqos->mdp_total_pixel) {
		CMDQ_SYSTRACE_BEGIN("%s pm mdp off\n", __func__);
		pm_qos_update_request(&mdp_clk_qos_request[thread_id], 0);
		CMDQ_LOG_PMQOS(
			"[MDP][%d]end task qos update throughput mdp %u (curr pixel only)\n",
			thread_id, 0);
		CMDQ_SYSTRACE_END();
	}
