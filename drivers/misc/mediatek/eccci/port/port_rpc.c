...
#include "port_rpc.h"
#define MODULE_SIM 1
#define SD_SIM_WAIT 2
#define CARD_TYPE_SIM 5
#ifdef CONFIG_MMC_DW_MUX_SDSIM
extern int sd_sim_detect_run(void *host, int status, int current_module, int need_sleep);
extern int get_card1_type(void);
static bool g_muxsdsim;
struct wakeup_source   hw_sim_sd_wakelock;
#endif
...
#ifdef CONFIG_MMC_DW_MUX_SDSIM
	node = of_find_node_by_name(NULL, "MD1_SIM1_HOT_PLUG_EINT");
	if (node != NULL) {
		ret = of_property_read_u32_index(node, "muxsdsim", 0, &value);
		if (ret != 0) {
			CCCI_NORMAL_LOG(md_id, RPC, "[HUAWEI]muxsdsim:? not found\n");
		} else {
			g_muxsdsim = value;
			CCCI_NORMAL_LOG(md_id, RPC, "[HUAWEI]muxsdsim: val:%d\n", value);
		}
	} else {
		CCCI_NORMAL_LOG(md_id, RPC, "%s: no found\n", "MD1_SIM1_HOT_PLUG_EINT");
	}
#endif
}
	...
	struct rpc_buffer *p_rpc_buf, unsigned int tmp_data[])
{
 ...
 err4:
	...
	case IPC_RPC_QUERY_CARD_TYPE:
		{
			unsigned int cardtray = 0;
			int val = CARD_TYPE_SIM;
			cardtray = *(unsigned int *)(pkt[0].buf);
			// SD detect
#ifdef CONFIG_MMC_DW_MUX_SDSIM
			CCCI_NORMAL_LOG(md_id, RPC,
					"SD/SIM get card type step in for [0x%X]: muxsdsim=%d, cardTray is %u\n",
					p_rpc_buf->op_id, g_muxsdsim, cardtray);
			if (g_muxsdsim == false) {
				val = CARD_TYPE_SIM;
			} else {
				__pm_wakeup_event(&hw_sim_sd_wakelock, jiffies_to_msecs(15 * HZ));
				val = sd_sim_detect_run(NULL, cardtray, MODULE_SIM, 0);
				if (val != SD_SIM_WAIT)
					val = get_card1_type();
			}
#endif
			tmp_data[0] = val;
			CCCI_NORMAL_LOG(md_id, RPC,
					"SD/SIM get card type value [0x%d]: pkt_num=%d!\n", val, pkt_num);
			pkt_num = 0;
			pkt[pkt_num].len = sizeof(unsigned int);
			pkt[pkt_num++].buf = (void *)&tmp_data[0];
			pkt[pkt_num].len = sizeof(unsigned int);
...
static int port_rpc_init(struct port_t *port)
{
	...
	if (first_init) {
		#ifdef CONFIG_MMC_DW_MUX_SDSIM
		wakeup_source_init(&hw_sim_sd_wakelock, "hw_sim_sd_wakelock");
		g_muxsdsim = true;
		#endif
