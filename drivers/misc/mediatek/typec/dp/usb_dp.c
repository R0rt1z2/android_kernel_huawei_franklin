...
#ifdef CONFIG_DP_AUX_SWITCH
#define DP_DISCONNECT 0x2
#define DP_CONNECT 0x4
#define DP_IRQ_EVENT 0x8
...
struct dp_interrupt_work {
	spinlock_t interrupt_lock;
	struct delayed_work interrupt_work;
};
struct dp_interrupt_work g_dp_interrupt_work;
uint8_t g_usb_polarity;
#endif
...
#ifdef CONFIG_DP_AUX_SWITCH
void dp_interrupt_set_func(int irq_statue)
{
	spin_lock(&(g_dp_interrupt_work.interrupt_lock));
	if (irq_statue == DP_CONNECT)
		schedule_delayed_work(&(g_dp_interrupt_work.interrupt_work),
			msecs_to_jiffies(500));
	else if (irq_statue == DP_DISCONNECT)
		cancel_delayed_work(&(g_dp_interrupt_work.interrupt_work));
	spin_unlock(&(g_dp_interrupt_work.interrupt_lock));
}

static void dp_interrupt_set_work(struct work_struct *work)
{
	UNUSED(work);
	mtk_dp_SWInterruptSet(DP_CONNECT);
}

static void dp_aux_switch_set(int enable, uint8_t polarity)
{
	if (enable == AUX_ENABLE) {
		dp_aux_switch_op(polarity == 0 ?
...
static void dp_hpd_irq_handler(uint8_t irq, uint8_t state, uint8_t polarity)
{
	if (state != 0) {
		dp_aux_switch_set(AUX_ENABLE, polarity);
		mdelay(100);
		if (irq != 0) {
			mtk_dp_SWInterruptSet(DP_IRQ_EVENT);
		} else {
			dp_interrupt_set_func(DP_DISCONNECT);
			mtk_dp_SWInterruptSet(DP_CONNECT);
		}
	} else {
		...
		if (hdp_state == 1) {
			dp_interrupt_set_func(DP_DISCONNECT);
			mtk_dp_SWInterruptSet(DP_DISCONNECT);
			dp_aux_switch_set(AUX_DISABLE, polarity);
		}
	}
	hdp_state = state;
}

static int chg_tcp_notifier_call_priv(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct tcp_notify *noti = data;

	if (data == NULL || nb == NULL) {
		usbc_dbg(K_ERR, "%s:input is NULL", __func__);
		return NOTIFY_BAD;
	}
	...
	if (event == TCP_NOTIFY_AMA_DP_STATE) {
		...
		g_usb_polarity = noti->ama_dp_state.polarity;
		if (active == 0) {
		...
		usbc_dbg(K_INFO, "TCP_NOTIFY_AMA_DP_STATE signal:%x pin:%x polarity:%x\n",
			signal, pin, g_usb_polarity);

		if (hdp_state == 0) {
			hdp_state = 1;
			dp_aux_switch_set(AUX_ENABLE, g_usb_polarity);
			dp_interrupt_set_func(DP_CONNECT);
		}
		...
		usbc_dbg(K_INFO, "TCP_NOTIFY_AMA_DP_HPD_STATE irq:%x state:%x\n",
			irq, state);
		dp_hpd_irq_handler(irq, state, g_usb_polarity);
		...
		if ((noti->typec_state.old_state == TYPEC_ATTACHED_SRC ||
			...
			if (hdp_state == 1) {
				...
				dp_interrupt_set_func(DP_DISCONNECT);
				mtk_dp_SWInterruptSet(DP_DISCONNECT);
				dp_aux_switch_set(AUX_DISABLE, g_usb_polarity);
			}
		}
	}
...
#ifdef CONFIG_DP_AUX_SWITCH
	if (is_need_define_notifier_call()) {
		dp_nb.notifier_call = chg_tcp_notifier_call_priv;
		register_tcp_dev_notifier(dp_tcpc_dev, &dp_nb,
			TCP_NOTIFY_TYPE_MODE | TCP_NOTIFY_TYPE_VBUS |
			TCP_NOTIFY_TYPE_USB | TCP_NOTIFY_TYPE_MISC);
		spin_lock_init(&(g_dp_interrupt_work.interrupt_lock));
		INIT_DELAYED_WORK(&g_dp_interrupt_work.interrupt_work,
			dp_interrupt_set_work);
		g_usb_polarity = 0;
		usbc_dbg(K_INFO, "%s aux switch is configed\n", __func__);
		return 0;
	}

	usbc_dbg(K_INFO, "%s use MTK usb switch driver\n", __func__);
#endif
...
#ifdef CONFIG_DP_AUX_SWITCH
	if (is_need_define_notifier_call()) {
		cancel_delayed_work_sync(&g_dp_interrupt_work.interrupt_work);
		return;
	};
#endif
...
void usb3_switch_ctrl_sel_dp(int sel)
{
	int res;
