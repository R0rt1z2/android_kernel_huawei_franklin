...
const void *get_wmt_cfg(void)
{
	struct device_node *node = NULL;

	node = of_find_node_by_path("/huawei_wifi_info");
	if (node == NULL) {
		 WMT_ERR_FUNC("Node is not available.\n");
		 return NULL;
	}
	return of_get_property(node, "antenna_types", NULL);
}
...
INT32 wmt_conf_read_file(VOID)
{
	...
	const char *wmt_cfg = NULL;
	char wmt_cfg_name[WMT_CFG_MAX_LEN] = {0};
	...
	wmt_cfg = get_wmt_cfg();
	if (wmt_cfg == NULL) {
		snprintf(wmt_cfg_name, sizeof(wmt_cfg_name), CUST_CFG_WMT_SOC);
	} else {
		snprintf(wmt_cfg_name, sizeof(wmt_cfg_name), "%s"CUST_CFG_WMT_SOC, wmt_cfg);
	}
	WMT_ERR_FUNC("wmt_cfg_name: %s\n", wmt_cfg_name);

	if (chip_type == WMT_CHIP_TYPE_SOC) {
		osal_memset(&gDevWmt.cWmtcfgName[0], 0, osal_sizeof(gDevWmt.cWmtcfgName));
		osal_strncat(&(gDevWmt.cWmtcfgName[0]), wmt_cfg_name, osal_sizeof(wmt_cfg_name));
	}
