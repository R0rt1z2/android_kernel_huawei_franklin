...
static bool g_isHeadsetOn = false;
...
{
	g_isHeadsetOn = status;

	WCN_DBG(FM_NTC | MAIN, "%s: [fm build-in] is_headset_on=%d\n", __func__, g_isHeadsetOn);
	if (fm_node && g_isHeadsetOn && gpio_is_valid(huawei_fm_info)) {
...
static void FmPowerOn()
{
	if (fm_node) {
		if ((!g_isHeadsetOn) && gpio_is_valid(huawei_fm_info)) {
