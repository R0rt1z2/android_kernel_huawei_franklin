...
void statsParseIPV4Info(IN struct ADAPTER *prAdapter, struct sk_buff *skb,
		uint8_t *pucEthBody, uint8_t eventType)
{
	...
	case IP_PRO_ICMP:
	{
		...
		case EVENT_RX:
			...
			prAdapter->rPerMonitor.rRxInfo.u4TotalRxIcmpCount++;
			break;
		case EVENT_TX:
			DBGLOG_LIMITED(TX, TRACE,
...
static void statsParsePktInfo(IN struct ADAPTER *prAdapter, uint8_t *pucPkt,
	struct sk_buff *skb, uint8_t status, uint8_t eventType)
{
		...
		case 3: /* key */
			...
			case EVENT_RX:
			{
				DBGLOG_LIMITED(RX, TRACE,
					...
					*((uint16_t *)(&pucEapol[5])));
				if (secGetEapolKeyType((unsigned char *)skb)
					== EAPOL_KEY_2_OF_2)
					prAdapter->rPerMonitor.rRxInfo.
							u4TotalRekeyCount++;
			}
				break;
			case EVENT_TX:
...
static void statsCheckPayloadFormat(IN struct ADAPTER *prAdapter,
	struct SW_RFB *prSwRfb)
{
	/* 2¡¦b11: the payload is from the first sub frame of an A-MSDU */
	if (prSwRfb->ucPayloadFormat == RX_PAYLOAD_FORMAT_FIRST_SUB_AMSDU)
		prAdapter->rPerMonitor.rRxInfo.u4TotalAmsduCount++;
}
 ...
 */
/*----------------------------------------------------------------------------*/
void StatsRxPktInfoDisplay(IN struct ADAPTER *prAdapter,
		struct SW_RFB *prSwRfb)
{
	uint8_t *pPkt = NULL;
	...
	statsCheckPayloadFormat(prAdapter, prSwRfb);
 ...
 */
/*----------------------------------------------------------------------------*/
void StatsTxPktInfoDisplay(IN struct ADAPTER *prAdapter, uint8_t *pPkt)
{
	...
	statsParsePktInfo(prAdapter, pPkt, NULL, 0, EVENT_TX);
}
