				  ...
				  IN struct SW_RFB *prSwRfb)
{
	...
	/* 4 <4> Update information from HIF RX Header */
	{
		...
		/* do moving average with factor 4, RCPI result =
		 * 3/4 previous RCPI + 1/4 new RCPI
		 */
		if (prBssDesc->ucRCPI) {
			u2RxRcpiTemp = (3 * prBssDesc->ucRCPI) + ucRxRCPI;

			if (u2RxRcpiTemp % 4 >= 2)
				ucRxRCPI = (u2RxRcpiTemp >> 2) + 1;
			else
				ucRxRCPI = (u2RxRcpiTemp >> 2);
		}
