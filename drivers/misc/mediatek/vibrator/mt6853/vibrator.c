...
void init_cust_vibrator_dtsi(struct platform_device *pdev)
{
	...
	if (pvib_cust == NULL) {
		...
		ret = of_property_read_u32(pdev->dev.of_node, "vib_vol_cal",
			&(pvib_cust->vib_vol_cal));
		if (!ret)
			VIB_DEBUG("vib_vol_cal: %d\n", pvib_cust->vib_vol_cal);
		else
			pvib_cust->vib_vol_cal = 0x00; // Cal vol default is 0
#endif
		pr_info("pvib_cust = %d, %d, %d, %d\n", pvib_cust->vib_timer,
...
void vibr_power_set(void)
{
	...
	if (hw != NULL) {
		...
		pmic_set_register_value(PMIC_RG_VIBR_VOCAL, hw->vib_vol_cal);
#endif
	} else {
