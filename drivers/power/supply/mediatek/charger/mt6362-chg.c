...
int mt6362_get_batt_by_usb_id(void)
{
	return 450000;
}

static struct charge_extra_ops mt6362_charge_extra_ops = {
        .get_batt_by_usb_id = mt6362_get_batt_by_usb_id,
};
...
static int mt6362_chg_probe(struct platform_device *pdev)
{
	...
	rc = charge_extra_ops_register(&mt6362_charge_extra_ops);
	if (rc != 0)
		dev_err(data->dev, "charge extra ops register fail\n");
