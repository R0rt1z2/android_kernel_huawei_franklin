...
#define COLOR_U32_PTR(x) ((unsigned int *)(unsigned long)x)
		...
		struct drm_file *file_priv)
{
	...
	PQIndexTable *mIoPQIndex = NULL;

	DDPINFO("%s...", __func__);
	mIoPQIndex = (PQIndexTable *)data;
	pq_index = get_Color_index();
	if (copy_from_user(pq_index, COLOR_U32_PTR(mIoPQIndex->pqindex), sizeof(struct DISPLAY_PQ_T)) == 0) {
		DDPINFO("copy pqindex sucess");
	}
