	...
	int force_lcm_update)
{
	...
	if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
#ifdef CONFIG_LCD_KIT_DRIVER
		if (dsi->ext && dsi->ext->params && dsi->ext->params->bf_stop_vdo_delay > 0) {
			msleep(dsi->ext->params->bf_stop_vdo_delay);
			DDPMSG("%s:bf_stop_vdo_delay:%d\n", __func__, dsi->ext->params->bf_stop_vdo_delay);
		}
#endif
		mtk_dsi_stop_vdo_mode(dsi, NULL);
	} else {
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_STOP_VDO_MODE:
		ret = (int *)params;
		*ret = mtk_dsi_stop_vdo_mode(dsi, handle);
		break;
	case ESD_CHECK_READ:
