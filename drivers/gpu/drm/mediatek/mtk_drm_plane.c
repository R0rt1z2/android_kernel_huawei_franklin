				    ...
				    struct drm_plane_state *old_state)
{
	...
	src_w = (unsigned int)drm_rect_width(&plane->state->src) >> 16;
	src_h = (unsigned int)drm_rect_height(&plane->state->src) >> 16;
	...
	state->pending.src_x = ((unsigned int)plane->state->src.x1 >> 16);
	state->pending.src_y = ((unsigned int)plane->state->src.y1 >> 16);
...
static void mtk_plane_attach_property(struct mtk_drm_plane *plane)
{
	...
	struct mtk_drm_property *plane_prop = NULL;
	int i;
	static int num;
