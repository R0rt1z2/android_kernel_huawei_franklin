	...
	struct cmdq_task *task, struct list_head **insert_pos)
{
	...
	struct cmdq_task *prev_task = NULL, *next_task = NULL;
	struct cmdq_task *cursor_task = NULL;
...
{
	struct cmdq *cmdq;
	struct cmdq_task *task = NULL, *last_task = NULL;
...
{
	struct cmdq_thread *thread = task->thread;
	struct cmdq_task *next_task = NULL;
...
{
	struct cmdq_task *task = NULL, *tmp = NULL, *curr_task = NULL;
	u32 irq_flag;
...
static irqreturn_t cmdq_irq_handler(int irq, void *dev)
{
	...
	struct cmdq_task *task = NULL, *tmp = NULL;
...
static void cmdq_thread_handle_timeout_work(struct work_struct *work_item)
{
	...
	struct cmdq *cmdq = container_of(thread->chan->mbox, struct cmdq, mbox);
	struct cmdq_task *task = NULL, *tmp = NULL, *timeout_task = NULL;
	unsigned long flags;
	...
	u64 **inst_out, dma_addr_t *pc_out)
{
	...
	size_t size = 0;
	u64 *end_va = NULL, *curr_va = NULL, inst = 0, last_inst[2] = {0};
	void *va_base = NULL;
	dma_addr_t pa_base;
	bool empty = true;
