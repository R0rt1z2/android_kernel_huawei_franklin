...
static int pm_qos_memory_lat_notify(struct notifier_block *b,
		unsigned long l, void *v)
{
	int opp_id = get_freq_ddr_opp(l);

	pm_qos_update_request(&dvfsrc_memory_latency_req, opp_id);
...
#ifdef CONFIG_HUAWEI_DRAM_FREQ_CONTROL
	dvfsrc->pm_qos_memory_lat_nb.notifier_call =
		pm_qos_memory_lat_notify;
#endif
...
#ifdef CONFIG_HUAWEI_DRAM_FREQ_CONTROL
	pm_qos_add_notifier(PM_QOS_MEMORY_LATENCY,
			&dvfsrc->pm_qos_memory_lat_nb);
#endif
}
