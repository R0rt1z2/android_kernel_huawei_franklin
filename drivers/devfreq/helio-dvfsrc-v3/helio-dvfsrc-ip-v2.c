...
static int get_bw_freq(int bw_value, struct bw_freq *bw_map, int size)
{
	int i;

	for (i = 0; i < size; i++) {
		if (bw_value >= bw_map[i].bw_value)
			return bw_map[i].freq_value;
	}
	return 0;
}

void plat_get_ddr_other_vote(int *freq)
{
	int apu_bw_qos, cpu_bw_qos, gpu_bw_qos, mm_bw_qos, other_bw_qos, all_bw_qos;
	int apu_bw_real, cpu_bw_real, gpu_bw_real, mm_bw_real, other_bw_real, all_bw_real;
	int isp_hrt_bw;
	int all_freq, isp_hrt_bw_freq, all_freq_real;

	if (!freq)
		return;

	apu_bw_qos = pm_qos_request(PM_QOS_APU_MEMORY_BANDWIDTH);
	cpu_bw_qos = pm_qos_request(PM_QOS_CPU_MEMORY_BANDWIDTH);
	gpu_bw_qos = pm_qos_request(PM_QOS_GPU_MEMORY_BANDWIDTH);
	mm_bw_qos = pm_qos_request(PM_QOS_MM_MEMORY_BANDWIDTH);
	other_bw_qos = pm_qos_request(PM_QOS_OTHER_MEMORY_BANDWIDTH);
	all_bw_qos = apu_bw_qos + cpu_bw_qos + gpu_bw_qos + mm_bw_qos + other_bw_qos;
	isp_hrt_bw = pm_qos_request(PM_QOS_ISP_HRT_BANDWIDTH);

	apu_bw_real = dvfsrc_read(DVFSRC_SW_BW_0);
	cpu_bw_real = dvfsrc_read(DVFSRC_SW_BW_1);
	gpu_bw_real = dvfsrc_read(DVFSRC_SW_BW_2);
	mm_bw_real = dvfsrc_read(DVFSRC_SW_BW_3);
	other_bw_real = dvfsrc_read(DVFSRC_SW_BW_4);
	/* bw reg data multiply 100 to get real data */
	all_bw_real = (apu_bw_real + cpu_bw_real + gpu_bw_real + mm_bw_real + other_bw_real) * 100;

	all_freq_real = get_bw_freq(all_bw_real, qos_map, ARRAY_SIZE(qos_map));
	all_freq = get_bw_freq(all_bw_qos, qos_map, ARRAY_SIZE(qos_map));
	isp_hrt_bw_freq = get_bw_freq(isp_hrt_bw, hrt_map, ARRAY_SIZE(hrt_map));

	*freq = max(all_freq, all_freq_real);
	*freq = max(*freq, isp_hrt_bw_freq);
}
#endif
