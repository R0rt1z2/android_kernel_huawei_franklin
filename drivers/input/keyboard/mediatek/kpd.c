...
#endif
#ifdef CONFIG_HW_COMB_KEY
	power_key_status_distinguish(pressed);
#endif
#ifdef CONFIG_HW_ZEROHUNG
...
#ifdef CONFIG_HW_COMB_KEY
	volume_key_status_distinguish(pressed);
#endif
#ifdef CONFIG_HW_ZEROHUNG
	hung_wp_screen_vkeys_cb(kpd_dts_data.kpd_sw_rstkey, pressed);
#endif
}
...
#ifdef CONFIG_HW_ZEROHUNG
			hung_wp_screen_vkeys_cb((unsigned int)linux_keycode, pressed);
#endif
#ifdef CONFIG_LONG_PRESS_MODE_EN
			if (pressed) {
