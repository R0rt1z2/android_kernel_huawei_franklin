...
#define HUAWEI_SOFTTIME_FREQ                  (32*1024)
 ...
 * 触发一个softtimer
 */
void huawei_softtimer_add(struct softtimer_list *timer)
{
 ...
 * 软计时器删除
 */
static int huawei_softtimer_try_to_del_sync(struct softtimer_list *timer)
{
	struct softtimer_list *p = NULL;
 ...
 * 删除一个softtimer
 */
int huawei_softtimer_delete(struct softtimer_list *timer)
{
	if (NULL == timer)    {
		return  -EIO;
	}
	for (;;) {
		int ret = huawei_softtimer_try_to_del_sync(timer);
		if (ret == 0) {
			return ret;
		}
		cpu_relax();
 ...
 * 创建软计时器
 */
int huawei_softtimer_create(struct softtimer_list *sft_info, softtimer_func func,
 ...
 * 修改软计时器
 */
int huawei_softtimer_modify(struct softtimer_list *timer, unsigned int new_expire_time)
{
	unsigned long delaytime;
 ...
 * 释放软计时器资源
 */
int huawei_softtimer_free(struct softtimer_list *p)
{
 ...
 * 软计时器去初始化
 */
void huawei_softtimer_uninit(void)
{
 ...
 * 软计时器初始化
 */
static int huawei_softtimer_init (struct platform_device *pdev)
{
	int ret;
	...
	if (!timer_control.hard_timer_addr) {
		pr_err("[%s] softtimer:huawei_softtimer_init ERROR.\n", __func__);
		goto iomap_fin;
	}
	INIT_LIST_HEAD(&(timer_control.timer_list_head));
	timer_control.rate                    = HUAWEI_SOFTTIME_FREQ;
	timer_control.softtimer_start_value  = ELAPESD_TIME_INVAILD;
	...
	timer_control.is_in_interrupt = TIMER_FALSE;
	pr_err("[%s] softtimer:huawei_softtimer_init enter,%d\n", __func__, timer_control.rate);
	return 0;
error:
	iounmap(timer_control.hard_timer_addr);
 ...
 * 软计时器超时回调
 */
static void huawei_softtimer_timeout(unsigned long data)
{
	(void)data;
	pr_err("[%s] huawei_softtimer_timeout\n", __func__);
	return ;
}
/*lint -e727*/
struct softtimer_list timer = {0};
void huawei_softtimer_test(int timeout)
{
	int ret;
	static int  flag;
	if (0 == flag) {
		huawei_softtimer_create(&timer, huawei_softtimer_timeout, (unsigned long)0, 0);
		flag = 1;
	}
	pr_err("[%s] huawei_softtimer_test enter\n", __func__);
	huawei_softtimer_delete(&timer);
	ret = huawei_softtimer_modify(&timer, (unsigned int)timeout);
	if (ret) {
		pr_err("[%s] %d:\n", __func__, __LINE__);
	}
	huawei_softtimer_add(&timer);
}
#endif
/*lint +e727*/
/*lint -e715*/
...
static int generic_softtimer_probe(struct platform_device *pdev)
{
	...
	return huawei_softtimer_init(pdev);
}
...
* softtimer功能的初始化
*/
static int __init huawei_timer_init(void)
{
 ...
 * 释放softtimer功能资源
 */
static void __exit huawei_timer_exit(void)
{
	platform_driver_unregister(&generic_softtimer_platdrv);
}
/*lint -e528 -esym(528,*) */
late_initcall(huawei_timer_init);
module_exit(huawei_timer_exit);
