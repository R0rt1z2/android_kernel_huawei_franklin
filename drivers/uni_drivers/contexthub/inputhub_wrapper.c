 ...
 */
int inputhub_wrapper_pack_and_send_cmd(int app_tag, int cmd, int sub_cmd, const char *buf, size_t count, struct read_info 
			*rd_info, bool is_lock)
{
	...
	ret = inputhub_wrapper_send_cmd(app_tag, cmd, send_buf, send_len, rd_info, is_lock);
	if (ret != 0) {
...
/*
 * Function    : inputhub_wrapper_pack_and_send_cmd_unified
 * Description : this interface support unified transfter no matter how long buf length is
 *             : This function will return error code, how to use depend on caller.
 * Input       : [app_tag] app tag.
 *             : [cmd] cmd, CMD_CMN_CONFIG_REQ, CMD_CMN_OPEN_REQ and etc.
 *             : [buf] For CMD_CMN_CONFIG_REQ, this is customized buffer.
 ...
 * Return      : 0:successs, others: fail
 */
int inputhub_wrapper_pack_and_send_cmd_unified(int app_tag, int cmd, int sub_cmd, const char *buf, size_t len,
			struct read_info *rd_info, bool is_lock)
{
	char *buffer = NULL;
	int ret;
	 ...
	 */
	if (cmd == CMD_CMN_CONFIG_REQ) {
		send_len = sizeof(u32) + (size_t)len;
		buffer = kzalloc(send_len, GFP_KERNEL); //lint !e838
		...
		((u32 *)buffer)[0] = sub_cmd;
		if (len != 0) {
			ret = memcpy_s(buffer + sizeof(u32), len, buf, len);
			if (ret != 0) {
				pr_err("[%s], memcpy failed!\n", __func__);
				ret = -EINVAL;
				goto OUT;
