...
#ifndef __HUAWEI_CONTEXTHUB_HUAWEI_SOFTTIMER_H__
#define __HUAWEI_CONTEXTHUB_HUAWEI_SOFTTIMER_H__
 ...
 * 添加软计时器
 */
extern void huawei_softtimer_add(struct softtimer_list *timer);
/*
 * 删除软计时器
 */
extern int huawei_softtimer_delete(struct softtimer_list *timer);
/*
 * 创建软计时器
 */
extern int huawei_softtimer_create(struct softtimer_list *timer, softtimer_func func,
       unsigned long para, unsigned int timeout);
/*
 * 修改软计时器
 */
extern int huawei_softtimer_modify(struct softtimer_list *timer, unsigned int new_expire_time);
#endif
