...
static int huawei_flp_pm_suspend(struct device *dev)
{
	dev_info(dev, "%s %d: \n", __func__, __LINE__);
	return 0;
}
 ...
 * 驱动resume函数
 */
static int huawei_flp_pm_resume(struct device *dev)
{
	dev_info(dev, "%s %d: \n", __func__, __LINE__);
	flp_port_resume();
...
/*lint -e785*/
struct dev_pm_ops huawei_flp_pm_ops = {
#ifdef CONFIG_PM_SLEEP
	.suspend = huawei_flp_pm_suspend,
	.resume  = huawei_flp_pm_resume,
#endif
};
...
static int generic_flp_remove(struct platform_device *pdev)
{
	...
	driver_remove_file(&generic_flp_platdrv.driver,
			   &driver_attr_debug_level);
	huawei_flp_unregister();
	return 0;
}

/*lint -e64*/
static int __init huawei_flp_init(void)
{
	pr_info("%s %d: \n", __func__, __LINE__);
...
static void __exit huawei_flp_exit(void)
{
	platform_driver_unregister(&generic_flp_platdrv);
}

/*lint -e528 -esym(528,*) */
late_initcall_sync(huawei_flp_init);
module_exit(huawei_flp_exit);
/*lint +e528 -esym(528,*) */
