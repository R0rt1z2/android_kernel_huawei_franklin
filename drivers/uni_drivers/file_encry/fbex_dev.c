...
/* mutex for huawei_fbex_ioctrl_cmd */
...
static void huawei_fbex_start_fn(struct work_struct *work)
{
	u32 ret;
	u32 cmd;
...
const static struct cmd_table g_cmd_table[] = {
	{ HUAWEI_FBEX_ADD_IV, SEC_FILE_ENCRY_CMD_ID_VOLD_ADD_IV },
	{ HUAWEI_FBEX_DEL_IV, SEC_FILE_ENCRY_CMD_ID_VOLD_DELETE_IV },
	{ HUAWEI_FBEX_USER_LOGOUT, SEC_FILE_ENCRY_CMD_ID_USER_LOGOUT },
};
...
static u32 huawei_fbex_copy_out(u32 cmd, struct fbex_req *from, void __user *to)
{
	u32 ret;
...
static u32 huawei_fbex_cmd_dispatch(u32 cmd, void __user *argp)
{
	u32 ret;
	u32 id;
	...
	if (!argp) {
		...
		file_encry_record_error(0, 0, 0, FBE2_ERROR_INIT_BUFFER);
		return FBE2_ERROR_INIT_BUFFER;
	}
	ret = copy_from_user(&g_fbex_data.req, argp, sizeof(g_fbex_data.req));
	if (ret) {
		pr_err("%s, copy from user fail\n", __func__);
		file_encry_record_error(0, 0, 0, FBE2_ERROR_INIT_INFO);
		return FBE2_ERROR_INIT_INFO;
	}
	...
	if (!wait_for_completion_timeout(&g_fbex_ta_comp,
		...
		file_encry_record_error(g_fbex_data.cmd, g_fbex_data.req.user,
					...
					FBE2_ERROR_CKEY_TIMEER_OUT);
		return FBE2_ERROR_CKEY_TIMEER_OUT;
	}
	...
	ret = huawei_fbex_copy_out(id, &g_fbex_data.req, argp);
	if (ret != 0) {
		file_encry_record_error(g_fbex_data.cmd, g_fbex_data.req.user,
					g_fbex_data.req.file, ret);
		return ret;
	}
	/* return result value from FBE2 service */
	return g_fbex_data.ret;
}
				    ...
				    unsigned long arg)
{
	...
	ret = huawei_fbex_cmd_dispatch(cmd, argp);
...
void __exit huawei_fbex_module_exit(void)
{
...
int __init huawei_fbex_module_init(void)
{
	int ret;
	...
	INIT_WORK(&g_fbex_work, huawei_fbex_start_fn);
	pr_info("%s success\n", __func__);
	return 0;
out:
	huawei_fbex_module_exit();
	pr_err("%s fail, %d\n", __func__, ret);
	return ret;
}

module_init(huawei_fbex_module_init);
module_exit(huawei_fbex_module_exit);

MODULE_DESCRIPTION("huawei fbex driver");
MODULE_ALIAS("huawei fbex driver");
MODULE_LICENSE("GPL");
