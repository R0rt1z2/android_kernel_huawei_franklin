...
void hi6526_dpm_extract_pdo_info(uint32_t pdo, struct dpm_pdo_info_t *info)
{
		...
		int min_uw, uint8_t policy)
{
	...
	hi6526_dpm_extract_pdo_info(snk_pdo, &sink);
	...
	for (i = 0; i < cnt; i++) {
		hi6526_dpm_extract_pdo_info(src_pdos[i], &source);
	...
	if (select.pos > 0) {
		hi6526_dpm_extract_pdo_info(src_pdos[select.pos - 1], &source);
