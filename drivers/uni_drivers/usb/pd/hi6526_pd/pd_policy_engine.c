...
static void pe_idle_reset_data(pd_port_t *pd_port)
{
	...
	hi6526_pd_reset_svid_data(pd_port);
	...
	case PE_BIST_TEST_DATA:
		hi6526_pd_enable_bist_test_mode(pd_port, false);
		break;

	case PE_BIST_CARRIER_MODE_2:
		hi6526_pd_disable_bist_mode2(pd_port);
		break;
...
static void hi6526_pe_idle1_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pe_idle_reset_data(pd_port);
	hi6526_pd_try_put_pe_idle_event(pd_port);
}

static void hi6526_pe_idle2_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_IDLE);
	pd_disable_timer(pd_port, PD_TIMER_PE_IDLE_TOUT);
	hi6526_pd_notify_pe_idle(pd_port);
}

void hi6526_pe_error_recovery_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pe_idle_reset_data(pd_port);
	hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_IDLE);
	hi6526_pd_notify_pe_error_recovery(pd_port);
	pd_free_pd_event(pd_port, pd_event);
}


void hi6526_pe_bist_test_data_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	hi6526_pd_enable_bist_test_mode(pd_port, true);
	pd_free_pd_event(pd_port, pd_event);
}

void hi6526_pe_bist_test_data_exit(pd_port_t *pd_port, pd_event_t *pd_event)
{
	hi6526_pd_enable_bist_test_mode(pd_port, false);
}

void hi6526_pe_bist_carrier_mode_2_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	hi6526_pd_send_bist_mode2(pd_port);
	pd_enable_timer(pd_port, PD_TIMER_BIST_CONT_MODE);
	pd_free_pd_event(pd_port, pd_event);
}

void hi6526_pe_bist_carrier_mode_2_exit(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
void hi6526_pe_power_ready_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_port->data_role == PD_ROLE_UFP)
		hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_READY_UFP);
	else
		hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_READY_DFP);
...
#define PE_STATE_ACTIONS(state) { .entry_action = hi6526_##state##_entry, }

static const struct pe_state_action_t pe_state_actions[] = {
	/* src activity */
	PE_STATE_ACTIONS(pe_src_startup),               /* hi6526_pe_src_startup_entry */
	PE_STATE_ACTIONS(pe_src_discovery),             /* hi6526_pe_src_discovery_entry */
	PE_STATE_ACTIONS(pe_src_send_capabilities),     /* hi6526_pe_src_send_capabilities_entry */
...
#ifdef CONFIG_USB_PD_SRC_STARTUP_DISCOVER_ID
	PE_STATE_ACTIONS(pe_src_vdm_identity_request), /* hi6526_pe_src_vdm_identity_request_entry */
	...
	PE_STATE_ACTIONS(pe_prs_snk_src_assert_rp), /* hi6526_pe_prs_snk_src_assert_rp_entry */
...
static pe_state_action_fcn_t pe_state_exit_actions[PE_STATE_MAX] = {
	/* Source */
	[PE_SRC_SEND_CAPABILITIES] = hi6526_pe_src_send_capabilities_exit,
	[PE_SRC_TRANSITION_SUPPLY] = hi6526_pe_src_transition_supply_exit,
	[PE_SRC_TRANSITION_TO_DEFAULT] = hi6526_pe_src_transition_to_default_exit,
	[PE_SRC_GET_SINK_CAP] = hi6526_pe_src_get_sink_cap_exit,
#ifdef CONFIG_USB_PD_REV30
#ifdef CONFIG_USB_PD_REV30_STATUS_REMOTE
	[PE_SRC_GET_SINK_STATUS] = hi6526_pe_src_get_sink_status_exit,
#endif
#endif
	/* Sink */
	[PE_SNK_WAIT_FOR_CAPABILITIES] = hi6526_pe_snk_wait_for_capabilities_exit,
	[PE_SNK_SELECT_CAPABILITY] = hi6526_pe_snk_select_capability_exit,
#ifdef CONFIG_USB_PD_REV30
#ifdef CONFIG_USB_PD_REV30_SRC_CAP_EXT_REMOTE
	[PE_SNK_GET_SOURCE_CAP_EXT] = hi6526_pe_snk_get_source_cap_ext_exit,
#endif
#ifdef CONFIG_USB_PD_REV30_STATUS_REMOTE
	[PE_SNK_GET_SOURCE_STATUS] = hi6526_pe_snk_get_source_status_exit,
#endif
#ifdef CONFIG_USB_PD_REV30_PPS_SINK
	[PE_SNK_GET_PPS_STATUS] = hi6526_pe_snk_get_pps_status_exit,
#endif
#endif
	[PE_SNK_TRANSITION_SINK] = hi6526_pe_snk_transition_sink_exit,
	[PE_SNK_TRANSITION_TO_DEFAULT] = hi6526_pe_snk_transition_to_default_exit,
	[PE_DR_SRC_GET_SOURCE_CAP] = hi6526_pe_dr_src_get_source_cap_exit,
	[PE_DR_SNK_GET_SINK_CAP] = hi6526_pe_dr_snk_get_sink_cap_exit,

	[PE_BIST_TEST_DATA] = hi6526_pe_bist_test_data_exit,
	[PE_BIST_CARRIER_MODE_2] = hi6526_pe_bist_carrier_mode_2_exit,
	...
	[PE_PRS_SRC_SNK_WAIT_SOURCE_ON] = hi6526_pe_prs_src_snk_wait_source_on_exit,
	[PE_PRS_SNK_SRC_SOURCE_ON] = hi6526_pe_prs_snk_src_source_on_exit,
	[PE_PRS_SNK_SRC_TRANSITION_TO_OFF] = hi6526_pe_prs_snk_src_transition_to_off_exit,
	[PE_VCS_WAIT_FOR_VCONN] = hi6526_pe_vcs_wait_for_vconn_exit,
};

static pe_state_action_fcn_t hi6526_pe_get_exit_action(uint8_t pe_state)
{
...
static void pd_pe_state_change(pd_port_t *pd_port, pd_event_t *pd_event, bool vdm_evt)
{
	...
	if ((new_state == PE_IDLE1) || (new_state == PE_IDLE2))
		...
		prev_exit_action = hi6526_pe_get_exit_action(old_state);
...
static int pd_handle_event(pd_port_t *pd_port, pd_event_t *pd_event, bool vdm_evt)
{
	...
	ret = hi6526_pd_process_event(pd_port, pd_event, vdm_evt);
	if (ret)
...
static int hi6526_pe_get_vdm_event(struct hi6526_tcpc_device *tcpc_dev,
		pd_event_t *pd_event)
{
	...
	case PE_DBG_READY:
#endif
		vdm_evt = hi6526_pd_get_vdm_event(tcpc_dev, pd_event);
		break;
...
static int pd_try_get_active_event(struct hi6526_tcpc_device *tcpc_dev,
		pd_event_t *pd_event)
{
	bool ret = false;
	...
	ret = hi6526_pd_get_deferred_tcp_event(tcpc_dev, &pd_port->tcp_event);
...
int hi6526_pd_policy_engine_run(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	if (hi6526_pd_get_event(tcpc_dev, &pd_event)) {
		D("pd event\n");
	} else if (hi6526_pe_get_vdm_event(tcpc_dev, &pd_event)) {
		vdm_evt = true;
		D("vdm event\n");
