...
static void typec_wait_ps_change(struct hi6526_tcpc_device *tcpc_dev,
		enum TYPEC_WAIT_PS_STATE state)
{
	...
	if (state == TYPEC_WAIT_PS_SRC_VSAFE0V)
		hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_RT_TIMER_SAFE0V_TOUT);
#endif

	if (tcpc_dev->typec_wait_ps_change == TYPEC_WAIT_PS_SRC_VSAFE0V
			&& state != TYPEC_WAIT_PS_SRC_VSAFE0V) {
		hi6526_tcpc_disable_timer(tcpc_dev, TYPEC_RT_TIMER_SAFE0V_DELAY);

#ifdef CONFIG_TYPEC_ATTACHED_SRC_SAFE0V_TIMEOUT
		hi6526_tcpc_disable_timer(tcpc_dev, TYPEC_RT_TIMER_SAFE0V_TOUT);
#endif
	}
...
static inline int typec_enable_vconn(struct hi6526_tcpc_device *tcpc_dev)
{
...
static inline void typec_transfer_state(struct hi6526_tcpc_device *tcpc_dev,
...
static inline int typec_handle_vbus_present(struct hi6526_tcpc_device *tcpc_dev);
static inline int typec_set_polarity(struct hi6526_tcpc_device *tcpc_dev,
		bool polarity)
{
...
static int typec_set_plug_orient(struct hi6526_tcpc_device *tcpc_dev,
 ...
 */
#ifdef CONFIG_TYPEC_CAP_NORP_SRC
static void typec_clear_norp_state(struct hi6526_tcpc_device *tcpc_dev)
{
 ...
 * Handle state change.
 */
static int typec_alert_attach_state_change(struct hi6526_tcpc_device *tcpc_dev)
{
	int ret;
	...
	ret = hi6526_tcpci_report_usb_port_changed(tcpc_dev);
...
#ifdef CONFIG_TYPEC_CAP_NORP_SRC
static bool typec_try_enter_norp_src(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	if (tcpci_check_vbus_valid(tcpc_dev)) {
		hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_TIMER_NORP_SRC);
		return true;
...
static bool typec_try_exit_norp_src(struct hi6526_tcpc_device *tcpc_dev)
{
	if (!typec_is_cc_ra() && !typec_is_cc_no_res()) {
		hi6526_tcpc_disable_timer(tcpc_dev, TYPEC_TIMER_NORP_SRC);
		return true;
	}
...
static bool typec_norp_src_detached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;

	if (tcpc_dev->no_rpsrc_state == 1) {
		tcpc_dev->no_rpsrc_state = 0;
		tcpc_dev->typec_attach_new = HI6526_TYPEC_DETTACHED_VBUS_ONLY;
...
static inline int typec_norp_src_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	if (tcpc_dev->no_rpsrc_state == 0) {
		tcpc_dev->no_rpsrc_state = 1;
		tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_VBUS_ONLY;
		tcpci_notify_typec_state(tcpc_dev);
	}
...
static void typec_unattached_power_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	if (tcpc_dev->typec_power_ctrl) {
		...
		hi6526_tcpci_report_power_control(tcpc_dev, false);
	}
}

static inline void typec_unattached_cc_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("+\n");
	...
	default:
		...
		case typec_audioaccessory:
			...
			hi6526_tcpc_enable_timer(tcpc_dev,
		...
		default:
			...
			hi6526_tcpc_poll_cc_status(tcpc_dev);
			break;
		}
		break;
	}
...
static void typec_unattached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
 ...
 */
static void typec_unattach_wait_pe_idle_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	tcpc_dev->typec_attach_new = HI6526_TYPEC_UNATTACHED;
 ...
 */
static void typec_source_attached_with_vbus_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_SRC;
	typec_wait_ps_change(tcpc_dev, TYPEC_WAIT_PS_DISABLE);
}

static void typec_source_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	 ...
	 */
	hi6526_tcpc_disable_timer(tcpc_dev, TYPEC_TRY_TIMER_DRP_TRY);

#ifdef CONFIG_TYPEC_CAP_ROLE_SWAP
	if (tcpc_dev->typec_during_role_swap) {
		tcpc_dev->typec_during_role_swap = TYPEC_ROLE_SWAP_NONE;
		hi6526_tcpc_disable_timer(tcpc_dev,
	...
	hi6526_tcpci_report_power_control(tcpc_dev, true);
...
static void typec_sink_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	TYPEC_NEW_STATE(typec_attached_snk);
	typec_wait_ps_change(tcpc_dev, TYPEC_WAIT_PS_DISABLE);

	tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_SNK;

#ifdef CONFIG_TYPEC_CAP_TRY_STATE
	if (tcpc_dev->typec_role >= TYPEC_ROLE_DRP)
		hi6526_tcpc_reset_typec_try_timer(tcpc_dev);
#endif
...
static void typec_custom_src_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
#ifdef CONFIG_TYPEC_CAP_CUSTOM_SRC_SUPPORT
	...
	if (cc1 == TYPEC_CC_VOLT_SNK_DFT && cc2 == TYPEC_CC_VOLT_SNK_DFT) {
		...
		tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_CUSTOM_SRC;
		return;
	}
#endif
...
static void typec_sink_dbg_acc_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	bool polarity = false;
	uint8_t rp_level;
	int cc1 = typec_get_cc1();
	...
	tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_DBGACC_SNK;
...
#ifdef CONFIG_TYPEC_CAP_TRY_SINK
static bool typec_role_is_try_sink(struct hi6526_tcpc_device *tcpc_dev)
{
...
static void typec_try_snk_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
	...
	tcpci_set_cc(tcpc_dev, TYPEC_CC_RD);
	hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_TRY_TIMER_DRP_TRY);
}

static void typec_trywait_src_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
...
static void typec_cc_snk_detect_vsafe5v_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
 ...
 */
static inline void typec_cc_snk_detect_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static void typec_cc_src_detect_vsafe0v_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
 ...
 */
static void typec_cc_src_detect_entry(struct hi6526_tcpc_device *tcpc_dev)
{
...
static void typec_cc_src_remove_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static void typec_cc_snk_remove_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static bool typec_debug_acc_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	TYPEC_NEW_STATE(typec_debugaccessory);
	TYPEC_DBG("[Debug] CC1&2 Both Rd\n");
	tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_DEBUG;
	return true;
}

static bool typec_audio_acc_attached_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");

	TYPEC_NEW_STATE(typec_audioaccessory);
	TYPEC_INFO("[Audio] CC1&2 Both Ra\n");
	tcpc_dev->typec_attach_new = HI6526_TYPEC_ATTACHED_AUDIO;

	return true;
}

static bool typec_cc_change_source_entry(struct hi6526_tcpc_device *tcpc_dev)
{
...
static bool typec_attached_snk_cc_change(struct hi6526_tcpc_device *tcpc_dev)
{
...
static bool typec_cc_change_sink_entry(struct hi6526_tcpc_device *tcpc_dev)
{
...
static bool typec_is_act_as_sink_role(struct hi6526_tcpc_device *tcpc_dev)
{
	bool as_sink = true;
	uint8_t cc_sum;
...
static bool typec_handle_cc_changed_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	hi6526_usb_typec_cc_status_change(typec_get_cc1(), typec_get_cc2());
...
static void typec_attach_wait_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	bool as_sink = false;
	...
	D("\n");
	if (tcpc_dev->typec_attach_old == HI6526_TYPEC_ATTACHED_SNK && !pd_en) {
		hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_TIMER_PDDEBOUNCE);
		D("RpLvl Alert\n");
		return;
	}

	if ((tcpc_dev->typec_attach_old != HI6526_TYPEC_UNATTACHED)
			|| (tcpc_dev->typec_state == typec_attached_src)) {
		hi6526_tcpc_reset_typec_debounce_timer(tcpc_dev);
	...
	case typec_unattachwait_pe:
		...
		hi6526_tcpc_disable_timer(tcpc_dev, TYPEC_RT_TIMER_PE_IDLE);
		typec_unattached_power_entry(tcpc_dev);
		break;
#endif
	...
	hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_TIMER_CCDEBOUNCE);
}

#ifdef TYPEC_EXIT_ATTACHED_SNK_VIA_VBUS
static int typec_attached_snk_cc_detach(struct hi6526_tcpc_device *tcpc_dev)
{
...
static void typec_detach_wait_entry(struct hi6526_tcpc_device *tcpc_dev)
{
	switch (tcpc_dev->typec_state) {
#ifdef TYPEC_EXIT_ATTACHED_SNK_VIA_VBUS
	case typec_attached_snk:
	...
	case typec_attached_src:
		...
		hi6526_tcpc_tcpci_init(tcpc_dev, true);
...
static bool typec_is_cc_attach(struct hi6526_tcpc_device *tcpc_dev)
{
...
int hi6526_tcpc_typec_handle_cc_change(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	hi6526_usb_typec_cc_alert(typec_get_cc1(), typec_get_cc2());
...
#ifdef CONFIG_TYPEC_CAP_TRY_STATE
static inline int typec_handle_drp_try_timeout(struct hi6526_tcpc_device *tcpc_dev)
{
...
static int typec_handle_debounce_timeout(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
static int typec_handle_error_recovery_timeout(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static int typec_handle_pe_idle(struct hi6526_tcpc_device *tcpc_dev)
{
	switch (tcpc_dev->typec_state) {
	case typec_unattachwait_pe:
		typec_unattached_entry(tcpc_dev);
...
static int typec_handle_src_reach_vsafe0v(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static int typec_handle_src_toggle_timeout(struct hi6526_tcpc_device *tcpc_dev)
{
	D("\n");
...
static int hi6526_tcpc_typec_handle_vsafe0v(struct hi6526_tcpc_device *tcpc_dev)
{
	if (tcpc_dev->typec_wait_ps_change == TYPEC_WAIT_PS_SRC_VSAFE0V) {
#ifdef CONFIG_TYPEC_ATTACHED_SRC_SAFE0V_DELAY
		hi6526_tcpc_enable_timer(tcpc_dev, TYPEC_RT_TIMER_SAFE0V_DELAY);
#else
...
static int typec_handle_safe0v_tout(struct hi6526_tcpc_device *tcpc_dev)
{
	int ret;
	...
	if (!tcpci_check_vbus_valid(tcpc_dev))
		return  hi6526_tcpc_typec_handle_vsafe0v(tcpc_dev);
	...
	hi6526_tcpci_vbus_level_init(tcpc_dev, power_status);

	TCPC_INFO("Safe0V TOUT: ps=%u\n", tcpc_dev->vbus_level);

	if (!tcpci_check_vbus_valid(tcpc_dev))
		ret = hi6526_tcpc_typec_handle_vsafe0v(tcpc_dev);
	else
...
static int tcpc_typec_handle_timeout(struct hi6526_tcpc_device *tcpc_dev,
		uint32_t timer_id)
{
	int ret = 0;
		...
		uint32_t timer_id)
{
	...
	if (timer_id >= TYPEC_TIMER_START_ID)
		...
		hi6526_tcpc_disable_timer(tcpc_dev, timer_id);
...
static int typec_handle_vbus_present(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
...
static int typec_attached_snk_vbus_absent(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
...
static inline int typec_handle_vbus_absent(struct hi6526_tcpc_device *tcpc_dev)
{
	I("\n");
	...
	hi6526_tcpc_typec_handle_vsafe0v(tcpc_dev);
#endif
		...
		uint8_t typec_role)
{
	...
	if (tcpc_dev->typec_attach_new != HI6526_TYPEC_UNATTACHED) {
		force_unattach = true;
	} else {
		switch (tcpc_dev->typec_role) {
		case TYPEC_ROLE_SNK:
...
static int typec_init_power_off_charge(struct hi6526_tcpc_device *tcpc_dev)
{
	int ret;
	bool cc_open = false;
...
int hi6526_tcpc_typec_init(struct hi6526_tcpc_device *tcpc_dev, uint8_t typec_role)
{
	int ret = 0;
	...
	tcpc_dev->typec_role = typec_role;
	tcpc_dev->typec_attach_new = HI6526_TYPEC_UNATTACHED;
	tcpc_dev->typec_attach_old = HI6526_TYPEC_UNATTACHED;
