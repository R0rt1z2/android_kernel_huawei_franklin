...
static void hi6526_dp_dfp_u_set_state(pd_port_t *pd_port, uint8_t state)
{
...
bool hi6526_dp_dfp_u_notify_pe_startup(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data)
{
	D("\n");
	if (pd_port->dpm_flags & DPM_FLAGS_CHECK_DP_MODE) {
		hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_DISCOVER_ID);
...
bool hi6526_dp_notify_pe_shutdown(
		...
		svdm_svid_data_t *svid_data, pd_event_t *pd_event, bool ack)
{
	...
	if (!ack) {
		hi6526_dp_dfp_u_set_state(pd_port,
				DP_DFP_U_ERR_DISCOVER_ID_NAK_TIMEOUT);
		return true;
	}

	if (pd_msg->payload[VDO_INDEX_IDH] & PD_IDH_MODAL_SUPPORT)
		hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_DISCOVER_SVIDS);
	else
		hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_ERR_DISCOVER_ID_TYPE);

	return true;
}

bool hi6526_dp_dfp_u_notify_discover_svid(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data, bool ack)
{
	...
	hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_DISCOVER_MODES);
	return true;
}
		...
		svdm_svid_data_t *svid_data)
{
	...
	for (j = 0; j < local->mode_cnt; j++) {
		uint32_t dp_local_mode = local->mode_vdo[j];

		if (!is_dp_v1_cap_valid(dp_local_mode))
			continue;

		for (i = 0; i < remote->mode_cnt; i++) {
			uint32_t dp_remote_mode = remote->mode_vdo[i];
...
bool hi6526_dp_dfp_u_notify_discover_modes(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data, bool ack)
{
	if (pd_port->dp_dfp_u_state != DP_DFP_U_DISCOVER_MODES)
		return false;
	...
	hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_ENTER_MODE);
		...
		svdm_svid_data_t *svid_data, uint8_t ops, bool ack)
{
	...
	hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_STATUS_UPDATE);

	pd_port->dp_status = pd_port->dp_first_connected;
	return dp_dfp_u_status_update(pd_port);
}

bool hi6526_dp_dfp_u_notify_exit_mode(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data, uint8_t ops)
{
	...
	hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_NONE);
	return true;
}
...
void hi6526_dp_dfp_u_request_dp_configuration(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	...
	hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_CONFIGURE);
		...
		uint32_t dp_status)
{
	...
	case DPSTS_DISCONNECT:
		hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_WAIT_ATTENTION);
		break;
...
bool hi6526_dp_dfp_u_notify_dp_status_update(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	...
	if (oper_mode) {
		...
		valid_connected = dp_dfp_u_update_dp_connected(
				pd_port, dp_status);
		if (valid_connected)
			hi6526_dp_dfp_u_request_dp_configuration(
					pd_port, pd_event);
	}
	return true;
}

bool hi6526_dp_dfp_u_notify_dp_configuration(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	uint8_t dp_status;
	...
	if (ack)
		hi6526_dp_dfp_u_set_state(pd_port, DP_DFP_U_OPERATION);
	else
...
void hi6526_dp_ufp_u_request_enter_mode(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data, uint8_t ops)
{
	pd_port->dp_status = pd_port->dp_first_connected;
...
void hi6526_dp_ufp_u_request_exit_mode(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data, uint8_t ops)
{
	pd_port->dp_status = 0;
...
int hi6526_dp_ufp_u_request_dp_status(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ack = true;
	uint32_t dp_status;
	...
	if (ack)
		return hi6526_pd_reply_svdm_request(pd_port,
				pd_event, CMDT_RSP_ACK, 1, &pd_port->dp_status);
	else
		return hi6526_pd_reply_svdm_request_simply(
				pd_port, pd_event, CMDT_RSP_NAK);
}

bool hi6526_dp_ufp_u_is_valid_dp_config(pd_port_t *pd_port, uint32_t dp_config)
{
	uint32_t sel_config;
	bool retval = false;
	uint32_t local_pin;
...
int hi6526_dp_ufp_u_request_dp_config(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ack = false;
	uint32_t dp_config;
	...
	case DP_UFP_U_WAIT:
	case DP_UFP_U_OPERATION:
		ack = hi6526_dp_ufp_u_is_valid_dp_config(pd_port, dp_config);
		if (ack) {
...
void hi6526_dp_ufp_u_send_dp_attention(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
{
	return hi6526_dp_ufp_u_request_dp_status(pd_port, pd_event);
}

int hi6526_pd_dpm_ufp_request_dp_config(pd_port_t *pd_port, pd_event_t *pd_event)
{
	return hi6526_dp_ufp_u_request_dp_config(pd_port, pd_event);
}

void hi6526_pd_dpm_ufp_send_dp_attention(pd_port_t *pd_port, pd_event_t *pd_event)
{
	hi6526_dp_ufp_u_send_dp_attention(pd_port, pd_event);
}

/* ---- DFP : DP Only ---- */

#ifdef CONFIG_USB_PD_ALT_MODE_DFP_SUPPORT
void hi6526_pd_dpm_dfp_send_dp_status_update(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
		...
		pd_port_t *pd_port, pd_event_t *pd_event, bool ack)
{
	...
	hi6526_dp_dfp_u_notify_dp_status_update(pd_port, pd_event, ack);
}

void hi6526_pd_dpm_dfp_send_dp_configuration(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
...
void hi6526_pd_dpm_dfp_inform_dp_configuration(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	pd_port->dpm_ack_immediately = true;
	hi6526_dp_dfp_u_notify_dp_configuration(pd_port, pd_event, ack);
}
#endif
