...
{
	struct hi6526_tcpc_device *tcpc = dev_get_drvdata(dual_role->dev.parent);
	int ret = 0;
...
static int tcpc_dual_role_pr_swap(struct hi6526_tcpc_device *tcpc, uint8_t new_role)
{
	...
	return hi6526_tcpm_power_role_swap(tcpc);
}

static int tcpc_dual_role_dr_swap(struct hi6526_tcpc_device *tcpc, uint8_t new_role)
{
	...
	return hi6526_tcpm_data_role_swap(tcpc);
}

static int tcpc_dual_role_vconn_swap(struct hi6526_tcpc_device *tcpc, uint8_t new_role)
{
	...
	return hi6526_tcpm_vconn_swap(tcpc);
}
...
#else
	case DUAL_ROLE_PROP_PR:
		ret = hi6526_tcpm_typec_role_swap(tcpc);
		if (ret)
			E("hi6526_tcpm_typec_role_swap failed\n");
		break;
...
static void tcpc_get_dual_desc(struct hi6526_tcpc_device *tcpc)
{
	...
	if (of_property_read_u32(np, "tcpc,supported_modes", &val) >= 0) {
...
int hi6526_tcpc_dual_role_phy_init(struct hi6526_tcpc_device *tcpc)
{
	...
	len = strlen(tcpc->desc.name) + strlen("dual-role-") + 1;
	dual_desc_name = devm_kzalloc(&tcpc->dev, len, GFP_KERNEL);
	if (!dual_desc_name) {
		I("dual_desc_name devm_kzalloc fail!\n");
		dual_desc->name = "dual-role-type_c_port0";
	} else {
		snprintf(dual_desc_name, len, "dual-role-%s", tcpc->desc.name);
		dual_desc->name = dual_desc_name;
	}
	D("dual_desc_name: %s\n", dual_desc->name);
