...
struct tcpc_ops {
	int (*init)(struct hi6526_tcpc_device *tcpc, bool sw_reset);
	int (*deinit)(struct hi6526_tcpc_device *tcpc);
	int (*alert_status_clear)(struct hi6526_tcpc_device *tcpc, uint32_t mask);
	int (*fault_status_clear)(struct hi6526_tcpc_device *tcpc, uint8_t status);
	int (*get_alert_status)(struct hi6526_tcpc_device *tcpc, uint32_t *alert);
	int (*get_power_status)(struct hi6526_tcpc_device *tcpc, uint16_t *pwr_status);
	int (*get_fault_status)(struct hi6526_tcpc_device *tcpc, uint8_t *status);
	int (*get_cc)(struct hi6526_tcpc_device *tcpc, int *cc1, int *cc2);
	int (*set_cc)(struct hi6526_tcpc_device *tcpc, int pull);
	int (*set_polarity_cc)(struct hi6526_tcpc_device *tcpc);
	int (*set_polarity)(struct hi6526_tcpc_device *tcpc, int polarity);
	int (*set_vconn)(struct hi6526_tcpc_device *tcpc, int enable);
	int (*source_vbus)(struct hi6526_tcpc_device *tcpc, uint8_t type,
			int mv, int ma);
	int (*sink_vbus)(struct hi6526_tcpc_device *tcpc, uint8_t type,
			int mv, int ma);

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	int (*set_msg_header)(struct hi6526_tcpc_device *tcpc,
			int power_role, int data_role, uint8_t rev);
	int (*set_rx_enable)(struct hi6526_tcpc_device *tcpc, uint8_t enable);
	int (*check_rx_busy) (struct hi6526_tcpc_device *tcpc);
	int (*get_message)(struct hi6526_tcpc_device *tcpc, uint32_t *payload,
			uint16_t *head, enum tcpm_transmit_type *type);
	int (*set_bist_test_mode)(struct hi6526_tcpc_device *tcpc, bool en);

	int (*transmit)(struct hi6526_tcpc_device *tcpc,
			enum tcpm_transmit_type type,
			uint16_t header, const uint32_t *data);
#ifdef CONFIG_USB_PD_RETRY_CRC_DISCARD
	int (*retransmit)(struct hi6526_tcpc_device *tcpc);
#endif
#endif /* CONFIG_USB_POWER_DELIVERY_SUPPORT */

	void (*set_vbus_detect)(struct hi6526_tcpc_device *tcpc, bool enable);
	void (*reset_pd_fsm)(struct hi6526_tcpc_device *tcpc);
	void (*tcpc_pd_fsm_state)(struct hi6526_tcpc_device *tcpc);
	int (*tcpc_cc_is_realdetach)(struct hi6526_tcpc_device *tcpc);
	void (*set_fr_swap)(struct hi6526_tcpc_device *tcpc, bool enable);
	void (*da_vbus_en_status)(struct hi6526_tcpc_device *tcpc);
	void (*sc_buck_ctrl)(struct hi6526_tcpc_device *tcpc, bool enable);
	bool (*vusb_uv_det_mask)(void);
	bool (*chip_version_v610)(struct hi6526_tcpc_device *tcpc);
	void (*snk_unattach_by_vbus_bypass)(struct hi6526_tcpc_device *tcpc_dev,
			bool en);
};
...
static inline void *hi6526_tcpc_get_dev_data(struct hi6526_tcpc_device *tcpc)
{
	return tcpc->drv_data;
}

static inline void hi6526_tcpci_lock_typec(struct hi6526_tcpc_device *tcpc)
{
	mutex_lock(&tcpc->typec_lock);
}

static inline void hi6526_tcpci_unlock_typec(struct hi6526_tcpc_device *tcpc)
{
