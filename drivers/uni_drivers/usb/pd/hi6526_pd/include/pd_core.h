/*
 * Copyright (C) 2018 Huawei Technology Corp.
 *
...
#ifdef CONFIG_HI6526_TCPC_QUIRK_V100
...
void hi6526_pd_extract_rdo_power(
...
{
	hi6526_tcpc_restart_timer(pd_port->tcpc_dev, timer_id);
}

static inline void pd_enable_timer(pd_port_t *pd_port, uint32_t timer_id)
{
	hi6526_tcpc_enable_timer(pd_port->tcpc_dev, timer_id);
}

static inline void pd_disable_timer(pd_port_t *pd_port, uint32_t timer_id)
{
	hi6526_tcpc_disable_timer(pd_port->tcpc_dev, timer_id);
}

static inline void pd_reset_pe_timer(pd_port_t *pd_port)
{
	hi6526_tcpc_reset_pe_timer(pd_port->tcpc_dev);
}
...
{
	hi6526_pd_free_event(pd_port->tcpc_dev, pd_event);
}
...
bool vdm_put_hw_event(struct hi6526_tcpc_device *tcpc_dev, uint8_t hw_event);
...
int hi6526_pd_send_ctrl_msg(pd_port_t *pd_port,
...
static inline void hi6526_pd_cable_flag_clear(pd_port_t *pd_port)
{
...
static inline void hi6526_pd_cable_flag_set(pd_port_t *pd_port)
{
#ifdef CONFIG_USB_PD_RESET_CABLE
...
{
	return hi6526_pd_send_svdm_request(pd_port, sop_type, USB_SID_PD,
...
{
	return hi6526_pd_send_svdm_request(pd_port, sop_type, svid,
...
{
	return hi6526_pd_send_svdm_request(pd_port, sop_type, USB_SID_DISPLAYPORT,
...
static inline int hi6526_pd_reply_svdm_request_simply(pd_port_t *pd_port,
		pd_event_t *pd_event, uint8_t reply)
{
	return hi6526_pd_reply_svdm_request(pd_port, pd_event, reply, 0, NULL);
}

#ifdef CONFIG_USB_PD_UVDM_SUPPORT
static inline int pd_send_uvdm(pd_port_t *pd_port, uint8_t sop_type)
{
	return hi6526_pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF,
...
static inline void hi6526_dfp_uvdm_receive_data(pd_event_t *pd_event)
{
