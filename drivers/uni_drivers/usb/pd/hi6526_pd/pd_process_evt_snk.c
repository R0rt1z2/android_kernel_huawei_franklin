...
static bool hi6526_pd_process_ctrl_msg_swap(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	bool ret = false;
	...
	case PD_CTRL_DR_SWAP:
		ret = hi6526_pd_process_ctrl_msg_dr_swap(pd_port, pd_event);
		break;

	case PD_CTRL_PR_SWAP:
		ret = hi6526_pd_process_ctrl_msg_pr_swap(pd_port, pd_event);
		break;

	case PD_CTRL_VCONN_SWAP:
		ret = hi6526_pd_process_ctrl_msg_vconn_swap(pd_port, pd_event);
		break;
...
static bool pd_process_ctrl_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_CTRL_PING:
		hi6526_pd_notify_pe_recv_ping_event(pd_port);
		break;
	...
	case PD_CTRL_REJECT:
		hi6526_pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_REJECT);
		ret = pd_process_ctrl_msg_reject(pd_port, pd_event);
		break;

	case PD_CTRL_WAIT:
		hi6526_pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_WAIT);
	...
	case PD_CTRL_PR_SWAP:
	case PD_CTRL_VCONN_SWAP:
		ret = hi6526_pd_process_ctrl_msg_swap(pd_port, pd_event);
		break;
	...
	if (!ret)
		ret = hi6526_pd_process_protocol_error(pd_port, pd_event);
...
static bool pd_process_data_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_DATA_BIST:
		ret = hi6526_pd_process_data_msg_bist(pd_port, pd_event);
		break;
...
static bool pd_process_hw_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_HW_RECV_HARD_RESET:
		ret = hi6526_pd_process_recv_hard_reset(pd_port, pd_event,
...
static bool pd_process_timer_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_TIMER_SENDER_RESPONSE:
		hi6526_pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_TIMEOUT);
	...
	case PD_TIMER_DFP_FLOW_DELAY:
		if (pd_port->pe_state_curr == PE_SNK_READY)
			hi6526_pd_dpm_notify_dfp_delay_done(pd_port, pd_event);
		break;
	...
	case PD_TIMER_UFP_FLOW_DELAY:
		if (pd_port->pe_state_curr == PE_SNK_READY)
			hi6526_pd_dpm_notify_ufp_delay_done(pd_port, pd_event);
		break;
...
bool hi6526_pd_process_event_snk(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
