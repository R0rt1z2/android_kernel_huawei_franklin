...
static int huawei_hkip_dispatch(void)
{
...
#ifdef CONFIG_HKIP_USING_IPI
static struct work_struct g_hkip_ipi_work;
struct workqueue_struct *g_hhee_ipi_wq;

static void huawei_hkip_ipi_fn(struct work_struct *work)
{
	if (huawei_hkip_dispatch())
		pr_err("%s, msg handle error\n", __func__);

	(void)work;
}

void hkip_irq_receive(void)
{
	queue_work(g_hhee_ipi_wq, &g_hkip_ipi_work);
}
#else
...
int hhee_msg_init(void)
{
	...
	if (!buf_base || buf_size != (HHEE_MSG_MAX_SLOTS + 1) * HHEE_MSG_SIZE) {
		pr_err("hkip get irq info hvc failed\n");
		ret = -EFAULT;
		goto err;
	}
	...
	if (!addr) {
		pr_err("hkip get virt addr failed\n");
		ret = -EFAULT;
		goto err;
	}
#ifdef CONFIG_HKIP_USING_IPI
	g_hhee_ipi_wq = create_singlethread_workqueue("g_hkip_ipi_work");
	if (!g_hhee_ipi_wq) {
		pr_err("Create HKIP IPI work failed\n");
		ret = -1;
		goto err;
	}
	INIT_WORK(&g_hkip_ipi_work, huawei_hkip_ipi_fn);
#endif
	msg_state = addr;
	return 0;
