	           ...
	           struct circular_buffer **incb, unsigned int logtype)
{
	...
	if (inlog_addr == 0 || inlog_size == 0) {
		pr_err("fail to get log addr and size of hhee\n");
		return -EINVAL;
	}
...
int hhee_logger_init(void)
{
	...
	ret = cb_init(ret_res.a0, ret_res.a1, &pmf_cb, PMF_LOG);
	if (ret)
		goto outfail;
