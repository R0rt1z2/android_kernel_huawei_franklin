 ...
 * ion_seccg_heap.c
 *
 * Copyright(C) 2004-2019 Huawei Technologies Co., Ltd. All rights reserved.
 *
...
static struct cma *seccg_cma;

#define ION_HEAP_TYPE_SEC_CONTIG 23

static struct cma *get_seccg_cma(void)
{
	return seccg_cma;
}

void set_seccg_cma(struct cma *cma)
{
	seccg_cma = cma;
}
						...
						struct ion_platform_heap *heap_data)
{
	...
	seccg_heap->heap.type = (int)ION_HEAP_TYPE_SEC_CONTIG;
	seccg_heap->heap_size = heap_data->size;
	seccg_heap->alloc_size = 0;
	seccg_heap->cma_alloc_size = 0;

	seccg_heap->cma_name = "dynamic-cma-pool";
	seccg_heap->per_alloc_sz = 0x2000000;
	seccg_heap->per_bit_sz = 0x200000;
	seccg_heap->water_mark = 0;
	seccg_heap->pool_shift = 21;
	seccg_heap->heap_attr = HEAP_NORMAL;
	seccg_heap->pre_alloc_attr = 0;
