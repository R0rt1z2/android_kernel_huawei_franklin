...
static unsigned int code_freq(unsigned int display_freq)
{
	switch (display_freq) {
	case 3200:
		return 3068;
...
#ifdef CONFIG_HUAWEI_DRAM_FREQ_CONTROL
unsigned int mtk_dramc_get_real_data_rate(void)
{
	struct dramc_dev_t *dramc_dev_ptr = NULL;
	struct fmeter_dev_t *fmeter_dev_ptr = NULL;
	unsigned int freq;
	...
	/* fmeter version 1 */
	if (fmeter_dev_ptr->version == 1) {
		freq = fmeter_v1(dramc_dev_ptr);
		return code_freq(freq);
	}

	return 0;
}
EXPORT_SYMBOL(mtk_dramc_get_real_data_rate);
#endif
