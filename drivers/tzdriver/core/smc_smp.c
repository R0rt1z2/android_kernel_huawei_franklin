...
void show_single_cmd_info(int *cmd, uint32_t len)
{
	uint32_t idx;
...
void show_cmd_bitmap(void)
{
	...
	if (memset_s(cmd_in, sizeof(int)* MAX_SMC_CMD, -1, sizeof(int)* MAX_SMC_CMD) ||
		memset_s(cmd_out, sizeof(int)* MAX_SMC_CMD, -1, sizeof(int)* MAX_SMC_CMD)) {
		tloge("memset failed\n");
		goto error;
	}
	...
	uint32_t cmd_type, uint8_t wait)
{
	...
	register uint32_t r1 asm("r1") = cmd_addr;
	register uint32_t r2 asm("r2") = cmd_type;
	register uint32_t r3 asm("r3") = (uint64_t)cmd_addr >> ADDR_TRANS_NUM;
