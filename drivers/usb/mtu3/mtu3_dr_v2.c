...
static void set_deafult_para(int mode, u32 *vrt, u32 *term, u32 *enhance)
{
	if (mode == DUAL_PROP_DEVICE) {
		*vrt = DEFAULT_DEV_VRT_REF;
		*term = DEFAULT_DEV_TERM_REF;
		*enhance = DEFAULT_DEV_ENHANCE;
	} else if (mode == DUAL_PROP_HOST) {
		*vrt = DEFAULT_HOST_VRT_REF;
		*term = DEFAULT_HOST_TERM_REF;
		*enhance = DEFAULT_HOST_ENHANCE;
	}
	mtu3_printk(K_CRIT, "%s default para: mode:%d, vrf:%d, term:%d, enhance:%d\n",
		__func__, mode, *vrt, *term, *enhance);
}

void get_phy_eye_para(int mode, u32 *vrt, u32 *term, u32 *enhance)
{
	struct device_node *node = NULL;

	node = of_find_compatible_node(NULL, NULL, "mediatek,phy_eye");
	if (!node) {
		set_deafult_para(mode, vrt, term, enhance);
		return;
	}

	if (mode == DUAL_PROP_DEVICE) {
		if (of_property_read_u32(node, "d_u2_vrt_ref", vrt) ||
			of_property_read_u32(node, "d_u2_term_ref", term) ||
			of_property_read_u32(node, "d_u2_enhance", enhance)) {
			pr_err("%s get dev phy para dts failed\n", __func__);
			*vrt = DEFAULT_DEV_VRT_REF;
			*term = DEFAULT_DEV_TERM_REF;
			*enhance = DEFAULT_DEV_ENHANCE;
		}
	} else if (mode == DUAL_PROP_HOST) {
		if (of_property_read_u32(node, "h_u2_vrt_ref", vrt) ||
			of_property_read_u32(node, "h_u2_term_ref", term) ||
			of_property_read_u32(node, "h_u2_enhance", enhance)) {
			pr_err("%s get host phy para dts failed\n", __func__);
			*vrt = DEFAULT_HOST_VRT_REF;
			*term = DEFAULT_HOST_TERM_REF;
			*enhance = DEFAULT_HOST_ENHANCE;
		}
	...
	mtu3_printk(K_CRIT, "%s mode:%d, vrf:%d, term:%d, enhance:%d\n",
		__func__, mode, *vrt, *term, *enhance);
}
...
static void ssusb_set_mode(struct work_struct *work)
{
	...
	u32 u2_vrt_ref = INVALID_PHY_PARA;
	u32 u2_term_ref = INVALID_PHY_PARA;
	u32 u2_enhance = INVALID_PHY_PARA;

	spin_lock_irqsave(&otg_sx->dr_lock, flags);
	usb_mode = __otg_sx->desire_usb_mode;
	spin_unlock_irqrestore(&otg_sx->dr_lock, flags);
	get_phy_eye_para(usb_mode, &u2_vrt_ref, &u2_term_ref, &u2_enhance);
	if (otg_sx->usb_mode != usb_mode) {
		...
		case DUAL_PROP_HOST:
			...
			phy_eye_diagram_write(u2_vrt_ref, u2_term_ref, u2_enhance);
			break;
