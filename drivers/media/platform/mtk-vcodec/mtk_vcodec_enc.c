...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_ENCODE_RC_FRAME_LEVEL_QP:
		...
		break;
	case V4L2_CID_MPEG_MTK_ENCODE_LOW_LATENCY:
		mtk_v4l2_debug(2,
			"V4L2_CID_MPEG_MTK_ENCODE_LOW_LATENCY: %d",
			ctrl->val);
		p->lowlatencymode = ctrl->val;
		ctx->param_change |= MTK_ENCODE_PARAM_ENCODE_LOW_LATENCY;
		break;
	case V4L2_CID_MPEG_MTK_ENCODE_LTR_MODE:
		mtk_v4l2_debug(2,
			"V4L2_CID_MPEG_MTK_ENCODE_LTR_MODE: ltr %d str %d",
			(ctrl->val >> 16), ctrl->val & 0xffff);
		p->ltr_mode = ctrl->val;
		ctx->param_change |= MTK_ENCODE_PARAM_LTR_MODE;
		break;
	default:
			       ...
			       struct venc_enc_param *param)
{
	...
	param->ltr_mode = enc_params->ltr_mode;
			    ...
			    struct v4l2_buffer *buf)
{
	...
	if (buf->flags & V4L2_BUF_FLAG_QP_META && buf->reserved2 != 0) {
		mtk_v4l2_debug(1, "[%d] Have QP info meta fd, buf->index:%d. mtkbuf:%p, pa:0x%x",
			ctx->id, buf->index, mtkbuf, buf->reserved2);
		mtkbuf->frm_buf.qpmap = buf->reserved2;
	}
	if (buf->flags & V4L2_BUF_FLAG_LTR_MARK_USE && buf->reserved != 0) {
		mtk_v4l2_debug(1, "[%d] Use ltr, buf->index:%d, used:0x%x",
			ctx->id, buf->index, buf->reserved);
		// 31: mark, 30: use, 29-0: use index
		mtkbuf->frm_buf.ltr_use_index = buf->reserved;
	} else {
		mtkbuf->frm_buf.ltr_use_index = 0;
	}
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	mtk_v4l2_debug(0,
	"fmt 0x%x, P/L %d/%d, w/h %d/%d, buf %d/%d, fps/bps %d/%d(%d), gop %d, ip# %d opr %d async %d grid size %d/%d b#%d, slbc %d, l/str %d/%d",
	...
	param.ltr_mode>>16, param.ltr_mode&0xffff);
...
static int mtk_venc_param_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if (!ret &&
	mtk_buf->param_change & MTK_ENCODE_PARAM_LTR_MODE) {
		enc_prm.ltr_mode = mtk_buf->enc_params.ltr_mode;
		mtk_v4l2_debug(1, "[%d] idx=%d, ltr_mode=%d",
				ctx->id,
				mtk_buf->vb.vb2_buf.index,
				mtk_buf->enc_params.ltr_mode);
		ret |= venc_if_set_param(ctx,
					VENC_SET_PARAM_LTR_MODE,
					&enc_prm);
	}
...
static void mtk_venc_worker(struct work_struct *work)
{
	...
	mtk_v4l2_debug(2,
		"mtkbuf %p, frmbuf %p, ltr use idx %x",
		src_buf_info, pfrm_buf, pfrm_buf->ltr_use_index);
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	memset(&cfg, 0, sizeof(cfg));
	cfg.id = V4L2_CID_MPEG_MTK_ENCODE_LTR_MODE;
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_WRITE_ONLY;
	cfg.name = "Video encode ltr mode (ltr, str)";
	cfg.min = 0;
	cfg.max = 0x000fffff;
	cfg.step = 1;
	cfg.def = 0;
	...
	memset(&cfg, 0, sizeof(cfg));
	cfg.id = V4L2_CID_MPEG_MTK_ENCODE_LOW_LATENCY;
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_WRITE_ONLY;
	cfg.name = "Video Encoder Low latency";
	cfg.min = 0;
	cfg.max = 1;
	cfg.step = 1;
	cfg.def = 0;
