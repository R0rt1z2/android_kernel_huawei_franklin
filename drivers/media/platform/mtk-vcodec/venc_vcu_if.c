					  ...
					  struct venc_enc_param *enc_param)
{
	...
	case VENC_SET_PARAM_ADJUST_FRAME_LEVEL_QP:
		...
		break;
	case VENC_SET_PARAM_LTR_MODE:
		out.data_item = 1;
		out.data[0] = enc_param->ltr_mode;
		break;
	default:
				   ...
				   unsigned int *bs_size)
{
	...
	if (frm_buf) {
		...
		out.ltr_use_index = frm_buf->ltr_use_index;

		mtk_vcodec_debug(vcu, "num_planes = %d input (dmabuf:%lx fd:%x), mtfd %x sz %d %x",
			frm_buf->num_planes,
			(unsigned long)frm_buf->fb_addr[0].dmabuf,
			out.input_fd[0],
			out.meta_fd, out.meta_size, out.ltr_use_index);
	}
