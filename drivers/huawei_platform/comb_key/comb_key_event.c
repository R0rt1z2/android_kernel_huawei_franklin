...
static unsigned int comb_key_status;
...
{
	static unsigned int last_comb_key_status = COMB_KEY_PRESS_RELEASE;

	if ((power_key_status == POWER_KEY_PRESS_DOWN) &&
			(volume_key_status == VOLUME_KEY_DOWN_PRESS)) {
...
{
	(void)bar;
	if ((foo == POWER_KEY_PRESS_DOWN) ||
			(foo == POWER_KEY_PRESS_RELEASE)) {
		...
		unsigned long foo, void *bar)
{
	...
	if ((foo == VOLUME_KEY_DOWN_PRESS) ||
			(foo == VOLUME_KEY_DOWN_RELEASE)) {
...
static ssize_t show_comb_key_status(struct device *dev,
		struct device_attribute *arr, char *buf)
{
	if (!buf)
		return 0;
	pr_info("%s = %d\n", __func__, comb_key_status);
...
{
	volume_key_unregister_notifier(&volume_key_notify);
	power_key_unregister_notifier(&power_key_notify);
	platform_device_unregister(&comb_key_info);
	pr_info("exit\n");
}
