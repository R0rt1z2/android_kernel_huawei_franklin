...
void StateMachineUnattached(struct Port *port)
{
	...
	if (TimerExpired(&port->VbusOnlyTimer) && port->Registers.Status.VBUSOK) {
		DeviceRead(port->I2cAddr, regInterrupta, 5, &port->Registers.Status.byte[2]); /* 5:length 2:two bytes */
		if (port->Registers.Status.VBUSOK) {
			if (!g_is_last_vbus_ok)
			...
			pr_info("FUSB:Fake Vbus ok int happend\n");
		}
	}
