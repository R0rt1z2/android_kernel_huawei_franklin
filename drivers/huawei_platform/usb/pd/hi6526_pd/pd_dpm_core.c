...
static const svdm_svid_ops_t svdm_svid_ops[] = {
#ifdef CONFIG_USB_PD_ALT_MODE_SUPPORT
	{
		...
		.dfp_inform_id = hi6526_dp_dfp_u_notify_discover_id,
		.dfp_inform_svids = hi6526_dp_dfp_u_notify_discover_svid,
		.dfp_inform_modes = hi6526_dp_dfp_u_notify_discover_modes,
		.dfp_inform_enter_mode = hi6526_dp_dfp_u_notify_enter_mode,
		.dfp_inform_exit_mode = hi6526_dp_dfp_u_notify_exit_mode,
		.dfp_inform_attention = hi6526_dp_dfp_u_notify_attention,
#endif
		.ufp_request_enter_mode = hi6526_dp_ufp_u_request_enter_mode,
		.ufp_request_exit_mode = hi6526_dp_ufp_u_request_exit_mode,

#ifdef CONFIG_USB_PD_ALT_MODE_DFP_SUPPORT
		.notify_pe_startup = hi6526_dp_dfp_u_notify_pe_startup,
		.notify_pe_ready = hi6526_dp_dfp_u_notify_pe_ready,
#endif
		.reset_state = hi6526_dp_reset_state,
	},
#endif
};

void hi6526_dpm_check_supported_modes(void)
{
	unsigned int i;
...
int hi6526_pd_dpm_enable_vconn(pd_port_t *pd_port, bool en)
{
	return hi6526_pd_set_vconn(pd_port, en);
}

int hi6526_pd_dpm_send_sink_caps(pd_port_t *pd_port)
{
...
#endif
	return hi6526_pd_send_data_msg(pd_port, TCPC_TX_SOP, PD_DATA_SINK_CAP,
			snk_cap->nr, snk_cap->pdos);
}

int hi6526_pd_dpm_send_source_caps(pd_port_t *pd_port)
{
	uint8_t i;
	...
	if (pd_port->power_cable_present) {
		cable_curr = hi6526_pd_extract_cable_curr(
	...
	for (i = 0; i < src_cap0->nr; i++)
		src_cap1->pdos[i] = hi6526_pd_reset_pdo_power(
				src_cap0->pdos[i], cable_curr);

	ret = hi6526_pd_send_data_msg(pd_port, TCPC_TX_SOP, PD_DATA_SOURCE_CAP,
		...
		pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	D("snk_pdo: 0x%x\n", snk_pdo);
	return hi6526_dpm_find_match_req_info(req_info,
		...
		struct dpm_rdo_info_t *req_info)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		find_cap = hi6526_dpm_find_match_req_info(req_info,
		...
		struct dpm_rdo_info_t *req_info)
{
	...
	hi6526_dpm_extract_pdo_info(snk_cap->pdos[0], &sink);
	hi6526_dpm_extract_pdo_info(src_cap->pdos[0], &source);
...
bool hi6526_pd_dpm_send_request(pd_port_t *pd_port, int mv, int ma)
{
	bool find_cap = false;
	struct dpm_rdo_info_t req_info;
	pd_port_power_caps *src_cap = &pd_port->remote_src_cap;
	...
	find_cap = hi6526_dpm_find_match_req_info(&req_info, snk_pdo, src_cap->nr,
...
bool hi6526_pd_dpm_update_tcp_request(pd_port_t *pd_port,
...
bool hi6526_pd_dpm_update_tcp_request_ex(pd_port_t *pd_port,
	struct tcp_dpm_pd_request_ex *pd_req)
{
	...
	hi6526_dpm_extract_pdo_info(src_cap->pdos[pd_req->pos - 1], &source);
...
bool hi6526_pd_dpm_update_tcp_request_again(pd_port_t *pd_port)
{
	bool find_cap = false;
...
void hi6526_pd_dpm_snk_evaluate_caps(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool find_cap = false;
	int sink_nr, source_nr;
	struct dpm_rdo_info_t req_info;
...
void hi6526_pd_dpm_snk_standby_power(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
void hi6526_pd_dpm_snk_transition_power(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
...
void hi6526_pd_dpm_snk_hard_reset(pd_port_t *pd_port, pd_event_t *pd_event)
{
	/*
...
/* ---- SRC ---- */
void hi6526_pd_dpm_src_evaluate_request(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint8_t rdo_pos;
	uint32_t rdo;
	uint32_t op_curr = 0;
	...
	rdo = pd_event->pd_msg->payload[0];
	rdo_pos = RDO_POS(rdo);
	pd_port->dpm_flags &= (~DPM_FLAGS_PARTNER_MISMATCH);
	if ((rdo_pos > 0) && (rdo_pos <= src_cap->nr)) {
		uint32_t source_vmax, source_i;
		uint32_t pdo = src_cap->pdos[rdo_pos - 1];

		hi6526_pd_extract_rdo_power(rdo, pdo, &op_curr, &max_curr);
		hi6526_pd_extract_pdo_power(pdo,
...
void hi6526_pd_dpm_src_transition_power(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
	hi6526_pd_enable_vbus_stable_detection(pd_port);

	tcpci_source_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_REQUEST,
			pd_port->request_v_new, pd_port->request_i_new);

	if (pd_port->request_v == pd_port->request_v_new)
		hi6526_pd_put_vbus_stable_event(pd_port->tcpc_dev);
...
void hi6526_pd_dpm_src_inform_cable_vdo(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
void hi6526_pd_dpm_src_hard_reset(pd_port_t *pd_port)
{
	D("\n");
	tcpci_source_vbus(pd_port->tcpc_dev,
			TCP_VBUS_CTRL_HRESET, TCPC_VBUS_SOURCE_0V, 0);
	hi6526_pd_enable_vbus_safe0v_detection(pd_port);
}
...
void hi6526_pd_dpm_ufp_request_id_info(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_dpm_ufp_reply_request(pd_port, pd_event,
			dpm_vdm_get_svid(pd_event) == USB_SID_PD);
}

void hi6526_pd_dpm_ufp_request_svid_info(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ack = false;
...
void hi6526_pd_dpm_ufp_request_mode_info(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint16_t svid;
	bool ack = false;
...
void hi6526_pd_dpm_ufp_request_enter_mode(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	bool ack = false;
	uint16_t svid = 0;
...
void hi6526_pd_dpm_ufp_request_exit_mode(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ack = false;
	uint16_t svid = 0;
	uint8_t ops = 0;
...
int hi6526_pd_dpm_ufp_response_id(pd_port_t *pd_port, pd_event_t *pd_event)
{
	return hi6526_pd_reply_svdm_request(pd_port, pd_event, CMDT_RSP_ACK,
			pd_port->id_vdo_nr, pd_port->id_vdos);
}

int hi6526_pd_dpm_ufp_response_svids(pd_port_t *pd_port, pd_event_t *pd_event)
{
	svdm_svid_data_t *svid_data = NULL;
	...
	ret = hi6526_pd_reply_svdm_request(pd_port, pd_event,
			CMDT_RSP_ACK, j, svids);

	return ret;
}

int hi6526_pd_dpm_ufp_response_modes(pd_port_t *pd_port, pd_event_t *pd_event)
{
	svdm_svid_data_t *svid_data = NULL;
	...
	svid_data = dpm_get_svdm_svid_data(pd_port, svid);
	if (svid_data) {
		return hi6526_pd_reply_svdm_request(pd_port, pd_event,
				CMDT_RSP_ACK,
				svid_data->local_mode.mode_cnt,
				svid_data->local_mode.mode_vdo);
	} else {
		PE_DBG("ERROR-4965\n");
		return hi6526_pd_reply_svdm_request_simply(pd_port, pd_event,
				CMDT_RSP_NAK);
	}
}
		...
		uint16_t svid)
{
	...
	hi6526_usb_pd_ufp_update_dock_svid(svid);
...
void hi6526_pd_dpm_dfp_inform_id(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
#if DPM_DBG_ENABLE
	pd_msg_t *pd_msg = pd_event->pd_msg;
...
void hi6526_pd_dpm_dfp_inform_svids(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	uint8_t count;
...
void hi6526_pd_dpm_dfp_inform_modes(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	uint8_t count;
	uint16_t svid;
...
void hi6526_pd_dpm_dfp_inform_enter_mode(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	uint16_t svid = 0;
	uint16_t expected_svid = pd_port->mode_svid;
...
void hi6526_pd_dpm_dfp_inform_exit_mode(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint16_t svid = 0;
	uint16_t expected_svid = pd_port->mode_svid;
	uint8_t ops = 0;
...
void hi6526_pd_dpm_dfp_inform_attention(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint16_t svid = 0;
	uint8_t ops = 0;
...
void hi6526_pd_dpm_dfp_inform_cable_vdo(pd_port_t *pd_port, pd_event_t *pd_event)
{
	const int size = sizeof(uint32_t) * VDO_MAX_SIZE;
	int ret;
...
void hi6526_pd_dpm_ufp_recv_uvdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_msg_t *pd_msg;
...
void hi6526_pd_dpm_dfp_send_uvdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
...
void hi6526_pd_dpm_dfp_inform_uvdm(pd_port_t *pd_port,
		pd_event_t *pd_event, bool ack)
{
	uint16_t svid;
	pd_msg_t *pd_msg = pd_event->pd_msg;
...
void hi6526_pd_dpm_dr_inform_sink_cap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_msg_t *pd_msg = pd_event->pd_msg;
...
void hi6526_pd_dpm_dr_inform_source_cap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_msg_t *pd_msg = pd_event->pd_msg;
	pd_port_power_caps *src_cap = &pd_port->remote_src_cap;
...
void hi6526_pd_dpm_drs_evaluate_swap(pd_port_t *pd_port, uint8_t role)
{
	pd_put_dpm_ack_event(pd_port);
}

void hi6526_pd_dpm_drs_change_role(pd_port_t *pd_port, uint8_t role)
{
	D("\n");
	hi6526_pd_set_data_role(pd_port, role);
 ...
 */
void hi6526_pd_dpm_prs_evaluate_swap(pd_port_t *pd_port, uint8_t role)
{
	int good_power;
	bool accept = true;
	bool sink = false;
...
void hi6526_pd_dpm_prs_turn_off_power_sink(pd_port_t *pd_port)
{
 ...
 */
static void hi6526_pd_assume_vbus_invalid(pd_port_t *pd_port)
{
	struct hi6526_tcpc_device *tcpc_dev = pd_port->tcpc_dev;
	...
	hi6526_pd_put_vbus_changed_event(tcpc_dev, false);
}

void hi6526_pd_dpm_prs_enable_power_source(pd_port_t *pd_port, bool en)
{
	...
	if (en) {
		hi6526_pd_enable_vbus_valid_detection(pd_port, en);
	} else {
		if (tcpci_tcpc_vusb_uv_det_masked(pd_port->tcpc_dev))
			hi6526_pd_assume_vbus_invalid(pd_port);
		else
			hi6526_pd_enable_vbus_safe0v_detection(pd_port);
	}
}

void hi6526_pd_dpm_prs_change_role(pd_port_t *pd_port, uint8_t role)
{
	D("\n");
	hi6526_pd_set_power_role(pd_port, role);
	pd_put_dpm_ack_event(pd_port);
}

/* DRP : Vconn Swap */
void hi6526_pd_dpm_vcs_evaluate_swap(pd_port_t *pd_port)
{
	dpm_response_request(pd_port, true);
}

void hi6526_pd_dpm_vcs_enable_vconn(pd_port_t *pd_port, bool en)
{
	D("\n");
	hi6526_pd_dpm_enable_vconn(pd_port, en);
...
#ifdef CONFIG_USB_PD_REV30
void hi6526_pd_dpm_inform_alert(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
int hi6526_pd_dpm_send_alert(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	return hi6526_pd_send_message(pd_port, TCPC_TX_SOP, PD_DATA_ALERT,
			PD_ADO_SIZE, &ado);
}

#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL
int hi6526_pd_dpm_send_status(pd_port_t *pd_port)
{
	...
	return hi6526_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_STATUS,
...
/* PE : Notify DPM */
static int pd_dpm_ready_power_role_swap(pd_port_t *pd_port)
{
	bool do_swap = false;
...
int hi6526_pd_dpm_notify_pe_startup(pd_port_t *pd_port)
{
	uint32_t caps;
	uint32_t flags = 0;
...
int hi6526_pd_dpm_notify_pe_hardreset(pd_port_t *pd_port)
{
	uint32_t flags = 0;
...
int hi6526_pd_dpm_notify_pe_ready(pd_port_t *pd_port, pd_event_t *pd_event)
{
	int ret = 0;

	D("dpm_flags 0x%08x\n", pd_port->dpm_flags);
	hi6526_pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_SUCCESS);
	...
	if (!pd_port->pe_ready) {
		...
		hi6526_tcpc_dual_role_instance_changed(pd_port->tcpc_dev->dr_usb);
#endif
		hi6526_pd_update_connect_state(pd_port, HI6526_PD_CONNECT_PE_READY);
	}
...
#ifdef CONFIG_USB_PD_DFP_FLOW_DELAY
int hi6526_pd_dpm_notify_dfp_delay_done(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
	if (pd_port->data_role == PD_ROLE_DFP) {
		pd_port->dpm_dfp_flow_delay_done = true;
...
int hi6526_pd_dpm_notify_ufp_delay_done(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("\n");
	if (pd_port->data_role == PD_ROLE_UFP) {
		pd_port->dpm_ufp_flow_delay_done = true;
		hi6526_pd_dpm_notify_pe_ready(pd_port, pd_event);
	}
...
/* dpm_core_init */
int hi6526_pd_dpm_core_init(pd_port_t *pd_port)
{
	unsigned int i;
