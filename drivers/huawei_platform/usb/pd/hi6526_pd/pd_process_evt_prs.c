		...
		pd_event_t *pd_event)
{
	...
	case PE_PRS_SRC_SNK_WAIT_SOURCE_ON:
		...
		hi6526_pd_unlock_msg_output(pd_port); /* for tSRCTransition */
		return false;
...
static bool pd_process_ctrl_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_CTRL_WAIT:
	case PD_CTRL_REJECT:
		hi6526_pd_notify_pe_cancel_pr_swap(pd_port);
...
static bool pd_process_timer_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_TIMER_SOURCE_TRANSITION:
		hi6526_pd_dpm_prs_enable_power_source(pd_port, false);
		D("-\n");
		return false;
...
bool hi6526_pd_process_event_prs(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ret = false;
