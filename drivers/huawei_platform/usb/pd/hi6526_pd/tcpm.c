...
int hi6526_tcpm_shutdown(struct hi6526_tcpc_device *tcpc_dev)
{
...
int hi6526_tcpm_inquire_remote_cc(struct hi6526_tcpc_device *tcpc_dev,
...
void hi6526_tcpm_force_cc_mode(struct hi6526_tcpc_device *tcpc_dev, int mode)
{
...
int hi6526_tcpm_typec_set_wake_lock(struct hi6526_tcpc_device *tcpc, bool wake_lock)
{
	if (wake_lock) {
...
int hi6526_tcpm_typec_set_rp_level(struct hi6526_tcpc_device *tcpc_dev, uint8_t level)
{
	uint8_t res;
	...
	return hi6526_tcpc_typec_set_rp_level(tcpc_dev, res);
}

/*
 * Force role swap, include power and data role, I guess!
 */
int hi6526_tcpm_typec_role_swap(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	if (hi6526_tcpc_typec_swap_role(tcpc_dev) == 0)
		return TCPM_SUCCESS;
#endif
 ...
 * Vendor defined case!
 */
int hi6526_tcpm_typec_notify_direct_charge(struct hi6526_tcpc_device *tcpc_dev,
		bool direct_charge)
{
	if (tcpc_dev->typec_attach_old == HI6526_TYPEC_UNATTACHED)
		return TCPM_ERROR_UNATTACHED;

	return hi6526_tcpc_typec_set_direct_charge(tcpc_dev, direct_charge);
}

int hi6526_tcpm_typec_change_role(struct hi6526_tcpc_device *tcpc_dev,
		uint8_t typec_role)
{
	return hi6526_tcpc_typec_change_role(tcpc_dev, typec_role);
}

static int hi6526_tcpm_check_typec_attached(struct hi6526_tcpc_device *tcpc)
{
	if (tcpc->typec_attach_old == HI6526_TYPEC_UNATTACHED ||
		tcpc->typec_attach_new == HI6526_TYPEC_UNATTACHED)
		return TCPM_ERROR_UNATTACHED;
...
int hi6526_tcpm_inquire_typec_remote_rp_curr(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	if (hi6526_tcpm_check_typec_attached(tcpc_dev))
		return 0;
...
EXPORT_SYMBOL(hi6526_tcpm_inquire_typec_remote_rp_curr);
...
EXPORT_SYMBOL(hi6526_tcpm_inquire_vbus_level);

uint8_t hi6526_tcpm_inquire_typec_attach_state(struct hi6526_tcpc_device *tcpc_dev)
{
	if (!tcpc_dev)
		return HI6526_TYPEC_UNATTACHED;
	return tcpc_dev->typec_attach_new;
}
EXPORT_SYMBOL(hi6526_tcpm_inquire_typec_attach_state);

/* Request TCPC to send PD Request */
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
static int tcpm_check_pd_attached(struct hi6526_tcpc_device *tcpc)
{
	D("\n");
	if (tcpc->typec_attach_old == HI6526_TYPEC_UNATTACHED) {
		D("HI6526_TYPEC_UNATTACHED\n");
		return TCPM_ERROR_UNATTACHED;
	}
...
bool hi6526_tcpm_support_pd(struct hi6526_tcpc_device *tcpc)
{
...
bool hi6526_tcpm_support_apdo(struct hi6526_tcpc_device *tcpc)
{
	return pd_is_source_support_apdo(&tcpc->pd_port);
}

int hi6526_tcpm_power_role_swap(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;
	...
	if (!pd_port->pd_prev_connected) {
		...
		return hi6526_tcpm_typec_role_swap(tcpc_dev);
#else
...
EXPORT_SYMBOL(hi6526_tcpm_power_role_swap);

int hi6526_tcpm_data_role_swap(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
...
EXPORT_SYMBOL(hi6526_tcpm_data_role_swap);

int hi6526_tcpm_vconn_swap(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
...
EXPORT_SYMBOL(hi6526_tcpm_vconn_swap);

int hi6526_tcpm_soft_reset(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
...
EXPORT_SYMBOL(hi6526_tcpm_soft_reset);

int hi6526_tcpm_hard_reset(struct hi6526_tcpc_device *tcpc_dev)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
...
EXPORT_SYMBOL(hi6526_tcpm_hard_reset);

int hi6526_tcpm_get_source_cap(struct hi6526_tcpc_device *tcpc_dev,
...
EXPORT_SYMBOL(hi6526_tcpm_get_source_cap);

int hi6526_tcpm_get_sink_cap(struct hi6526_tcpc_device *tcpc_dev,
		struct tcpm_power_cap *cap)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
...
EXPORT_SYMBOL(hi6526_tcpm_get_sink_cap);

static int hi6526_tcpm_get_local_sink_cap(struct hi6526_tcpc_device *tcpc_dev,
		struct local_sink_cap *cap)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		hi6526_dpm_extract_pdo_info(snk_cap->pdos[i], &sink);
		cap[i].mv = sink.vmin;
		cap[i].ma = sink.ma;
		cap[i].uw = sink.uw;
	}
...
static int hi6526_tcpm_request(struct hi6526_tcpc_device *tcpc_dev, int mv, int ma)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	...
	mutex_lock(&pd_port->pd_lock);
	ret = hi6526_pd_dpm_send_request(pd_port, mv, ma);
	mutex_unlock(&pd_port->pd_lock);
...
void hi6526_tcpm_request_voltage(struct hi6526_tcpc_device *tcpc_dev, int set_voltage)
{
	bool overload = false;
	int vol_mv = 0;
	int cur_ma = 0;
	...
	D("+\n");
	if (!hi6526_tcpm_support_pd(tcpc_dev))
		return;

	ret = hi6526_tcpm_get_local_sink_cap(tcpc_dev, sink_cap_info);
	if (ret != TCPM_SUCCESS) {
		D("hi6526_tcpm_get_local_sink_cap ret %d\n", ret);
		return;
	}
	...
	ret = hi6526_tcpm_request(tcpc_dev, vol_mv, cur_ma);
	if (ret != TCPM_SUCCESS)
		TYPEC_INFO("hi6526_tcpm_request ret %d\n", ret);

	D("-\n");
}
EXPORT_SYMBOL(hi6526_tcpm_request_voltage);

int hi6526_tcpm_discover_cable(struct hi6526_tcpc_device *tcpc_dev, uint32_t *vdos,
...
void hi6526_tcpm_detect_emark_cable(struct hi6526_tcpc_device *tcpc_dev)
{
	hi6526_pd_cable_flag_set(&tcpc_dev->pd_port);
	(void)hi6526_tcpm_data_role_swap(tcpc_dev);
	tcpc_dev->pd_port.dpm_flags |= DPM_FLAGS_CHECK_CABLE_ID_DFP;
}

#ifdef CONFIG_USB_PD_UVDM_SUPPORT
int hi6526_tcpm_send_uvdm(struct hi6526_tcpc_device *tcpc_dev,
...
EXPORT_SYMBOL(hi6526_tcpm_send_uvdm);
...
static bool tcpm_extract_power_cap_val(uint32_t pdo, struct tcpm_power_cap_val *cap)
{
	...
	hi6526_dpm_extract_pdo_info(pdo, &info);
...
#ifdef CONFIG_USB_PD_REV30
static int tcpm_put_tcp_dpm_event(struct hi6526_tcpc_device *tcpc,
		struct tcp_dpm_event *event)
{
	...
	ret = hi6526_pd_put_deferred_tcp_event(tcpc, event);
	if (!ret)
		return TCPM_ERROR_PUT_EVENT;
...
static int tcpm_put_tcp_dpm_event_cb(struct hi6526_tcpc_device *tcpc,
		struct tcp_dpm_event *event,
...
static int tcpm_dpm_bk_event_cb(struct hi6526_tcpc_device *tcpc,
...
static int __tcpm_put_tcp_dpm_event_bk(
		struct hi6526_tcpc_device *tcpc, struct tcp_dpm_event *event,
...
static int tcpm_put_tcp_dpm_event_bk(struct hi6526_tcpc_device *tcpc,
...
static int tcpm_put_tcp_dpm_event_cbk1(struct hi6526_tcpc_device *tcpc,
		struct tcp_dpm_event *event,
...
static int tcpm_put_tcp_dpm_event_cbk2(struct hi6526_tcpc_device *tcpc,
		struct tcp_dpm_event *event,
		const struct tcp_dpm_event_cb_data *cb_data,
		uint32_t tout_ms, uint8_t *data, uint8_t size)
{
...
static int tcpm_dpm_pd_get_pps_status_raw(struct hi6526_tcpc_device *tcpc,
		const struct tcp_dpm_event_cb_data *cb_data,
		...
		struct pd_pps_status *pps_status)
{
	...
	pd_port_t *pd_port = (tcpc ? &tcpc->pd_port : NULL);
...
int hi6526_tcpm_set_apdo_charging_policy(struct hi6526_tcpc_device *tcpc, uint8_t policy,
		int mv, int ma, struct tcp_dpm_event_cb_data *cb_data)
{
	...
	if (!hi6526_tcpm_support_apdo(tcpc)) {
		D("Not Support APDO");
...
int hi6526_tcpm_inquire_pd_source_apdo(struct hi6526_tcpc_device *tcpc, uint8_t apdo_type,
		uint8_t *cap_i, struct tcpm_power_cap_val *cap_val)
{
	...
	ret = hi6526_tcpm_get_source_cap(tcpc, &cap);
	if (ret != TCPM_SUCCESS)
		return ret;
...
int hi6526_tcpm_set_pd_charging_policy(struct hi6526_tcpc_device *tcpc, uint8_t policy,
...
EXPORT_SYMBOL(hi6526_tcpm_dpm_pd_request);
...
int hi6526_tcpm_register_tcpc_dev_notifier(struct hi6526_tcpc_device *tcp_dev,
		struct notifier_block *nb)
{
	return hi6526_register_tcp_dev_notifier(tcp_dev, nb);
}
EXPORT_SYMBOL(hi6526_tcpm_register_tcpc_dev_notifier);

int hi6526_tcpm_unregister_tcpc_dev_notifier(struct hi6526_tcpc_device *tcp_dev,
		struct notifier_block *nb)
{
	return hi6526_unregister_tcp_dev_notifier(tcp_dev, nb);
}
EXPORT_SYMBOL(hi6526_tcpm_unregister_tcpc_dev_notifier);
