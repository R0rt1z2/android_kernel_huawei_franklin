...
static inline void tcpci_alert_frswap(struct hi6526_tcpc_device *tcpc_dev)
{
	D("FR_swap\n");
...
static inline int tcpci_alert_cc_changed(struct hi6526_tcpc_device *tcpc_dev)
{
	return hi6526_tcpc_typec_handle_cc_change(tcpc_dev);
}

void hi6526_tcpci_vbus_level_init(struct hi6526_tcpc_device *tcpc_dev,
...
static int tcpci_alert_power_status_changed(struct hi6526_tcpc_device *tcpc_dev)
{
	int rv;
	...
	rv = hi6526_tcpc_typec_handle_ps_change(tcpc_dev, tcpc_dev->vbus_level);
	if (rv < 0)
		return rv;

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_pd_put_vbus_changed_event(tcpc_dev, true);
#endif
...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
static int tcpci_alert_tx_success(struct hi6526_tcpc_device *tcpc_dev)
{
	uint8_t tx_state;
	pd_event_t evt = {
	...
	if (tx_state == PD_TX_STATE_WAIT_CRC_VDM)
		hi6526_pd_put_vdm_event(tcpc_dev, &evt, false);
#ifdef CONFIG_HI6526_TCPC_QUIRK_V100
	else if (tx_state == PD_TX_STATE_WAIT_CRC_PD)
#else
	else
#endif
		hi6526_pd_put_event(tcpc_dev, &evt, false);

	return 0;
}

static int tcpci_alert_tx_failed(struct hi6526_tcpc_device *tcpc_dev)
{
	uint8_t tx_state;

	if (tcpc_dev->typec_attach_new == HI6526_TYPEC_ATTACHED_SNK)
...
static int tcpci_alert_tx_discard(struct hi6526_tcpc_device *tcpc_dev)
{
	uint8_t tx_state;
	...
	if (tx_state == PD_TX_STATE_WAIT_CRC_VDM) {
		hi6526_pd_put_last_vdm_event(tcpc_dev);
#ifdef CONFIG_HI6526_TCPC_QUIRK_V100
	} else if (tx_state == PD_TX_STATE_WAIT_CRC_PD) {
#else
	} else {
		...
		if (retry_crc_discard) {
			...
			hi6526_tcpc_enable_timer(tcpc_dev, PD_TIMER_DISCARD);
#else
...
static int tcpci_alert_recv_msg(struct hi6526_tcpc_device *tcpc_dev)
{
	int retval;
	...
	pd_msg = hi6526_pd_alloc_msg(tcpc_dev);
	if (!pd_msg) {
	...
	if (retval < 0) {
		...
		hi6526_pd_free_msg(tcpc_dev, pd_msg);
		return retval;
	}

	pd_msg->frame_type = (uint8_t)type;

	hi6526_pd_put_pd_msg_event(tcpc_dev, pd_msg);

	return 0;
}

static int tcpci_alert_rx_overflow(struct hi6526_tcpc_device *tcpc_dev)
{
	int rv;
...
static int tcpci_alert_recv_hard_reset(struct hi6526_tcpc_device *tcpc_dev)
{
	hi6526_pd_put_recv_hard_reset_event(tcpc_dev);
...
static int tcpci_alert_fault(struct hi6526_tcpc_device *tcpc_dev)
{
	uint8_t status = 0;
...
static bool tcpci_check_hard_reset_complete(
			struct hi6526_tcpc_device *tcpc_dev, uint32_t alert_status)
{
	D("+\n");

	if ((alert_status & TCPC_REG_ALERT_HRESET_SUCCESS) ==
			TCPC_REG_ALERT_HRESET_SUCCESS) {
		hi6526_pd_put_sent_hard_reset_event(tcpc_dev);
		return true;
	}
...
static int __tcpci_alert(struct hi6526_tcpc_device *tcpc_dev)
{
	int rv;
	uint32_t alert_status = 0;
	...
	rv = tcpci_get_alert_status(tcpc_dev, &alert_status);
	if (rv)
		return HI6526_TCPC_ALERT_I2C_TIMEOUT;

	if (!alert_status)
		return HI6526_TCPC_ALERT_STATUS_ZERO;
...
int hi6526_tcpci_alert(struct hi6526_tcpc_device *tcpc_dev)
{
	int ret;
	static uint32_t cnt = 0;

	hi6526_tcpci_lock_typec(tcpc_dev);
	ret = __tcpci_alert(tcpc_dev);
	hi6526_tcpci_unlock_typec(tcpc_dev);

	/*
	 * Charger Irq or I2C Timeout: cnt ascend by 1,
	 * PD Irq: cnt inited to 0.
	 */
	if (ret == HI6526_TCPC_ALERT_STATUS_ZERO
		|| ret == HI6526_TCPC_ALERT_I2C_TIMEOUT) {
		cnt++;

		if (cnt >= HI6526_TCPC_OTHER_IRQ_MAX_LOOPS) {
...
EXPORT_SYMBOL(hi6526_tcpci_alert);

static int __tcpci_set_wake_lock_pd(struct hi6526_tcpc_device *tcpc,
...
static int tcpci_set_wake_lock_pd(struct hi6526_tcpc_device *tcpc, bool pd_lock)
{
...
static int tcpci_report_usb_port_attached(struct hi6526_tcpc_device *tcpc)
{
	...
	hi6526_tcpc_dual_role_instance_changed(tcpc->dr_usb);
#endif /* CONFIG_DUAL_ROLE_USB_INTF */

	if (tcpc->typec_attach_new != HI6526_TYPEC_ATTACHED_AUDIO)
		tcpci_set_wake_lock_pd(tcpc, true);

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_pd_put_cc_attached_event(tcpc, tcpc->typec_attach_new);
#endif

	return 0;
}

static int tcpci_report_usb_port_detached(struct hi6526_tcpc_device *tcpc)
{
...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_pd_put_cc_detached_event(tcpc);
#endif
	return 0;
}
...
int hi6526_tcpci_report_usb_port_changed(struct hi6526_tcpc_device *tcpc)
{
	...
	} else if (tcpc->typec_attach_new == HI6526_TYPEC_UNATTACHED) {
...
EXPORT_SYMBOL(hi6526_tcpci_report_usb_port_changed);

static inline int tcpci_report_power_control_on(struct hi6526_tcpc_device *tcpc)
{
	tcpci_set_wake_lock_pd(tcpc, true);
	return 0;
}

static inline int tcpci_report_power_control_off(struct hi6526_tcpc_device *tcpc)
{
	tcpci_set_wake_lock_pd(tcpc, false);
	return 0;
}
