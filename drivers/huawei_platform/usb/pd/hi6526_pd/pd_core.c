...
int hi6526_pd_core_init(struct hi6526_tcpc_device *tcpc_dev)
{
	int ret;
	...
	pd_port->pd_connect_state = HI6526_PD_CONNECT_NONE;
	...
	hi6526_pd_dpm_core_init(pd_port);

	PE_INFO("hi6526_pd_core_init\n");
...
void hi6526_pd_extract_rdo_power(uint32_t rdo, uint32_t pdo,
...
uint32_t hi6526_pd_reset_pdo_power(uint32_t pdo, uint32_t imax)
{
	uint32_t ioper;
...
void hi6526_pd_extract_pdo_power(uint32_t pdo,
...
uint32_t hi6526_pd_extract_cable_curr(uint32_t vdo)
{
	uint32_t cable_curr;
...
void hi6526_pd_reset_svid_data(pd_port_t *pd_port)
{
	uint8_t i;
	...
	/* clear */
	hi6526_usb_pd_ufp_update_dock_svid(0);
	D("-\n");
}

int hi6526_pd_reset_protocol_layer(pd_port_t *pd_port)
{
	int i = 0;

	hi6526_pd_notify_pe_reset_protocol(pd_port);
...
int hi6526_pd_set_rx_enable(pd_port_t *pd_port, uint8_t enable)
{
	D("0x%x\n", enable);
	return tcpci_set_rx_enable(pd_port->tcpc_dev, enable);
}

void hi6526_pd_set_vbus_detect(pd_port_t *pd_port, bool enable)
{
	D("\n");
...
int hi6526_pd_enable_vbus_valid_detection(pd_port_t *pd_port, bool wait_valid)
{
	D("wait_valid %d\n", wait_valid);

	if (wait_valid) {
		hi6526_pd_notify_pe_wait_vbus_once(pd_port, PD_WAIT_VBUS_VALID_ONCE);
	} else {
		hi6526_pd_notify_pe_wait_vbus_once(pd_port, PD_WAIT_VBUS_INVALID_ONCE);
	}

	return 0;
}

int hi6526_pd_enable_vbus_safe0v_detection(pd_port_t *pd_port)
{
	D("+-\n");
	PE_DBG("WaitVSafe0V\n");
	hi6526_pd_notify_pe_wait_vbus_once(pd_port, PD_WAIT_VBUS_SAFE0V_ONCE);
	return 0;
}

int hi6526_pd_enable_vbus_stable_detection(pd_port_t *pd_port)
{
	D("+-\n");
	PE_DBG("WaitVStable\n");
	hi6526_pd_notify_pe_wait_vbus_once(pd_port, PD_WAIT_VBUS_STABLE_ONCE);
	return 0;
}

int hi6526_pd_set_data_role(pd_port_t *pd_port, uint8_t dr)
{
	int ret;
	...
	tcpci_notify_role_swap(pd_port->tcpc_dev, HI6526_TCP_NOTIFY_DR_SWAP, dr);
...
int hi6526_pd_set_power_role(pd_port_t *pd_port, uint8_t pr)
{
	int ret;
	...
	hi6526_pd_notify_pe_pr_changed(pd_port);
	...
	tcpci_notify_role_swap(pd_port->tcpc_dev, HI6526_TCP_NOTIFY_PR_SWAP, pr);
	D("-\n");
	return ret;
}

int hi6526_pd_init_role(pd_port_t *pd_port, uint8_t pr, uint8_t dr, bool vr)
{
	int ret;
	D("+\n");
	pd_port->power_role = pr;
...
int hi6526_pd_set_vconn(pd_port_t *pd_port, int enable)
{
	int ret;
	...
	tcpci_notify_role_swap(pd_port->tcpc_dev,
				HI6526_TCP_NOTIFY_VCONN_SWAP, enable);
...
int hi6526_pd_reset_local_hw(pd_port_t *pd_port)
{
	D("+\n");
	hi6526_pd_notify_pe_transit_to_default(pd_port);
	hi6526_pd_unlock_msg_output(pd_port);

	pd_reset_pe_timer(pd_port);
	hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_HARDRESET);
	...
	hi6526_pd_cable_flag_clear(pd_port);
	...
	hi6526_pd_set_vconn(pd_port, false);
	...
	if (pd_port->power_role == PD_ROLE_SINK) {
		...
		hi6526_pd_set_data_role(pd_port, PD_ROLE_UFP);
	} else {
		pd_port->state_machine = PE_STATE_MACHINE_SOURCE;
		D("state_machine %d(%s)\n", pd_port->state_machine,
			pd_pe_state_machine_name(pd_port->state_machine));
		hi6526_pd_set_data_role(pd_port, PD_ROLE_DFP);
	}

	hi6526_pd_dpm_notify_pe_hardreset(pd_port);
...
int hi6526_pd_enable_bist_test_mode(pd_port_t *pd_port, bool en)
{
	D("+-\n");
...
int hi6526_pd_handle_soft_reset(pd_port_t *pd_port, uint8_t state_machine)
{
	int ret;
	...
	D("state_machine %s\n", pd_pe_state_machine_name(pd_port->state_machine));
	hi6526_pd_reset_protocol_layer(pd_port);
	hi6526_pd_notify_tcp_event_buf_reset(pd_port, TCP_DPM_RET_DROP_RECV_SRESET);
	ret = hi6526_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_ACCEPT);
	D("-\n");
	return ret;
}
...
int hi6526_pd_send_message(pd_port_t *pd_port, uint8_t sop_type,
		uint8_t msg, uint16_t count, const uint32_t *data)
{
...
#ifdef CONFIG_USB_PD_REV30
		hi6526_pd_sync_sop_spec_revision(pd_port, pd_port->remote_rev);
	...
	hi6526_pd_notify_pe_transmit_msg(pd_port, type);
...
int hi6526_pd_send_ctrl_msg(pd_port_t *pd_port, uint8_t sop_type, uint8_t msg)
{
	return hi6526_pd_send_message(pd_port, sop_type, msg, 0, NULL);
}

int hi6526_pd_send_data_msg(pd_port_t *pd_port, uint8_t sop_type,
		uint8_t msg, uint8_t cnt, uint32_t *payload)
{
	return hi6526_pd_send_message(pd_port, sop_type, msg, cnt, payload);
}

#ifdef CONFIG_USB_PD_REV30
int hi6526_pd_send_ext_msg(pd_port_t *pd_port,
...
int hi6526_pd_send_cable_soft_reset(pd_port_t *pd_port)
{
	...
	return hi6526_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP_PRIME, PD_CTRL_SOFT_RESET);
#endif
}
...
bool pd_put_pe_event(pd_port_t *pd_port, uint8_t pe_event)
{
	...
	return hi6526_pd_put_event(pd_port->tcpc_dev, &evt, false);
}
...
bool vdm_put_dpm_vdm_request_event(pd_port_t *pd_port, uint8_t event)
{
	...
	ret = hi6526_pd_put_vdm_event(pd_port->tcpc_dev, &evt, false);
	if (ret)
...
bool vdm_put_hw_event(
	struct hi6526_tcpc_device *tcpc_dev, uint8_t hw_event)
{
	pd_event_t evt = {
	...
	return hi6526_pd_put_vdm_event(tcpc_dev, &evt, false);
}
...
bool vdm_put_dpm_event(pd_port_t *pd_port, uint8_t dpm_event, pd_msg_t *pd_msg)
{
	...
	return hi6526_pd_put_vdm_event(pd_port->tcpc_dev, &evt, false);
}
...
bool pd_put_hw_event(struct hi6526_tcpc_device *tcpc_dev, uint8_t hw_event)
{
	pd_event_t evt = {
		.event_type = PD_EVT_HW_MSG,
		.msg = hw_event,
	...
	return hi6526_pd_put_event(tcpc_dev, &evt, false);
}

bool hi6526_pd_put_cc_attached_event(struct hi6526_tcpc_device *tcpc_dev, uint8_t type)
{
	pd_event_t evt = {
		.event_type = PD_EVT_HW_MSG,
...
int hi6526_pd_send_soft_reset(pd_port_t *pd_port, uint8_t state_machine)
{
	int ret;
	...
	D("state_machine %s\n", pd_pe_state_machine_name(pd_port->state_machine));
	hi6526_pd_reset_protocol_layer(pd_port);
	hi6526_pd_notify_tcp_event_buf_reset(pd_port, TCP_DPM_RET_DROP_SENT_SRESET);

	ret = hi6526_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_SOFT_RESET);
	return ret;
}

int hi6526_pd_send_hard_reset(pd_port_t *pd_port)
{
	int ret;
	struct hi6526_tcpc_device *tcpc_dev = pd_port->tcpc_dev;
	...
	hi6526_pd_notify_pe_send_hard_reset(pd_port);
...
int hi6526_pd_send_bist_mode2(pd_port_t *pd_port)
{
	int ret = 0;
...
int hi6526_pd_disable_bist_mode2(pd_port_t *pd_port)
{
	D("\n");
...
int hi6526_pd_send_svdm_request(pd_port_t *pd_port,
		...
		uint32_t timer_id)
{
	...
	ret = hi6526_pd_send_data_msg(pd_port, sop_type,
...
int hi6526_pd_reply_svdm_request(pd_port_t *pd_port, pd_event_t *pd_event,
				uint8_t reply, uint8_t cnt, uint32_t *data_obj)
{
	...
	rv = hi6526_pd_send_data_msg(pd_port, TCPC_TX_SOP, PD_DATA_VENDOR_DEF, cnt + 1, payload);
	D("-\n");

	return rv;
}

void hi6526_pd_lock_msg_output(pd_port_t *pd_port)
{
	D("+-\n");
	...
	hi6526_pd_dbg_info_lock();
}

void hi6526_pd_unlock_msg_output(pd_port_t *pd_port)
{
	D("+-\n");
	if (!pd_port->msg_output_lock)
		return;
	...
	hi6526_pd_dbg_info_unlock();
}

int hi6526_pd_update_connect_state(pd_port_t *pd_port, uint8_t state)
{
	int ret;
	...
	switch (state) {
	case HI6526_PD_CONNECT_TYPEC_ONLY:
		if (pd_port->power_role == PD_ROLE_SOURCE) {
			state = HI6526_PD_CONNECT_TYPEC_ONLY_SRC;
		} else {
			switch (pd_port->tcpc_dev->typec_remote_rp_level) {
			case TYPEC_CC_VOLT_SNK_DFT:
				state = HI6526_PD_CONNECT_TYPEC_ONLY_SNK_DFT;
				break;

			case TYPEC_CC_VOLT_SNK_1_5:
			case TYPEC_CC_VOLT_SNK_3_0:
				state = HI6526_PD_CONNECT_TYPEC_ONLY_SNK;
				break;
			}
		}
		break;

	case HI6526_PD_CONNECT_PE_READY:
		state = pd_port->power_role == PD_ROLE_SOURCE ?
			HI6526_PD_CONNECT_PE_READY_SRC : HI6526_PD_CONNECT_PE_READY_SNK;
		break;

	case HI6526_PD_CONNECT_NONE:
		break;
	}
...
void hi6526_pd_sync_sop_spec_revision(pd_port_t *pd_port, uint8_t rev)
{
...
void hi6526_pd_sync_sop_prime_spec_revision(pd_port_t *pd_port, uint8_t rev)
{
	if (pd_port->dpm_charging_policy != DPM_CHARGING_POLICY_PPS)
		return;
