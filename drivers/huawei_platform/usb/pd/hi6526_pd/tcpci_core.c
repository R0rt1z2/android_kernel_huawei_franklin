...
#define to_tcpc_device(obj) container_of(obj, struct hi6526_tcpc_device, dev)
static struct class *hi6526_tcpc_class;
...
#ifdef CONFIG_HI6526_DEBUG_FS
#define TCPC_DEVICE_ATTR_MODE (S_IRUGO | S_IWUSR)
#else
#define TCPC_DEVICE_ATTR_MODE S_IRUGO
#endif

static struct device_attribute tcpc_device_attributes[] = {
	TCPC_DEVICE_ATTR(role_def, TCPC_DEVICE_ATTR_MODE),
	TCPC_DEVICE_ATTR(rp_lvl, TCPC_DEVICE_ATTR_MODE),
	TCPC_DEVICE_ATTR(pd_test, TCPC_DEVICE_ATTR_MODE),
	TCPC_DEVICE_ATTR(info, S_IRUGO),
	TCPC_DEVICE_ATTR(timer, TCPC_DEVICE_ATTR_MODE),
...
static void tcpc_show_cap_info(struct hi6526_tcpc_device *tcpc, char *buf)
{
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
...
static void tcpc_show_pd_state(struct hi6526_tcpc_device *tcpc, char *buf)
{
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	int size;
...
static ssize_t tcpc_show_property(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct hi6526_tcpc_device *tcpc = to_tcpc_device(dev);
	...
	case TCPC_DESC_REMOTE_RP_LEVEL:
		(void)hi6526_tcpm_inquire_remote_cc(tcpc, &cc1, &cc2, from_ic);
		size = snprintf_s(buf, MAX_PROP_LEN, MAX_PROP_LEN - 1,
				" cc1 %s cc2 %s\n",
	...
	case TCPC_DESC_PE_READY:
		size = snprintf_s(buf, MAX_PROP_LEN, MAX_PROP_LEN - 1,
				"%s\n",
				(tcpc->pd_port.pe_ready ? "yes" : "no"));
		if (size < 0)
			E("show pe_ready fail!\n");
		break;
#endif
	default:
		break;
...
static void tcpc_enable_timer(struct hi6526_tcpc_device *tcpc, uint32_t timer_id)
{
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	if (timer_id > 0 && timer_id <= PD_PE_TIMER_END_ID)
		...
		hi6526_tcpc_enable_timer(tcpc, timer_id);
#else
...
static void tcpc_pd_test(struct hi6526_tcpc_device *tcpc, unsigned int test_id)
{
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	...
	case 1: /* Power Role Swap */
		hi6526_tcpm_power_role_swap(tcpc);
		break;

	case 2: /* Data Role Swap */
		hi6526_tcpm_data_role_swap(tcpc);
		break;

	case 3: /* Vconn Swap */
		hi6526_tcpm_vconn_swap(tcpc);
		break;

	case 4: /* Software Reset */
		hi6526_tcpm_soft_reset(tcpc);
		break;

	case 5: /* Hardware Reset */
		hi6526_tcpm_hard_reset(tcpc);
		break;

	case 6: /* src cap */
		hi6526_tcpm_get_source_cap(tcpc, &cap);
		break;

	case 7: /* snk cap */
		hi6526_tcpm_get_sink_cap(tcpc, &cap);
		break;

	case 8: /* unattach */
		hi6526_tcpc_typec_force_unattach(tcpc);
		break;
...
static inline void tcpc_set_local_rp_level(struct hi6526_tcpc_device *tcpc,
		unsigned int level)
{
	hi6526_tcpm_typec_set_rp_level(tcpc, level);
}

#endif /* CONFIG_HI6526_DEBUG_FS */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (offset) {
	case TCPC_DESC_ROLE_DEF:
		D("to hi6526_tcpm_typec_change_role\n");
		(void)hi6526_tcpm_typec_change_role(tcpc, val);
		break;
...
static int tcpc_match_device_by_name(struct device *dev, const void *data)
{
	...
	struct hi6526_tcpc_device *tcpc = dev_get_drvdata(dev);

	return strcmp(tcpc->desc.name, name) == 0;
}

struct hi6526_tcpc_device *hi6526_tcpc_dev_get_by_name(const char *name)
{
	struct device *dev = NULL;

	dev = class_find_device(hi6526_tcpc_class,
...
{
	struct hi6526_tcpc_device *tcpc_dev = to_tcpc_device(dev);

	D("+\n");

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_tcpci_event_deinit(tcpc_dev);
#endif

	hi6526_tcpci_timer_deinit(tcpc_dev);
	devm_kfree(dev, tcpc_dev);
	D("-\n");
}

struct hi6526_tcpc_device *hi6526_tcpc_device_register(struct device *parent,
		...
		struct tcpc_ops *ops, void *drv_data)
{
	...
	tcpc->dev.class = hi6526_tcpc_class;
	...
	wakeup_source_init(&tcpc->attach_wake_lock,
			"hi6526_tcpc_attach_wakelock");
	wakeup_source_init(&tcpc->dettach_temp_wake_lock,
			"hi6526_tcpc_detach_wakelock");

	hi6526_tcpci_timer_init(tcpc);

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_tcpci_event_init(tcpc);
	hi6526_pd_core_init(tcpc);
#endif

#ifdef CONFIG_DUAL_ROLE_USB_INTF
	ret = hi6526_tcpc_dual_role_phy_init(tcpc);
	if (ret < 0)
...
EXPORT_SYMBOL(hi6526_tcpc_device_register);

int hi6526_tcpc_tcpci_init(struct hi6526_tcpc_device *tcpc, bool sw_reset)
{
	int ret;
	...
	D("power_status 0x%x\n", power_status);
	hi6526_tcpci_vbus_level_init(tcpc, power_status);
...
static int tcpc_device_irq_enable(struct hi6526_tcpc_device *tcpc)
{
	int ret;

	D("+\n");

	ret = hi6526_tcpc_tcpci_init(tcpc, false);
	if (ret < 0) {
		pr_err("%s tcpc init fail\n", __func__);
		return ret;
	}

	hi6526_tcpci_lock_typec(tcpc);
	ret = hi6526_tcpc_typec_init(tcpc, tcpc->desc.role_def);
	hi6526_tcpci_unlock_typec(tcpc);
...
int hi6526_tcpc_schedule_init_work(struct hi6526_tcpc_device *tcpc)
{
	return tcpc_device_irq_enable(tcpc);
}
EXPORT_SYMBOL(hi6526_tcpc_schedule_init_work);

int hi6526_register_tcp_dev_notifier(struct hi6526_tcpc_device *tcp_dev,
...
EXPORT_SYMBOL(hi6526_register_tcp_dev_notifier);

int hi6526_unregister_tcp_dev_notifier(struct hi6526_tcpc_device *tcp_dev,
		struct notifier_block *nb)
{
	return srcu_notifier_chain_unregister(&tcp_dev->evt_nh, nb);
}
EXPORT_SYMBOL(hi6526_unregister_tcp_dev_notifier);

void hi6526_tcpc_device_unregister(struct device *dev, struct hi6526_tcpc_device *tcpc)
{
	D("+\n");
...
EXPORT_SYMBOL(hi6526_tcpc_device_unregister);

static int __init hi6526_tcpc_class_init(void)
{
	D("+\n");

	if (!hi6526_tcpc_ready()) {
		E("Not support Hi6526 PD!\n");
		return -ENODEV;
	}

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	hi6526_dpm_check_supported_modes();
#endif

	hi6526_tcpc_class = class_create(THIS_MODULE, "tcpc");
	if (IS_ERR(hi6526_tcpc_class)) {
		E("Unable to create tcpc class; errno = %ld\n",
			PTR_ERR(hi6526_tcpc_class));
		return PTR_ERR(hi6526_tcpc_class);
	}

	tcpc_init_attrs(&tcpc_dev_type);
	hi6526_tcpc_class->suspend = NULL;
	hi6526_tcpc_class->resume = NULL;

	D("-\n");
	return 0;
}

static void __exit hi6526_tcpc_class_exit(void)
{
	D("\n");
	class_destroy(hi6526_tcpc_class);
}

device_initcall(hi6526_tcpc_class_init);
module_exit(hi6526_tcpc_class_exit);

MODULE_DESCRIPTION("Hi6526 tcpc");
MODULE_LICENSE("GPL");
