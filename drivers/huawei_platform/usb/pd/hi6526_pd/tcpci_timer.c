...
static uint64_t tcpc_get_timer_enable_mask(struct hi6526_tcpc_device *tcpc)
{
	uint64_t data;
	unsigned long flags;
...
static void tcpc_reset_timer_enable_mask(struct hi6526_tcpc_device *tcpc)
{
	unsigned long flags;
...
static void tcpc_clear_timer_enable_mask(struct hi6526_tcpc_device *tcpc, uint32_t nr)
{
	unsigned long flags;
...
static void tcpc_set_timer_enable_mask(struct hi6526_tcpc_device *tcpc, uint32_t nr)
{
	unsigned long flags;
...
static uint64_t tcpc_get_timer_tick(struct hi6526_tcpc_device *tcpc)
{
	uint64_t data;
	unsigned long flags;
...
static inline void tcpc_clear_timer_tick(struct hi6526_tcpc_device *tcpc, uint32_t nr)
{
	unsigned long flags;
...
static inline void tcpc_set_timer_tick(struct hi6526_tcpc_device *tcpc, uint32_t nr)
{
	unsigned long flags;
...
static const char * const hi6526_tcpc_timer_name[] = {
...
static void on_pe_timer_timeout(struct hi6526_tcpc_device *tcpc_dev, uint32_t timer_id)
{
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	pd_event_t pd_event;

	D("timer_id %s(%u)\n", hi6526_tcpc_timer_name[timer_id], timer_id);
	...
	case PD_TIMER_UVDM_RESPONSE:
		hi6526_pd_put_vdm_event(tcpc_dev, &pd_event, false);
		break;
	...
	case PD_TIMER_VBUS_STABLE:
		hi6526_pd_put_vbus_stable_event(tcpc_dev);
		break;
#endif

#if CONFIG_USB_PD_VBUS_PRESENT_TOUT
	case PD_TIMER_VBUS_PRESENT:
		hi6526_pd_put_vbus_present_event(tcpc_dev);
		break;
#endif
	...
	default:
		hi6526_pd_put_event(tcpc_dev, &pd_event, false);
		break;
	}
#endif
...
static inline struct hi6526_tcpc_device *tcpc_hrtimer_to_dev(struct hrtimer *timer,
		int index)
{
	return container_of(timer, struct hi6526_tcpc_device, tcpc_timer[index]);
}
...
static enum hrtimer_restart tcpc_timer_bist_cont_mode(struct hrtimer *timer)
{
	...
	struct hi6526_tcpc_device *tcpc_dev = tcpc_hrtimer_to_dev(timer, index);
...
static inline void tcpc_reset_timer_range(struct hi6526_tcpc_device *tcpc,
...
void hi6526_tcpc_restart_timer(struct hi6526_tcpc_device *tcpc, uint32_t timer_id)
{
	uint64_t mask;
	...
	D("Restart %s\n", hi6526_tcpc_timer_name[timer_id]);
	mask = tcpc_get_timer_enable_mask(tcpc);
	if (mask & RT_MASK64(timer_id))
		hi6526_tcpc_disable_timer(tcpc, timer_id);
	hi6526_tcpc_enable_timer(tcpc, timer_id);
}

void hi6526_tcpc_enable_timer(struct hi6526_tcpc_device *tcpc, uint32_t timer_id)
{
	uint32_t r, mod, tout;
...
void hi6526_tcpc_disable_timer(struct hi6526_tcpc_device *tcpc_dev, uint32_t timer_id)
{
	uint64_t mask;
...
void hi6526_tcpc_timer_reset(struct hi6526_tcpc_device *tcpc_dev)
{
	uint64_t mask;
	unsigned int i;
...
bool hi6526_tcpc_timer_enabled(struct hi6526_tcpc_device *tcpc, uint32_t timer_id)
{
	unsigned long flags;
	bool enable = false;
...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
void hi6526_tcpc_reset_pe_timer(struct hi6526_tcpc_device *tcpc_dev)
{
...
void hi6526_tcpc_reset_typec_debounce_timer(struct hi6526_tcpc_device *tcpc)
{
	D("\n");
	mutex_lock(&tcpc->timer_lock);
	tcpc_reset_timer_range(tcpc, TYPEC_TIMER_START_ID,
...
void hi6526_tcpc_reset_typec_try_timer(struct hi6526_tcpc_device *tcpc)
{
	D("\n");
	mutex_lock(&tcpc->timer_lock);
...
static void tcpc_handle_timer_triggered(struct hi6526_tcpc_device *tcpc_dev)
{
	...
	for (; i < PD_TIMER_NR; i++) {
		if (triggered_timer & RT_MASK64(i)) {
			hi6526_tcpc_typec_handle_timeout(tcpc_dev, i);
			tcpc_clear_timer_tick(tcpc_dev, i);
		}
	}
...
static int hi6526_tcpc_timer_thread(void *param)
{
...
int hi6526_tcpci_timer_init(struct hi6526_tcpc_device *tcpc_dev)
{
	int i;

	D("+\n");

	tcpc_dev->timer_task = kthread_create(hi6526_tcpc_timer_thread, tcpc_dev,
			"hi6526_tcpc_timer_%s.%pK",
...
int hi6526_tcpci_timer_deinit(struct hi6526_tcpc_device *tcpc_dev)
{
	uint64_t mask;
	unsigned int i;
