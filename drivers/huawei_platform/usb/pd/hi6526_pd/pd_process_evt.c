...
const char * const hi6526_pd_dpm_pd_request_name[] = {
	"pr_swap",
	"dr_swap",
	"vs_swap",
	"gotomin",
...
static inline void print_dpm_pd_request(uint8_t msg)
{
	if (msg < PD_DPM_PD_REQUEST_NR) {
		PE_EVT_INFO("dpm_pd_req(%s)\n",
				hi6526_pd_dpm_pd_request_name[msg]);
		D("dpm_pd_req(%s)\n", hi6526_pd_dpm_pd_request_name[msg]);
	}
}
...
bool hi6526_pd_make_pe_state_transit(pd_port_t *pd_port, uint8_t curr_state,
...
bool hi6526_pd_make_pe_state_transit_virt(pd_port_t *pd_port,
		uint8_t curr_state, const pe_state_reaction_t *state_reaction)
{
	bool ret = false;

	ret = hi6526_pd_make_pe_state_transit(
...
bool hi6526_pd_make_pe_state_transit_force(
...
bool hi6526_pd_make_pe_state_transit_single(pd_port_t *pd_port,
...
bool hi6526_pd_process_protocol_error(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
static bool hi6526_pd_process_cable_ctrl_msg_accept(
...
static bool hi6526_pd_process_event_cable(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ret = false;

#ifdef CONFIG_USB_PD_RESET_CABLE
	if (pd_event->msg == PD_CTRL_ACCEPT)
		ret = hi6526_pd_process_cable_ctrl_msg_accept(pd_port, pd_event);
#endif
	return ret;
}

bool hi6526_pd_process_data_msg_bist(pd_port_t *pd_port, pd_event_t *pd_event)
{
	if (!pd_check_pe_state_ready(pd_port))
		return false;

	if (pd_port->request_v > 5000) { /* Vbus: Vsafe_5V */
	...
	case BDO_MODE_TEST_DATA:
		...
		hi6526_pd_noitfy_pe_bist_mode(pd_port, PD_BIST_MODE_TEST_DATA);
		return true;

	case BDO_MODE_CARRIER2:
		D("bist_cm2\n");
		PE_TRANSIT_STATE(pd_port, PE_BIST_CARRIER_MODE_2);
		hi6526_pd_noitfy_pe_bist_mode(pd_port, PD_BIST_MODE_DISABLE);
		return true;
	...
	case BDO_MODE_EYE:
		...
		hi6526_pd_set_rx_enable(pd_port, PD_RX_CAP_PE_DISABLE);
		hi6526_pd_noitfy_pe_bist_mode(pd_port, PD_BIST_MODE_DISABLE);
		return false;
	}
}
...
bool hi6526_pd_process_ctrl_msg_dr_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool reject = false;
	...
	if (reject) {
		hi6526_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_REJECT);
		return false;
	}
 ...
 * for issue a dr swap
 */
static bool hi6526_pd_process_dpm_msg_dr_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
 ...
 */
bool hi6526_pd_process_ctrl_msg_pr_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool reject = false;
 ...
 */
static bool hi6526_pd_process_dpm_msg_pr_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
bool hi6526_pd_process_ctrl_msg_vconn_swap(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	if (!pd_check_pe_state_ready(pd_port))
		return false;
 ...
 */
static bool hi6526_pd_process_dpm_msg_vconn_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	if (!(pd_port->dpm_caps & DPM_CAP_LOCAL_VCONN_SUPPLY))
		return false;
...
static void hi6526_pd_copy_msg_data(pd_port_t *pd_port, uint8_t *payload,
...
#ifdef CONFIG_USB_PD_REV30
static void hi6526_pd_copy_msg_data_from_ext_evt(
		pd_port_t *pd_port, pd_msg_t *pd_msg)
{
	...
	hi6526_pd_copy_msg_data(pd_port, ext_data, size, 1);
}
#endif

static void hi6526_pd_copy_msg_data_from_evt(
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_msg_t *pd_msg = pd_event->pd_msg;
	...
	switch (pd_event->event_type) {
	case PD_EVT_DATA_MSG:
		hi6526_pd_copy_msg_data(pd_port, (uint8_t *)pd_msg->payload,
			pd_get_msg_hdr_cnt(pd_port), sizeof(uint32_t));
		break;

#ifdef CONFIG_USB_PD_REV30
	case PD_EVT_EXT_MSG:
		hi6526_pd_copy_msg_data_from_ext_evt(pd_port, pd_msg);
		return;
#endif

	default:
		hi6526_pd_copy_msg_data(pd_port, NULL, 0, 0);
	}
}

bool hi6526_pd_process_recv_hard_reset(pd_port_t *pd_port,
...
static bool hi6526_pd_process_dpm_msg_pw_request(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
...
static bool hi6526_pd_process_dpm_msg_bist_cm2(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	hi6526_pd_send_data_msg(pd_port, TCPC_TX_SOP, PD_DATA_BIST, 1, &bist);

	return false;
}

static bool hi6526_pd_process_dpm_msg_gotomin(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
static bool hi6526_pd_process_dpm_msg_softreset(pd_port_t *pd_port, pd_event_t *pd_event)
{
	if (!pd_check_pe_state_ready(pd_port))
		return false;
...
static int hi6526_pd_handle_tcp_event_cable_softreset(pd_port_t *pd_port)
{
#ifdef CONFIG_PD_DFP_RESET_CABLE
	if (!pd_check_pe_state_ready(pd_port))
		return false;
...
static bool hi6526_pd_process_dpm_msg_hardreset(pd_port_t *pd_port, pd_event_t *pd_event)
{
	if (!pd_check_pe_state_ready(pd_port))
		return false;
...
static bool hi6526_pd_process_dpm_msg_get_source_cap(
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	switch (pd_port->pe_state_curr) {
	case PE_SNK_READY:
...
static bool hi6526_pd_process_dpm_msg_get_sink_cap(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	switch (pd_port->pe_state_curr) {
	case PE_SRC_READY:
 ...
 * DPM event haldler
 */
static bool hi6526_pd_process_event_dpm_pd_request(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	bool ret = false;
...
#endif
		ret = hi6526_pd_process_dpm_msg_pr_swap(pd_port, pd_event);
		break;
...
#endif
		ret = hi6526_pd_process_dpm_msg_dr_swap(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_VCONN_SWAP:
		ret = hi6526_pd_process_dpm_msg_vconn_swap(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_GOTOMIN:
		ret = hi6526_pd_process_dpm_msg_gotomin(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_SOFTRESET:
		ret = hi6526_pd_process_dpm_msg_softreset(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_CBL_SOFTRESET:
		ret = hi6526_pd_handle_tcp_event_cable_softreset(pd_port);
		break;

	case PD_DPM_PD_REQUEST_HARDRESET:
		ret = hi6526_pd_process_dpm_msg_hardreset(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_GET_SOURCE_CAP:
		ret = hi6526_pd_process_dpm_msg_get_source_cap(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_GET_SINK_CAP:
		ret = hi6526_pd_process_dpm_msg_get_sink_cap(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_PW_REQUEST:
		ret = hi6526_pd_process_dpm_msg_pw_request(pd_port, pd_event);
		break;

	case PD_DPM_PD_REQUEST_BIST_CM2:
		ret = hi6526_pd_process_dpm_msg_bist_cm2(pd_port, pd_event);
		break;
		...
		pd_event_t *pd_event)
{
	...
	if (pd_msg->frame_type == TCPC_TX_SOP_PRIME)
		hi6526_pd_sync_sop_prime_spec_revision(
...
static bool pe_exit_idle_state(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (act_as_sink)
		hi6526_pd_init_role(pd_port, PD_ROLE_SINK,
			PD_ROLE_UFP, PD_ROLE_VCONN_OFF);
	else
		hi6526_pd_init_role(pd_port, PD_ROLE_SOURCE,
	...
	hi6526_pd_notify_pe_running(pd_port);
	hi6526_pd_dpm_notify_pe_startup(pd_port);
		...
		pd_event_t *pd_event)
{
	...
	case PE_IDLE1:
		if (pd_event_msg_match(pd_event, PD_EVT_PE_MSG, PD_PE_IDLE))
			return false;
		hi6526_pd_try_put_pe_idle_event(pd_port);
		/* fallthrough */
	case PE_IDLE2:
		/*
		 * for mqp test:
		...
		if ((pd_event->event_type == PD_EVT_PE_MSG) &&
				(hi6526_pd_convert_through_u8(pd_event->msg) ==
				PD_PE_VDM_RESET)) {
	...
	if (pd_event->event_type == PD_EVT_HW_MSG) {
		switch (hi6526_pd_convert_through_u8(pd_event->msg)) {
		case PD_HW_CC_ATTACHED:
			trap = false;
			break;

		case PD_HW_CC_DETACHED:
			hi6526_pd_notify_pe_idle(pd_port);
			break;
...
static bool __hi6526_pd_process_event(pd_port_t *pd_port, pd_event_t *pd_event,
		bool vdm_evt)
{
	bool ret = false;

	hi6526_pd_copy_msg_data_from_evt(pd_port, pd_event);

	/* vdm route */
	if (vdm_evt) {
		D("vdm event\n");
		return hi6526_pd_process_event_vdm(pd_port, pd_event);
	}

	if (pd_event->event_type == PD_EVT_TCP_MSG)
		return hi6526_pd_process_event_tcp(pd_port, pd_event);

#ifdef CONFIG_USB_PD_CUSTOM_DBGACC_SUPPORT
	if (pd_port->custom_dbgacc) {
		D("custom dbgacc\n");
		return hi6526_pd_process_event_dbg(pd_port, pd_event);
	}
#endif
	...
	if ((pd_event->event_type == PD_EVT_CTRL_MSG) &&
		...
		return hi6526_pd_process_event_cable(pd_port, pd_event);
	}

	/* Special DPM event route */
	if (pd_event_msg_match(pd_event, PD_EVT_DPM_MSG, PD_DPM_PD_REQUEST)) {
		D("PD_DPM_PD_REQUEST\n");
		return hi6526_pd_process_event_dpm_pd_request(pd_port, pd_event);
	}

	switch (pd_port->state_machine) {
	case PE_STATE_MACHINE_DR_SWAP:
		ret = hi6526_pd_process_event_drs(pd_port, pd_event);
		break;

	case PE_STATE_MACHINE_PR_SWAP:
		ret = hi6526_pd_process_event_prs(pd_port, pd_event);
		break;

	case PE_STATE_MACHINE_VCONN_SWAP:
		ret = hi6526_pd_process_event_vcs(pd_port, pd_event);
		break;
	...
	/* power_role initialized in hi6526_pd_init_role, when exit idle */
	if (pd_port->power_role == PD_ROLE_SINK)
		ret = hi6526_pd_process_event_snk(pd_port, pd_event);
	else
		ret = hi6526_pd_process_event_src(pd_port, pd_event);

	return ret;
}

bool hi6526_pd_process_event(pd_port_t *pd_port, pd_event_t *pd_event,
		bool vdm_evt)
{
	bool ret = false;
	...
	ret = __hi6526_pd_process_event(pd_port, pd_event, vdm_evt);
