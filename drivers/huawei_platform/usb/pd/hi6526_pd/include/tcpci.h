...
void hi6526_tcpc_dual_role_instance_changed(
		struct dual_role_phy_instance *dual_role);
int hi6526_tcpc_dual_role_phy_init(struct hi6526_tcpc_device *tcpc);
int usb_typec_otg_pwr_src(void);
void hi6526_tcpc_poll_cc_status(struct hi6526_tcpc_device *tcpc);
void hi6526_tcpci_vbus_level_init(struct hi6526_tcpc_device *tcpc,
		uint16_t power_status);
int hi6526_tcpci_alert(struct hi6526_tcpc_device *tcpc);
int hi6526_tcpci_report_usb_port_changed(struct hi6526_tcpc_device *tcpc);
int hi6526_tcpci_report_power_control(struct hi6526_tcpc_device *tcpc, bool en);
const char *pd_pe_state_name(uint8_t pe_state);

static inline int tcpci_check_vbus_valid(struct hi6526_tcpc_device *tcpc)
{
	return tcpc->vbus_level >= TCPC_VBUS_VALID;
}

static inline int tcpci_check_vsafe0v(struct hi6526_tcpc_device *tcpc, bool detect_en)
{
	return (tcpc->vbus_level == TCPC_VBUS_INVALID);
}

static inline void tcpci_alert_status_clear(struct hi6526_tcpc_device *tcpc,
		uint32_t mask)
{
	(void)tcpc->ops->alert_status_clear(tcpc, mask);
}

static inline void tcpci_fault_status_clear(struct hi6526_tcpc_device *tcpc,
		uint8_t status)
{
...
static inline int tcpci_get_alert_status(struct hi6526_tcpc_device *tcpc,
		uint32_t *alert)
{
	return tcpc->ops->get_alert_status(tcpc, alert);
}

static inline int tcpci_get_fault_status(struct hi6526_tcpc_device *tcpc,
		uint8_t *fault)
{
...
static inline int tcpci_get_power_status(struct hi6526_tcpc_device *tcpc,
		uint16_t *pw_status)
{
	return tcpc->ops->get_power_status(tcpc, pw_status);
}

static inline void tcpci_set_frswap(struct hi6526_tcpc_device *tcpc, bool en)
{
	if (tcpc->ops->set_fr_swap)
		tcpc->ops->set_fr_swap(tcpc, en);
}

static inline void tcpci_sc_buck_ctrl(struct hi6526_tcpc_device *tcpc, bool enable)
{
...
static inline void tcpci_wait_source_vbus(struct hi6526_tcpc_device *tcpc, bool expect)
{
 ...
 * 1: cc changed
 */
static inline int tcpci_get_cc(struct hi6526_tcpc_device *tcpc)
{
	int ret, cc1, cc2;
...
static inline void tcpci_set_cc(struct hi6526_tcpc_device *tcpc, int pull)
{
	int ret;
...
static inline void tcpci_snk_src_set_cc(struct hi6526_tcpc_device *tcpc, int pull)
{
...
static inline bool tcpci_ccdebouce_timer_enabled(struct hi6526_tcpc_device *tcpc)
{
	return hi6526_tcpc_timer_enabled(tcpc, TYPEC_TIMER_CCDEBOUNCE);
}

static inline int tcpci_set_polarity(struct hi6526_tcpc_device *tcpc, int polarity)
{
	return tcpc->ops->set_polarity(tcpc, polarity);
}

static inline int tcpci_set_vconn(struct hi6526_tcpc_device *tcpc, int enable)
{
	struct tcp_notify tcp_noti;

	tcp_noti.en_state.en = enable != 0;
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_SOURCE_VCONN, &tcp_noti);

	return tcpc->ops->set_vconn(tcpc, enable);
}

static inline void tcpci_reset_pd_fsm(struct hi6526_tcpc_device *tcpc)
{
	if (tcpc->ops->reset_pd_fsm)
		tcpc->ops->reset_pd_fsm(tcpc);
}

static inline bool tcpci_tcpc_chip_version_v610(struct hi6526_tcpc_device *tcpc)
{
...
static inline void tcpci_set_vbus_detect(struct hi6526_tcpc_device *tcpc, bool enable)
{
...
static inline void tcpci_snk_unattach_by_vbus_bypass(struct hi6526_tcpc_device *tcpc,
		bool enable)
{
...
static inline void tcpci_tcpc_print_pd_fsm_state(struct hi6526_tcpc_device *tcpc)
{
	if (tcpc->ops->tcpc_pd_fsm_state)
		tcpc->ops->tcpc_pd_fsm_state(tcpc);
}

static inline int tcpci_tcpc_cc_is_realdetach(struct hi6526_tcpc_device *tcpc)
{
...
static inline bool tcpci_tcpc_vusb_uv_det_masked(struct hi6526_tcpc_device *tcpc)
{
...
static inline int tcpci_set_msg_header(
		struct hi6526_tcpc_device *tcpc, int power_role, int data_role)
{
...
static inline int tcpci_set_rx_enable(struct hi6526_tcpc_device *tcpc, uint8_t enable)
{
	return tcpc->ops->set_rx_enable(tcpc, enable);
}

static inline int tcpci_get_message(struct hi6526_tcpc_device *tcpc, uint32_t *payload,
		uint16_t *head, enum tcpm_transmit_type *type)
{
	return tcpc->ops->get_message(tcpc, payload, head, type);
}

static inline int tcpci_transmit(struct hi6526_tcpc_device *tcpc,
...
static inline int tcpci_set_bist_test_mode(struct hi6526_tcpc_device *tcpc, bool en)
{
	return tcpc->ops->set_bist_test_mode(tcpc, en);
}

static inline int tcpci_set_bist_carrier_mode(struct hi6526_tcpc_device *tcpc,
		uint8_t pattern)
{
	if (pattern)
...
static inline int tcpci_retransmit(struct hi6526_tcpc_device *tcpc)
{
...
static inline int tcpci_notify_typec_state(struct hi6526_tcpc_device *tcpc)
{
	struct tcp_notify tcp_noti;
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_TYPEC_STATE, &tcp_noti);

	return 0;
}

static inline int tcpci_notify_role_swap(struct hi6526_tcpc_device *tcpc,
...
static inline int tcpci_source_vbus(struct hi6526_tcpc_device *tcpc,
		uint8_t type, int mv, int ma)
{
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_SOURCE_VBUS, &tcp_noti);
...
static inline int tcpci_sink_vbus(struct hi6526_tcpc_device *tcpc,
		uint8_t type, int mv, int ma)
{
	struct tcp_notify tcp_noti;
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_SINK_VBUS, &tcp_noti);
...
static inline int tcpci_disable_vbus_control(struct hi6526_tcpc_device *tcpc)
{
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_DIS_VBUS_CTRL, &tcp_noti);
#else
...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
static inline int tcpci_notify_pd_state(struct hi6526_tcpc_device *tcpc, uint8_t connect)
{
	struct tcp_notify tcp_noti;

	tcp_noti.pd_state.connected = connect;
	srcu_notifier_call_chain(&tcpc->evt_nh, HI6526_TCP_NOTIFY_PD_STATE, &tcp_noti);

	return 0;
}

static inline void tcpci_notify_cable_vdo(struct hi6526_tcpc_device *tcpc,
		uint32_t cable_vdo)
{
	struct tcp_notify tcp_noti;

	tcp_noti.cable_vdo.vdo = cable_vdo;
	srcu_notifier_call_chain(&tcpc->evt_nh, HI6526_TCP_NOTIFY_CABLE_VDO, &tcp_noti);
}

static inline int tcpci_enter_mode(struct hi6526_tcpc_device *tcpc,
		uint16_t svid, uint8_t ops, uint32_t mode)
{
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_ENTER_MODE, &tcp_noti);
	return 0;
}

static inline int tcpci_exit_mode(struct hi6526_tcpc_device *tcpc, uint16_t svid)
{
	struct tcp_notify tcp_noti;
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_EXIT_MODE, &tcp_noti);
	return 0;
}

#ifdef CONFIG_USB_PD_ALT_MODE_SUPPORT
static inline int tcpci_report_hpd_state(struct hi6526_tcpc_device *tcpc,
		uint32_t dp_status)
{
	struct tcp_notify tcp_noti;
	int ret = 0;
	...
	if (PD_DP_CFG_DFP_D(tcpc->pd_port.local_dp_config)) {
		...
		ret = srcu_notifier_call_chain(&tcpc->evt_nh,
				HI6526_TCP_NOTIFY_AMA_DP_HPD_STATE, &tcp_noti);
	}

	return ret;
}

static inline int tcpci_dp_status_update(struct hi6526_tcpc_device *tcpc,
			uint32_t dp_status)
{
...
static inline int tcpci_dp_configure(struct hi6526_tcpc_device *tcpc,
		uint32_t dp_config)
{
	struct tcp_notify tcp_noti;
	int ret;
	...
	ret = srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_AMA_DP_STATE, &tcp_noti);

	return ret;
}

static inline int tcpci_dp_attention(struct hi6526_tcpc_device *tcpc,
		uint32_t dp_status)
{
	struct tcp_notify tcp_noti;
	...
	srcu_notifier_call_chain(&tcpc->evt_nh,
			HI6526_TCP_NOTIFY_AMA_DP_ATTENTION, &tcp_noti);
	return tcpci_report_hpd_state(tcpc, dp_status);
}

static inline int tcpci_dp_notify_status_update_done(struct hi6526_tcpc_device *tcpc,
		uint32_t dp_status, bool ack)
{
	return 0;
}

static inline int tcpci_dp_notify_config_start(struct hi6526_tcpc_device *tcpc)
{
...
static inline int tcpci_dp_notify_config_done(struct hi6526_tcpc_device *tcpc,
...
static inline int tcpci_notify_uvdm(struct hi6526_tcpc_device *tcpc, bool ack)
{
	struct tcp_notify tcp_noti;
	...
	srcu_notifier_call_chain(&tcpc->evt_nh, HI6526_TCP_NOTIFY_UVDM, &tcp_noti);
...
static inline int tcpci_notify_alert(struct hi6526_tcpc_device *tcpc, uint32_t ado)
{
	struct tcp_notify tcp_noti;

	tcp_noti.alert_msg.ado = ado;
	srcu_notifier_call_chain(&tcpc->evt_nh, HI6526_TCP_NOTIFY_ALERT, &tcp_noti);
	return 0;
}
#endif
