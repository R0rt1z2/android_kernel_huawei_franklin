...
static void wireless_charge_send_charge_uevent(struct wireless_charge_device_info *di,
	int icon_type)
{
...
static void wireless_charge_calc_avg_iout(struct wireless_charge_device_info *di)
{
	int i;
	int iout = 0;
	static int index;
...
static void wlrx_get_tx_prop(struct wireless_charge_device_info *di)
{
	int tx_type;
...
static void wlc_get_supported_max_rx_vout(struct wireless_charge_device_info *di, struct wlrx_dts *dts)
{
...
static int wireless_charge_vout_control(struct wireless_charge_device_info *di,
	int pmode_index)
{
	int ret;
	int tx_vout;
...
static void wlc_revise_vout_para(struct wireless_charge_device_info *di, struct wlrx_dts *dts)
{
	int ret;
	int vfc_reg = 0;
	int rx_vout_reg = 0;
...
static void wlc_iout_control(struct wireless_charge_device_info *di, struct wlrx_dts *dts)
{
	int ret;
...
static int wireless_charge_set_power_mode(struct wireless_charge_device_info *di,
	struct wlrx_dts *dts, int pid)
{
	int ret;
...
static void wlrx_switch_power_mode(struct wireless_charge_device_info *di,
...
static void wireless_charge_send_acc(struct wireless_charge_device_info *di, bool acc_on)
{
...
static void wireless_charge_para_init(struct wireless_charge_device_info *di)
{
	...
	wlc_set_iin_prop(dts->rx_istep, CHARGE_CURRENT_DELAY);
}
...
static void wireless_charge_control_work(struct work_struct *work)
{
	...
	struct wireless_charge_device_info *di = container_of(work,
		struct wireless_charge_device_info, wireless_ctrl_work.work);
...
static void wireless_charge_watchdog_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&g_wireless_di->wireless_watchdog_work,
...
static void wireless_charge_rx_sample_work(struct work_struct *work)
{
	...
	int rx_iout = 0;
	struct wireless_charge_device_info *di = container_of(work,
		struct wireless_charge_device_info, rx_sample_work.work);
...
{
	struct wireless_charge_device_info *di = container_of(work,
		struct wireless_charge_device_info, wireless_pwroff_reset_work);
...
static void wlc_rx_power_on_ready_handler(struct wireless_charge_device_info *di)
{
...
static int wireless_charge_chrg_event_notifier_call(struct notifier_block *chrg_event_nb,
	unsigned long event, void *data)
{
	struct wireless_charge_device_info *di = container_of(chrg_event_nb,
		struct wireless_charge_device_info, chrg_event_nb);
...
static struct power_sysfs_attr_info wireless_charge_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(wireless_charge, 0444, WIRELESS_CHARGE_SYSFS_TX_ADAPTOR_TYPE,
		tx_adaptor_type),
	...
	power_sysfs_attr_ro(wireless_charge, 0444, WIRELESS_CHARGE_SYSFS_NORMAL_CHRG_SUCC,
		normal_chrg_succ),
	power_sysfs_attr_ro(wireless_charge, 0444, WIRELESS_CHARGE_SYSFS_FAST_CHRG_SUCC,
		fast_chrg_succ),
	power_sysfs_attr_rw(wireless_charge, 0644, WIRELESS_CHARGE_SYSFS_FOD_COEF, fod_coef),
	power_sysfs_attr_rw(wireless_charge, 0644, WIRELESS_CHARGE_SYSFS_INTERFERENCE_SETTING,
		interference_setting),
	power_sysfs_attr_rw(wireless_charge, 0644, WIRELESS_CHARGE_SYSFS_RX_SUPPORT_MODE,
		rx_support_mode),
	power_sysfs_attr_rw(wireless_charge, 0644, WIRELESS_CHARGE_SYSFS_THERMAL_CTRL,
		thermal_ctrl),
	power_sysfs_attr_rw(wireless_charge, 0644, WIRELESS_CHARGE_SYSFS_IGNORE_FAN_CTRL,
		ignore_fan_ctrl),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	int temp = 0;
	struct wlrx_dts *dts = wlrx_get_dts();
	struct wireless_charge_device_info *di = dev_get_drvdata(dev);
...
static void wlrx_init_probe_para(struct wireless_charge_device_info *di)
{
	di->sysfs_data.rx_support_mode = WLC_RX_SP_ALL_MODE;
	di->qval_support_mode = WLC_RX_SP_ALL_MODE;
