...
static s32 handle_install_package(struct hwdps_install_package_t __user *udata)
{
	...
	if (copy_from_user(&kdata, udata,
		sizeof(struct hwdps_install_package_t)) != 0 ||
 ...
 * but i promise nowhere else.
 */
static long hwdps_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	s32 ret;
	...
	case HWDPS_SYNC_INSTALLED_PACKAGES:
		...
		break;
	default:
		hwdps_pr_warn("Invalid command: %d\n", cmd);
		ret = -EINVAL;
	}
	return (long)ret;
}
...
static s32 __init hwdps_init(void)
{
	...
	hwdps_pr_info("Installing "HWDPS_DEVICE_NAME" ...");
	...
	hwdps_pr_info(HWDPS_DEVICE_NAME" installed");
	return 0;
}
