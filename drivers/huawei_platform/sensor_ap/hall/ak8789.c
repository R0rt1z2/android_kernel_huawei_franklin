 ...
 * Date:    2020.07.20
 * Description: ak8789 driver module
 *
...
static int ak8789_get_info(struct hall_cdev *cdev, char *buf, unsigned int len)
{
	...
	nwr_size = snprintf(temp, lave_size, "[hall]total count:%d\n",
		data->count);
	if (nwr_size < 0) {
...
int ak8789_register_report_data(int ms)
{
	...
	queue_delayed_work(data->hall_wq,
		&temp_dev->h_delayed_work, HZ / 20); // use fixed time 20
...
static void ak8789_get_other_configs(struct hall_device *hall_dev,
	...
	struct device_node *node)
{
	...
	ak8789_get_other_configs(hall_dev, node);
...
#ifdef HALL_TEST
static int ak8789_hall_irq_time_test(int irq, struct hall_device *h_dev,
...
static int ak8789_reset_interrupt(struct hall_device *h_dev, int irq)
{
	int ret;
...
static irqreturn_t ak8789_event_handler(int irq, void *hall_dev)
{
	...
	ret = ak8789_reset_interrupt(hall_dev, irq);
	if (ret != 0) {
		hwlog_err("%s: ak8789_reset_interrupt fail\n", __func__);
		return IRQ_NONE;
	}
...
static void hall_upload_error_interrupt(struct ak8789_data *data,
	struct hall_device *hall, enum hall_type type)
{
	int count;
	int check_result;
...
static int requst_gpio_by_hall_type(struct hall_device *hall_dev,
	struct device_node *node, enum hall_type type)
{
	int ret;
	int gpio;
	const char *hall_pole = NULL;

	switch (type) {
	case NORTH:
		hall_pole = HALL_SINGLE_N_POLE;
		break;
	case SOUTH:
		hall_pole = HALL_SINGLE_S_POLE;
		break;
	case MAX_TYPE_NUM:
		return -EINVAL;
	default:
	...
	gpio = of_get_named_gpio(node, hall_pole, 0);
	if (!gpio_is_valid(gpio)) {
		hwlog_err("%s: %s is invalid\n", __func__, hall_pole);
		return -EINVAL;
	}
	ret = gpio_request((unsigned int)gpio, pole_name[type]);
	if (ret < 0) {
		hwlog_err("%s: request gpio %d fail\n", __func__, gpio);
		return ret;
	}
	ret = gpio_direction_input(gpio);
	if (ret < 0) {
		hwlog_err("%s: set gpio %d fail\n", __func__, gpio);
		return ret;
	}
	hall_dev->h_info.gpio[type] = (unsigned int)gpio;

	ret = snprintf(hall_dev->h_info.name[type],
		HALL_MAX_STRING_LEN, "hall%d_%s",
		hall_dev->hall_id, pole_name[type]);
	if (ret < 0) {
		hwlog_err("%s: to name gpio %d fail\n", __func__, gpio);
		return ret;
	}
...
static int ak8789_request_gpio(struct hall_device *hall_dev,
	struct device_node *node)
{
	int ret;
	unsigned int hall_type;
	...
	if (support_type_north(hall_type)) {
		ret = requst_gpio_by_hall_type(hall_dev, node, NORTH);
		if (ret) {
			hwlog_err("%s: north pole fail\n", __func__);
			return ret;
		}
	}

	if (support_type_south(hall_type)) {
		ret = requst_gpio_by_hall_type(hall_dev, node, SOUTH);
		if (ret) {
			hwlog_err("%s: south pole fail\n", __func__);
			goto free_north_gpio;
		}
	}
...
free_north_gpio:
	if (support_type_north(hall_type)) {
		gpio_free(hall_dev->h_info.gpio[NORTH]);
		hwlog_err("%s: north gpio is free\n", __func__);
	}
	return ret;
}
...
static int init_single_hall(struct platform_device *pdev,
	struct hall_device **hall, struct device_node *node)
{
	...
	ret = ak8789_request_gpio(p, node);
	if (ret < 0) {
...
static int get_hall_number(struct platform_device *pdev, int *num)
{
...
static int get_hall_device(struct ak8789_data *data)
{
	struct hall_device *hall = NULL;
	struct device_node *node = NULL;
	struct device_node *temp = NULL;
	...
	if ((data == NULL) | (data->pdev == NULL)) {
		hwlog_err("%s: null point\n", __func__);
		return -ENODEV;
	}

	node = data->pdev->dev.of_node;
	if (node == NULL) {
		hwlog_err("%s: null of_node\n", __func__);
		return -ENODEV;
	}
	...
	for_each_child_of_node(node, temp) {
		...
		if (strncmp(state, "on", sizeof("on")) == 0) {
			count++;
			ret = init_single_hall(data->pdev, &hall, temp);
			if (ret) {
				hwlog_err("%s: init single hall fail\n", __func__);
				return ret;
			}
...
static void ak8789_register_func(struct ak8789_data *data)
{
...
static int ak8789_probe(struct platform_device *pdev)
{
	...
	ret = get_hall_number(pdev, &num_halls);
	if (ret | (num_halls == 0)) {
		hwlog_err("%s: get hall number fail\n", __func__);
		return -ENODEV;
	}
	...
	ret = get_hall_device(data);
	if (ret)
		goto free_hall_dev;

	ak8789_register_func(data);
