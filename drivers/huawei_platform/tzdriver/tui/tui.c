...
static int check_ion_sg_table(struct sg_table *sg_table)
{
	if (sg_table == NULL) {
...
static bool tui_send_smc_cmd(int32_t event, struct mb_cmd_pack *mb_pack, struct tc_ns_smc_cmd smc_cmd)
{
	uint32_t uid;
	kuid_t kuid;
	...
	smc_cmd.operation_h_phys = virt_to_phys(&mb_pack->operation) >> HIGH_VALUES;
	smc_cmd.agent_id = event;
	smc_cmd.uid = uid;

	if (tc_ns_smc(&smc_cmd) != 0) {
		tloge("tc ns smc fail");
		return false;
	}
...
int tui_send_event(int event, struct teec_tui_parameter *tui_param)
{
	...
	if (check_value) {
		...
		if (!tui_send_smc_cmd(event, mb_pack, smc_cmd))
			ret = -1;
...
{
#ifndef CONFIG_TEE_TUI_MTK
	if (wait_event_interruptible(tui_msg_wq, g_tui_msg_flag))
...
static ssize_t tui_dbg_process_tp(char *tokens, char **begins)
{
	long value = 0;
				...
				loff_t *ppos)
{
	...
	if (TUI_POLL_TP == event_type) {
		if (tui_dbg_process_tp(tokens, &begins) != 0)
			return -EFAULT;
	}
