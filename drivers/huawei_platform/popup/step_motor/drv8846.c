...
struct pwm_spec_config pwm_config = {
	...
	.PWM_MODE_OLD_REGS.DATA_WIDTH = 1777,
	.PWM_MODE_OLD_REGS.THRESH = 888,
};
...
static enum hrtimer_restart drv_speed_ctrl_timer_func(struct hrtimer *timer)
{
	...
	if (di->cur_state == STATE_SPEED_UP) {
		...
		mt_set_pwm_wave_num_hal(0, 53824);  // wave_num
		mt_set_pwm_data_width_hal(0, 821);  // plus time
		mt_set_pwm_thresh_hal(0, 410);      // half plus time
		...
		mt_set_pwm_data_width_hal(0, 1777); // plus time
		mt_set_pwm_thresh_hal(0, 888);     // half plus time
		mt_set_pwm_enable_hal(0);
...
static int drv8846_get_pinctrl_gpio_config(struct drv_data *di)
{
	...
	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.st_en_low);
	if (ret < 0)
		hwlog_err("gpio_st_en low init failed\n");
