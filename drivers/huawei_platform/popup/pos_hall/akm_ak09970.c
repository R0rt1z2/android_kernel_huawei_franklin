...
{
#ifdef CONFIG_HUAWEI_OEMINFO
	struct oeminfo_info_user *nv_user_info = NULL;
	int ret;
	...
	hwlog_info("%s: read_calibrate_data_from_nv %d in!!\n",
		__func__, nv_number);
	nv_user_info = kzalloc(sizeof(*nv_user_info), GFP_KERNEL);
	if (!nv_user_info) {
		hwlog_err("%s: failed to allocate memory\n", __func__);
		return AK09970_RETCODE_COMMON_ERR;
	}
	nv_user_info->oeminfo_operation = OEMINFO_READ;
	nv_user_info->oeminfo_id = nv_number;
	nv_user_info->valid_size = nv_size;
	ret = oeminfo_direct_access(nv_user_info);
	if (ret != 0) {
		hwlog_err("%s: read nv %d error = %d\n", __func__, nv_number, ret);
		kfree(nv_user_info);
		return AK09970_RETCODE_COMMON_ERR;
	}
	memcpy(nv_info, nv_user_info->oeminfo_data,
		(sizeof(nv_user_info->oeminfo_data) < nv_user_info->valid_size) ?
		sizeof(nv_user_info->oeminfo_data) : nv_user_info->valid_size);
	kfree(nv_user_info);
#endif
	...
	int nv_size, const char *nv_name, const struct type_nv_data *nv_info)
{
	...
	nv_user_info->oeminfo_operation = OEMINFO_WRITE;
	nv_user_info->oeminfo_id = nv_number;
	nv_user_info->valid_size = nv_size;
	memcpy(nv_user_info->oeminfo_data, nv_info,
		(sizeof(nv_user_info->oeminfo_data) < nv_user_info->valid_size) ?
		sizeof(nv_user_info->oeminfo_data) : nv_user_info->valid_size);
	ret = oeminfo_direct_access(nv_user_info);
	if (ret != 0) {
		hwlog_err("%s: write nv %d error = %d\n", __func__, nv_number, ret);
		kfree(nv_user_info);
		return AK09970_RETCODE_COMMON_ERR;
	}
	kfree(nv_user_info);
...
static int ak09970_hard_reset()
{
	int ret;

	ret = pinctrl_select_state(pinctrl, rst_high);
	if (ret < 0) {
	...
	ret = pinctrl_select_state(pinctrl, rst_low);
	if (ret < 0) {
	...
	char *buf)
{
	...
	if (!nv_has_data || is_nv_data_error(nv_para)) {
...
static int init_ak09970_device(struct i2c_client *client)
{
	...
	// step
	rst_high = pinctrl_lookup_state(pinctrl, "gpio_rst_high");
	if (IS_ERR_OR_NULL(rst_high)) {
		hwlog_err("can not gpio_rst_high pinstate\n");
		return -EINVAL;

	}
	rst_low = pinctrl_lookup_state(pinctrl, "gpio_rst_low");
	if (IS_ERR_OR_NULL(rst_low)) {
		hwlog_err("can not lookup gpio_rst_low pinstate\n");
		return -EINVAL;
	}
	ret = ak09970_hard_reset();
	if (ret != AK09970_RETCODE_SUCCESS) {
...
static void ak09970_get_pinctrl_res()
{

	pinctrl = pinctrl_get(ak09970_dev_data->class_dev);

	if (IS_ERR(pinctrl)) {
		hwlog_err("%s could not get pinctrl\n", __func__);
		return;
	}
	hwlog_info("%s success\n", __func__);
}
...
int ak09970_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	ak09970_dev_data->class_dev = &client->dev;
	ak09970_get_pinctrl_res();
