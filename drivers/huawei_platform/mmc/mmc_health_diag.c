...
static unsigned long long mmcqd_t_usage_wr = 0; // the time which spent in function mmc_blk_issue_rw_rq in one stat cycle
static unsigned long long mmcqd_t_usage_rd = 0;
static unsigned int mmcqd_rq_size_wr = 0; // the size which read and write totally in one stat cycle
static unsigned int mmcqd_rq_size_rd = 0;
...
unsigned int mmc_calculate_ioworkload_and_rwspeed(unsigned long long time, struct request *rqc, struct gendisk *disk)
{
	...
	if (rqc)
		rq_byte = blk_rq_bytes(rqc); // get the rw size this rw
