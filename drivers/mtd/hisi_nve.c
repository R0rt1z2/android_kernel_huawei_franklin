...
#define NVE_NUM_MIN 0
#define NVE_NUM_MAX 9
#define NVE_ASCII_NUM_0  0x30
#define NVE_ASCII_CHAR_A 0x41
#define NVE_DOUBLE 2
#define NVE_TRIPLE 3
#define NVE_QUADRUPLE 4
#define NVE_SLEEP_20 20
	...
	struct nv_partittion_struct *nve_ramdisk)
{
	...
	if (g_valid_nv_item_num != valid_items) {
		pr_err("[%s]error,valid_items(%u) isnot equal to(%u),device read data is broken.\n",
				     ...
				     struct nv_partittion_struct *nve_ramdisk)
{
	...
	for (i = 0; i < (int)check_items; i++) {
		...
		if (memcpy_s(crc_data, NVE_NV_CRC_HEADER_SIZE, nv_item, NVE_NV_CRC_HEADER_SIZE) != EOK) {
			pr_err("[NVE]memcpy_s fail!\n");
			return -EINVAL;
		}
		if (memcpy_s(crc_data + NVE_NV_CRC_HEADER_SIZE, (size_t)NVE_NV_DATA_SIZE, nv_item->nv_data,
...
static int nve_read(loff_t from, size_t len, u_char *buf)
{
	...
	set_fs(get_ds());
	/*lint +e501*/
	fd = sys_open(g_nve_block_device_name, O_RDONLY, 0);
	if (fd < 0) {
...
static int nve_write(loff_t from, size_t len, u_char *buf)
{
	...
	set_fs(get_ds());
	/*lint +e501*/
	fd = sys_open(g_nve_block_device_name, O_RDWR, 0);
	if (fd < 0) {
	...
	struct nv_partittion_struct *ramdisk, uint32_t index)
{
	...
	ret = strncmp(NVE_HEADER_NAME,
		nve_partition_header->nve_partition_name, sizeof(NVE_HEADER_NAME));
	if (ret)
		return ret;
...
static int write_ramdisk_to_device(unsigned int id, struct nv_partittion_struct *ramdisk)
{
	struct nve_partition_header_struct *nve_partition_header = NULL;
	int ret;
	loff_t start_addr;
	...
	if (ret) {
		pr_err("[%s]after writing partition to device,read partition and check failed!\n",
			__func__);
		/* recover the age */
		nve_partition_header->nve_age = nve_update_age - 1;
		return ret;
	...
	struct nv_partittion_struct *ramdisk)
{
	...
	if (memset_s((void *)ramdisk, (unsigned long)NVE_PARTITION_SIZE,
		0, (unsigned long)NVE_PARTITION_SIZE) != EOK)
		return -EINVAL;
...
static int nve_restore(struct nve_ramdisk_struct *nvep)
{
	...
	if (nvep->nve_current_id == NVE_INVALID_NVM) {
		...
		if (nve_read((loff_t)nvep->nve_current_id * NVE_PARTITION_SIZE,
			(size_t)NVE_PARTITION_SIZE, (u_char *)nvep->nve_store_ramdisk)) {
	...
	ret = memcpy_s(nvep->nve_current_ramdisk,
		NVE_PARTITION_SIZE, nvep->nve_store_ramdisk, NVE_PARTITION_SIZE);
	if (ret != EOK) {
		pr_err("[%s]memcpy_s fail[%d]\n", __func__, ret);
		return ret;
	}
...
struct hisi_nve_info_user g_nv_read_info;
struct hisi_nve_info_user g_nv_write_info;
struct hisi_nve_info_user g_nv_init_info;
...
{
	if ((num >= NVE_NUM_MIN) && (num <= NVE_NUM_MAX))
		return (char)(num + '0');
	else
		return (char)((num - (NVE_NUM_MAX + 1)) + 'A');
}
...
static void nve_out_log(struct hisi_nve_info_user *user_info, int is_read)
{
	...
	for (i = 0, j = 0; i < user_info->valid_size; i++) {
		log[j++] = num2char((user_info->nv_data[i] >> NVE_QUADRUPLE) & 0xF);
...
int nve_write_test(uint32_t nv_item_num, uint32_t valid_size)
{
	...
	if (memset_s(&g_nv_write_info, sizeof(g_nv_write_info), 0, sizeof(g_nv_write_info)) != EOK)
		return NVE_TEST_ERR;
	ret = strncpy_s(g_nv_write_info.nv_name, (size_t)NV_NAME_LENGTH, "NVTEST", sizeof("NVTEST"));
	if (ret != EOK) {
		pr_err("[%s:%d]strncpy_s ret : %d\n", __func__, __LINE__, ret);
		return NVE_TEST_ERR;
	}
	g_nv_write_info.nv_number = nv_item_num;
	g_nv_write_info.valid_size = valid_size;
	g_nv_write_info.nv_operation = NV_WRITE;
	if (memset_s(g_nv_write_info.nv_data,
		(size_t)NVE_NV_DATA_SIZE, 0x0, (size_t)NVE_NV_DATA_SIZE) != EOK)
		return NVE_TEST_ERR;

	ret = memcpy_s(g_nv_write_info.nv_data,
	...
	g_nv_write_info.nv_data[NVE_NV_DATA_SIZE - 1] = '\0';
	ret = hisi_nve_direct_access(&g_nv_write_info);
	if (!ret) {
		pr_err("test nv write success!\n");
		pr_err("test nv write value:%s!\n", g_nv_write_info.nv_data);
		return NVE_TEST_OK;
	}
...
static int nve_read_init_value(uint32_t nv_item_num, uint32_t valid_size)
{
	...
	if (memset_s(&g_nv_init_info, sizeof(g_nv_init_info), 0, sizeof(g_nv_init_info)) != EOK)
		return NVE_TEST_ERR;
	ret = strncpy_s(g_nv_init_info.nv_name, (size_t)NV_NAME_LENGTH, "NVTEST", sizeof("NVTEST"));
	if (ret != EOK) {
		pr_err("[%s:%d]strncpy_s ret : %d\n", __func__, __LINE__, ret);
		return NVE_TEST_ERR;
	}
	g_nv_init_info.nv_number = nv_item_num;
	g_nv_init_info.valid_size = valid_size;
	g_nv_init_info.nv_operation = NV_READ;

	ret = hisi_nve_direct_access(&g_nv_init_info);
	if (!ret) {
		ret = memcpy_s(log_nv_data,
			       sizeof(log_nv_data),
			       g_nv_init_info.nv_data,
			       g_nv_init_info.valid_size);
		if (ret != EOK) {
			pr_err("[%s]memcpy_s fail[%d]\n", __func__, ret);
			return NVE_TEST_ERR;
		}
...
static int nve_write_init_value(void)
{
	...
	g_nv_init_info.nv_operation = NV_WRITE;
	ret = hisi_nve_direct_access(&g_nv_init_info);
	if (!ret) {
		ret = memcpy_s(log_nv_data,
				sizeof(log_nv_data),
...
int nve_read_test(uint32_t nv_item_num, uint32_t valid_size)
{
	...
	if (memset_s(&g_nv_read_info, sizeof(g_nv_read_info), 0, sizeof(g_nv_read_info)) != EOK)
		return NVE_TEST_ERR;
	ret = strncpy_s(g_nv_read_info.nv_name, (size_t)NV_NAME_LENGTH, "NVTEST", sizeof("NVTEST"));
	if (ret != EOK) {
		pr_err("[%s:%d]strncpy_s ret : %d\n", __func__, __LINE__, ret);
		return NVE_TEST_ERR;
	}
	g_nv_read_info.nv_number = nv_item_num;
	g_nv_read_info.valid_size = valid_size;
	g_nv_read_info.nv_operation = NV_READ;

	ret = hisi_nve_direct_access(&g_nv_read_info);
	if (!ret) {
		ret = memcpy_s(log_nv_data,
				sizeof(log_nv_data),
				g_nv_read_info.nv_data,
				g_nv_read_info.valid_size);
		if (ret != EOK) {
			pr_err("[%s]memcpy_s fail[%d]\n", __func__, ret);
			return NVE_TEST_ERR;
		}
...
int nve_read_write_auto(uint32_t nv_item_num, uint32_t valid_size)
{
	...
	for (i = 0; i < NVE_TEST_TIMES; i++) {
		...
		if (strncmp((const char *)g_nv_read_info.nv_data,
			    (const char *)g_nv_write_info.nv_data,
			    (size_t)NVE_TEST_VALID_SIZE) == 0) {
			pr_err("nve write and read value is same, test successed!\n");
		} else {
			pr_err("nve write and read value is not same, test failed!\n");
			return NVE_TEST_ERR;
		}
	}
...
uint64_t nve_current_partition_check_crc_test(void)
{
	...
	for (i = 0; i < NVE_TEST_STRESS_TIMES; i++) {
		...
		check_crc_end_time = ktime_get();
		if (!ret) {
			pr_err("NVE partition CRCcheck succ!cost time [%llu]ns,test_time = %d!\n",
			       (check_crc_end_time - check_crc_start_time), i);
		} else {
...
static int read_nve_to_ramdisk(void)
{
	 ...
	 */
	if (g_nve_data_struct->nve_partition_count <= NVE_TRIPLE) {
		ret = -ENODEV;
		goto out;
	}
	...
	unsigned int check_items, struct nve_ramdisk_struct *nvep)
{
	...
	if (ret) {
		...
		if (nve_read((unsigned long)nvep->nve_current_id *
			     NVE_PARTITION_SIZE,
			     NVE_PARTITION_SIZE,
			     (unsigned char *)nvep->nve_current_ramdisk)) {
		...
		struct hisi_nve_info_user *user_info)
{
	...
	if (user_info->nv_operation == NV_READ) {
		...
		ret = memcpy_s(user_info->nv_data,
		 ...
		 */
		ret = memcpy_s(&nv_item_backup,
			sizeof(struct nv_items_struct), nv_item, sizeof(struct nv_items_struct));
		if (ret != EOK) {
			pr_err("[NVE]memcpy_s fail[%d]\n", ret);
			return ret;
		}
		/* write nv to ram */
		ret = memset_s(nv_item->nv_data,
			(unsigned long)NVE_NV_DATA_SIZE, 0x0, (unsigned long)NVE_NV_DATA_SIZE);
		if (ret != EOK) {
			pr_err("[NVE]memset_s fail[%d]\n", ret);
			return ret;
		}
		ret = memcpy_s(nv_item->nv_data, (unsigned long)NVE_NV_DATA_SIZE,
			user_info->nv_data, (unsigned long)user_info->valid_size);
		if (ret != EOK) {
			pr_err("[NVE]memcpy_s fail[%d]\n", ret);
			return ret;
		}
		...
		if (ret) {
			 ...
			 */
			ret = memcpy_s(nv_item, sizeof(struct nv_items_struct),
				&nv_item_backup, sizeof(struct nv_items_struct));
			if (ret != EOK)
				pr_err("[NVE]memcpy_s fail[%d]\n", ret);
			/* recover the current id */
...
static int nve_setup(const char *val, const struct kernel_param *kp)
{
	...
	ret = memcpy_s(g_nve_block_device_name,
				   strlen(val) + 1,
				   val,
				   strlen(val) + 1);
	if (ret != EOK) {
