...
int send_als_calibrate_data_to_mcu(int32_t tag)
{
	...
	if (send_calibrate_data_to_mcu(tag, SUB_CMD_SET_OFFSET_REQ,
		dev_info->als_sensor_calibrate_data, ALS_CALIDATA_NV_ONE_SIZE, false))
		return -1;
...
int send_als_under_tp_calibrate_data_to_mcu(int32_t tag)
{
	...
	send_calibrate_data_to_mcu(tag, SUB_CMD_SET_ALS_UD_CALIB_DATA,
		(const void *)&dev_info->als_under_tp_cal_data,
		sizeof(dev_info->als_under_tp_cal_data), false);
	...
	struct als_platform_data *pf_data, struct als_device_info *dev_info)
{
	...
	send_calibrate_data_to_mcu(tag, SUB_CMD_SET_ALS_UD_CALIB_DATA,
		(const void *)&dev_info->als_under_tp_cal_data,
		sizeof(dev_info->als_under_tp_cal_data), true);
}
