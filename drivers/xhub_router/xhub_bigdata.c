...
static const char * const big_data_str_map[] = {
...
static int iomcu_big_data_fetch(uint32_t event_id, void *data, uint32_t length)
{
	...
	if (g_xhub_state != XHUB_ST_NORMAL) {
		hwlog_err("%s fail g_xhub_state=%d\n", __func__, g_xhub_state);
		return -1;
	}
...
static int process_big_data(uint32_t event_id, void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(big_data_event); ++i) {
