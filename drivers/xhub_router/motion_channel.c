...
static int extend_step_counter_process(bool enable)
{
	...
	xhub_sensor_enable_stepcounter(enable, TYPE_EXTEND);
	if (enable) {
		app_config[0] = enable;
		app_config[1] = TYPE_EXTEND;

		xhub_sensor_setdelay(TAG_STEP_COUNTER, &extend_open_param);
...
static int extend_step_counter_process_nolock(bool enable)
{
	...
	if (really_do_enable_disable(&step_ref_cnt, enable, TYPE_EXTEND))
		xhub_sensor_enable_nolock(TAG_STEP_COUNTER, enable);
	if (enable) {
		interval_param_t extend_open_param = {
			.period = 20, /* default delay_ms */
			.batch_count = 1,
		...
		xhub_sensor_setdelay_nolock(TAG_STEP_COUNTER,
			&extend_open_param);
	...
	if (cmd == CMD_CMN_OPEN_REQ) {
		...
		if (really_do_enable_disable(&motion_ref_cnt, true, type)) {
			if (use_lock) {
				...
				xhub_sensor_setdelay(tag, &interval_param);
			} else {
				xhub_sensor_enable_nolock(tag, true);
				xhub_sensor_setdelay_nolock(tag,
					&interval_param);
			}
		...
		if (really_do_enable_disable(&motion_ref_cnt, false, type)) {
			if (use_lock)
				...
				xhub_sensor_enable_nolock(tag, false);
...
static ssize_t mhb_read(struct file *file, char __user *buf, size_t count,
			loff_t *pos)
{
	return xhub_route_read(ROUTE_MOTION_PORT, buf, count);
}
			 ...
			 size_t len, loff_t *ppos)
{
	...
	if (motion_type == MOTIONHUB_TYPE_POPUP_CAM) {
		if (xhub_route_write(ROUTE_MOTION_PORT,
			user_data, len) == 0)
...
static int __init motionhub_init(void)
{
	...
	ret = xhub_route_open(ROUTE_MOTION_PORT);
	if (ret != 0) {
	...
	register_xhub_recovery_notifier(&motion_recovery_notify);
	read_supported_motions_type_from_dts();
	hwlog_info("%s ok\n", __func__);
	goto OUT;
CLOSE:
	xhub_route_close(ROUTE_MOTION_PORT);
OUT:
	return ret;
}
