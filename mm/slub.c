...
static inline unsigned long hw_get_canary_value(const void *canary,
	unsigned long value)
{
	return (value ^ (uintptr_t)canary) & CANARY_MASK;
}

static bool no_hw_slub_df_check(struct kmem_cache *s)
{
...
static void hw_set_canary(struct kmem_cache *s, void *object,
	unsigned long value)
{
...
static inline bool hw_check_canary(struct kmem_cache *s, void *object,
	unsigned long value)
{
	unsigned long *canary = NULL;
...
static bool hw_check_and_set_canary(struct kmem_cache *s, void *object,
	unsigned long value)
{
	unsigned long *canary = NULL;
...
#ifdef CONFIG_HW_SLUB_DF
	if (unlikely(fill_random_malloc))
		hw_set_canary(kmem_cache_node, n,
			kmem_cache_node->hw_random_malloc);
#endif
...
int set_harden_double_free_status(bool status)
{
	...
	{
		pr_err("the harden double free status is already disabled\n");
		return 0;
	}

	if (status && (slub_debug & SLAB_DOUBLEFREE_CHECK))
	{
		pr_err("the harden double free status is already enabled\n");
		return 0;
	}
#endif
