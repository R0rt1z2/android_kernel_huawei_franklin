...
static void hw_blk_lld_inline_crypto_bio_split_pre(
	const struct bio *bio, struct bio *split)
{
	if (!bio->ci_key)
		return;

	if (bio->ci_key_len != HISI_BLK_CI_KEY_LEN &&
	    bio->ci_key_len != HISI_BLK_CI_KEY_LEN_2)
...
static void hw_blk_lld_inline_crypto_bio_split_post(struct bio *bio)
{
	if (!bio->ci_key)
		return;
...
void hw_blk_bio_split_pre(const struct bio *bio, const struct bio *split)
{
	hw_blk_lld_inline_crypto_bio_split_pre(
		(struct bio *)bio, (struct bio *)split);
}

void hw_blk_bio_split_post(const struct bio *bio)
{
	hw_blk_lld_inline_crypto_bio_split_post((struct bio *)bio);
}
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO
	hw_blk_bio_split_pre(bio, split);
#endif
