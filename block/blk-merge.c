...
static bool hw_blk_lld_inline_crypto_bio_merge_allow(
...
bool hw_blk_bio_merge_allow(const struct request *rq, const struct bio *bio)
{
	if (!hw_blk_lld_inline_crypto_bio_merge_allow(rq, bio))
		return false;
	return true;
}
#endif
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO
	if (!hw_blk_bio_merge_allow(req, next->bio))
		return 0;
#endif
	/*
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO
	if (!hw_blk_bio_merge_allow(rq, bio))
		return false;
#endif
