...
static void hw_blk_lld_inline_crypto_init_request_from_bio(
	struct request *req, const struct bio *bio)
{
	if (!bio->ci_key)
		return;

	if (unlikely(bio->ci_key_len != HISI_BLK_CI_KEY_LEN &&
		     bio->ci_key_len != HISI_BLK_CI_KEY_LEN_2))
		pr_err("%s: init rq key len not %d or %d\n",
			__func__, HISI_BLK_CI_KEY_LEN, HISI_BLK_CI_KEY_LEN_2);
	req->ci_key = bio->ci_key;
	req->ci_key_len = bio->ci_key_len;
	req->ci_key_index = bio->ci_key_index;
}

void hw_blk_request_init_from_bio(struct request *req, struct bio *bio)
{
	hw_blk_lld_inline_crypto_init_request_from_bio(req, bio);
}
#endif
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO
	hw_blk_request_init_from_bio(req, bio);
#endif
